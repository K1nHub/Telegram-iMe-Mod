...
public final class CharsetJVMKt {
    ...
    public static final int encodeImpl(CharsetEncoder charsetEncoder, CharSequence input, int i, int i2, Buffer dst) {
        ...
        ByteBuffer m1912getMemorySK3TCg8 = dst.m1912getMemorySK3TCg8();
        int writePosition = dst.getWritePosition();
        int limit = dst.getLimit() - writePosition;
        ByteBuffer m1907slice87lwejk = Memory.m1907slice87lwejk(m1912getMemorySK3TCg8, writePosition, limit);
        CoderResult result = charsetEncoder.encode(wrap, m1907slice87lwejk, false);
        if (result.isMalformed() || result.isUnmappable()) {
            Intrinsics.checkNotNullExpressionValue(result, "result");
            throwExceptionWrapped(result);
        }
        if (!(m1907slice87lwejk.limit() == limit)) {
            throw new IllegalStateException("Buffer's limit change is not allowed".toString());
        }
        dst.commitWritten(m1907slice87lwejk.position());
    ...
    public static final boolean encodeComplete(CharsetEncoder charsetEncoder, Buffer dst) {
        ...
        int limit = dst.getLimit() - writePosition;
        ByteBuffer m1907slice87lwejk = Memory.m1907slice87lwejk(m1912getMemorySK3TCg8, writePosition, limit);
        CoderResult result = charsetEncoder.encode(EmptyCharBuffer, m1907slice87lwejk, true);
        if (result.isMalformed() || result.isUnmappable()) {
            Intrinsics.checkNotNullExpressionValue(result, "result");
            throwExceptionWrapped(result);
        }
