...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    public static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f460ws;
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                for (UnicastSubject<T> unicastSubject : this.f460ws) {
        ...
        void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f460ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (poll instanceof WindowOperation) {
                        ...
                        UnicastSubject<T> unicastSubject3 = windowOperation.f461w;
                        if (unicastSubject3 != null) {
                            if (list.remove(unicastSubject3)) {
                                windowOperation.f461w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    disposeBoundary();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f459w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    public static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f461w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f461w = unicastSubject;
            this.open = b;
        }
    }
    ...
    public static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f459w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f459w = unicastSubject;
        }
