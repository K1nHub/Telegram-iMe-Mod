...
public class OkHttpClientTransport implements ConnectionClientTransport, ExceptionHandlingFrameWriter.TransportExceptionHandler {
    ...
    private void startStream(OkHttpClientStream okHttpClientStream) {
        Preconditions.checkState(okHttpClientStream.m677id() == -1, "StreamId already assigned");
            ...
            public void run() {
                ...
                try {
                    try {
                        ...
                        okHttpClientTransport4.attributes = okHttpClientTransport4.attributes.toBuilder().set(Grpc.TRANSPORT_ATTR_REMOTE_ADDR, sSLSocket.getRemoteSocketAddress()).set(Grpc.TRANSPORT_ATTR_LOCAL_ADDR, sSLSocket.getLocalSocketAddress()).set(Grpc.TRANSPORT_ATTR_SSL_SESSION, sSLSession).set(GrpcAttributes.ATTR_SECURITY_LEVEL, sSLSession == null ? SecurityLevel.NONE : SecurityLevel.PRIVACY_AND_INTEGRITY).build();
                        OkHttpClientTransport okHttpClientTransport5 = OkHttpClientTransport.this;
                        okHttpClientTransport5.clientFrameHandler = new ClientFrameHandler(okHttpClientTransport5, http2.newReader(buffer2, true));
                        synchronized (OkHttpClientTransport.this.lock) {
                    ...
                    okHttpClientTransport6.clientFrameHandler = new ClientFrameHandler(okHttpClientTransport6, http2.newReader(buffer, true));
                    throw th;
                }
            }
        });
    ...
    void sendConnectionPrefaceAndSettings() {
        synchronized (this.lock) {
            ...
            if (i > 65535) {
                this.frameWriter.windowUpdate(0, i - RtpPacket.MAX_SEQUENCE_NUMBER);
            }
        }
    }
    ...
    private Request createHttpProxyRequest(InetSocketAddress inetSocketAddress, String str, String str2) {
        ...
        Request.Builder header = url.header("Host", build.host() + ":" + build.port()).header(RtspHeaders.USER_AGENT, this.userAgent);
