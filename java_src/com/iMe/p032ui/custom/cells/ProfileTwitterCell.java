...
import com.iMe.p032ui.custom.SmallActionButton;
...
public final class ProfileTwitterCell extends TextDetailCell implements KoinComponent {
    ...
    public final void setupWithTwitterStatus(TwitterAccountStatus twitterAccountStatus) {
        ...
        if (twitterAccountStatus instanceof TwitterAccountStatus.Active) {
            setTextAndValue('@' + ((TwitterAccountStatus.Active) twitterAccountStatus).getNickname(), getResourceManager().getString(C3316R.string.drawer_social_network_twitter), false);
            ViewExtKt.gone(getButtonAdd());
            AppCompatImageView imageView = getImageView();
            ViewExtKt.visible(imageView);
            imageView.setImageResource(C3316R.C3318drawable.fork_drawer_social_twitter);
            ViewExtKt.setImageColor(imageView, getResourceManager().getColor(C3316R.C3317color.twitter));
        } else if (twitterAccountStatus instanceof TwitterAccountStatus.Error) {
            setTextAndValue('@' + ((TwitterAccountStatus.Error) twitterAccountStatus).getNickname(), getResourceManager().getString(C3316R.string.drawer_social_network_twitter), false);
            ViewExtKt.gone(getButtonAdd());
            AppCompatImageView imageView2 = getImageView();
            ViewExtKt.visible(imageView2);
            imageView2.setImageResource(C3316R.C3318drawable.msg_report);
            ViewExtKt.setImageColor(imageView2, getResourceManager().getColor(C3316R.C3317color.common_red));
        } else if (Intrinsics.areEqual(twitterAccountStatus, TwitterAccountStatus.None.INSTANCE)) {
            setTextAndValue(getResourceManager().getString(C3316R.string.profile_twitter_not_connected), getResourceManager().getString(C3316R.string.drawer_social_network_twitter), false);
    ...
    public final SmallActionButton createAddButton() {
        ...
        smallActionButton.setText(LocaleController.getString("Add", C3316R.string.Add));
