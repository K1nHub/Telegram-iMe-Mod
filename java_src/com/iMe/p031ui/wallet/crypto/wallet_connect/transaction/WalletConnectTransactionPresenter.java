...
public final class WalletConnectTransactionPresenter extends BasePresenter<WalletConnectTransactionView> {
    ...
    public final DialogModel getSendConfirmationDialogModel() {
        ...
        if (walletConnectTransactionScreenType instanceof WalletConnectTransactionScreenType.Send) {
            return new DialogModel(this.resourceManager.getString(C3290R.string.wallet_amount_confirm_alert_title), getConfirmMessage(), this.resourceManager.getString(C3290R.string.common_cancel), this.resourceManager.getString(C3290R.string.wallet_amount_confirm_alert_ok_btn));
        }
        if (walletConnectTransactionScreenType instanceof WalletConnectTransactionScreenType.Sign) {
            return new DialogModel(this.resourceManager.getString(C3290R.string.wallet_connect_message_sign_confirm_title), getConfirmMessage(), this.resourceManager.getString(C3290R.string.common_cancel), this.resourceManager.getString(C3290R.string.wallet_connect_transaction_button_sign));
        }
    ...
    private final void subscribeToTransactionProcessingWithResultsHandle(Observable<Result<String>> observable) {
        ...
        Disposable subscribe = RxExtKt.withLoadingDialog((Observable) observeOn, (BaseView) viewState, true).subscribe(new RxExtKt$sam$i$io_reactivex_functions_Consumer$0(new C2225xc1738946(this)), new RxExtKt$sam$i$io_reactivex_functions_Consumer$0(new C2226xc1738947((BaseView) getViewState())));
    ...
    private final String getConfirmMessage() {
        ...
        WalletConnectTransactionScreenType walletConnectTransactionScreenType = this.screenType;
        if (walletConnectTransactionScreenType instanceof WalletConnectTransactionScreenType.Send) {
            i = C3290R.string.wallet_amount_send_confirm_alert_description;
        } else if (!(walletConnectTransactionScreenType instanceof WalletConnectTransactionScreenType.Sign)) {
            throw new NoWhenBranchMatchedException();
        } else {
            i = C3290R.string.wallet_connect_transaction_confirm_sign_description;
        }
