...
public final class TokenBalanceProvider extends BaseNodeProvider<AccountItem.Token> {
    ...
    public void convert(BaseViewHolder helper, AccountItem.Token item) {
        ...
        int i2 = C3290R.C3293id.constraint_trade_info;
        ...
        int i9 = C3290R.C3293id.text_account_balance;
        BaseViewHolder imageResource = BaseQuickAdapterExtKt.setMediumTypeface(BaseQuickAdapterExtKt.setThemedTextColor(themedTextColor5, i9, Theme.key_chat_messagePanelText), i9, i4, i8, i6, i7).setImageResource(C3290R.C3293id.image_coin_icon, TokenInfoExtKt.getLogo(item.getBalance().getInfo(), this.cryptoPreferenceHelper.getNetworkType()));
        int i10 = C3290R.C3293id.image_price_direction;
        BaseViewHolder themedImageColor = BaseQuickAdapterExtKt.setThemedImageColor(BaseQuickAdapterExtKt.setThemedImageColor(imageResource.setImageResource(i10, PriceRateExtKt.getPriceDirection(item.getBalance().getPriceRate()).getIcon()), i10, PriceRateExtKt.getPriceDirection(item.getBalance().getPriceRate()).getColorKey()), C3290R.C3293id.image_trade, i5);
        ...
        BaseQuickAdapterExtKt.applyForView(BaseQuickAdapterExtKt.setVisibleElseGone(text.setText(i4, this.resourceManager.getString(item.getBalance().getInfo().getShortName()) + " / " + this.resourceManager.getString(TokenInfo.Fiat.USD.INSTANCE.getFullName())).setText(i6, TokenBalanceExtKt.getDollarsRateText(item.getBalance(), this.resourceManager)).setText(i8, this.resourceManager.getString(C3290R.string.wallet_dashboard_balance_24h_rate, Float.valueOf(item.getBalance().getPriceRate().getRatePercentageChange24h()))).setText(i7, maskFormatter.textOrMask(this.cryptoPreferenceHelper.getCryptoHiddenBalance(), TokenBalanceExtKt.getDollarsBalanceText(item.getBalance(), this.resourceManager))), i2, item.isQuotationVisible()), C3290R.C3293id.text_coin_ticker, new TokenBalanceProvider$convert$1(item, this));
    }
    ...
    public void convert(BaseViewHolder helper, AccountItem.Token item, List<? extends Object> payloads) {
        ...
        MaskFormatter maskFormatter = MaskFormatter.INSTANCE;
        BaseViewHolder text = helper.setText(i, maskFormatter.textOrMask(this.cryptoPreferenceHelper.getCryptoHiddenBalance(), TokenBalanceExtKt.getTotalBalance(balance))).setText(C3290R.C3293id.text_account_balance_in_dollars, maskFormatter.textOrMask(this.cryptoPreferenceHelper.getCryptoHiddenBalance(), TokenBalanceExtKt.getDollarsBalanceText(balance, this.resourceManager)));
        int i2 = C3290R.C3293id.image_price_direction;
        BaseViewHolder imageResource = BaseQuickAdapterExtKt.setThemedImageColor(text.setImageResource(i2, PriceRateExtKt.getPriceDirection(balance.getPriceRate()).getIcon()), i2, PriceRateExtKt.getPriceDirection(balance.getPriceRate()).getColorKey()).setImageResource(C3290R.C3293id.image_coin_icon, TokenInfoExtKt.getLogo(balance.getInfo(), this.cryptoPreferenceHelper.getNetworkType()));
        int i3 = C3290R.C3293id.text_trade_percent;
        BaseQuickAdapterExtKt.setThemedTextColor(imageResource, i3, PriceRateExtKt.getPriceDirection(balance.getPriceRate()).getColorKey()).setText(C3290R.C3293id.text_trade_price, TokenBalanceExtKt.getDollarsRateText(balance, this.resourceManager)).setText(i3, this.resourceManager.getString(C3290R.string.wallet_dashboard_balance_24h_rate, Float.valueOf(balance.getPriceRate().getRatePercentageChange24h())));
    }
}
