...
public final class TRONWalletTransferDataSourceImpl implements WalletTransferDataSource {
    ...
    public Observable<Result<CryptoTransferMetadata>> getTransferMetadata(TokenCode tokenCode, String str, String str2, NetworkType networkType) {
        ...
        Observable map = this.walletApi.getTRONCryptoTransferData(new GetDataForCryptoTransferRequest(tokenCode.getName(), str, str2, networkType.name())).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1561x101c5dbf(this.firebaseErrorHandler)));
    ...
    public Observable<Result<Boolean>> transfer(TransactionArgs args) {
        ...
        Observable flatMap = sign(args).flatMap(new ObservableExtKt$sam$i$io_reactivex_functions_Function$0(new C1562xb62448cf(this)));
