...
public final class SimplexRepositoryImpl implements SimplexRepository {
    ...
    public Observable<Result<BuyingCryptoPayment>> createBuyingCryptoPayment(Currency digitalCurrency, Currency fiatCurrency, float f, int i, String quoteId, NetworkType networkType) {
        ...
        Observable<R> map = this.simplexApi.createBuyingCryptoPaymentRequest(createBuyingCryptoPaymentRequest).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1832xab3d1946(this.firebaseErrorHandler, createBuyingCryptoPaymentRequest)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<BuyingCryptoPayment>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1831x9f6dc81b(this.errorHandler)));
    ...
    public Observable<Result<List<BuyingCryptoQuote>>> getBuyingCryptoQuote(List<BuyingCryptoQuoteArgs> quoteRequests) {
        ...
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<List<BuyingCryptoQuote>>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1833xe982d2d7(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
}
