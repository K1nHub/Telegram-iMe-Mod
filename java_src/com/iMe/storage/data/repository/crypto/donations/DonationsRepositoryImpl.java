...
public final class DonationsRepositoryImpl implements DonationsRepository {
    ...
    public Observable<Result<Boolean>> isDonationsEnabled(long j) {
        ...
        final FirebaseFunctionsErrorHandler firebaseFunctionsErrorHandler = this.firebaseErrorHandler;
        Observable<R> map = isDonationsEnable.map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new Function1<ApiBaseResponse<IsDonationsEnableResponse>, Result<? extends Boolean>>() { // from class: com.iMe.storage.data.repository.crypto.donations.DonationsRepositoryImpl$isDonationsEnabled$$inlined$mapSuccess$1
            {
                super(1);
            }
        ...
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        final ApiErrorHandler apiErrorHandler = this.errorHandler;
        Observable<Result<Boolean>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new Function1<Throwable, Result<? extends Boolean>>() { // from class: com.iMe.storage.data.repository.crypto.donations.DonationsRepositoryImpl$isDonationsEnabled$$inlined$handleError$1
            {
                super(1);
            }
