...
public final class PinCodeRepositoryImpl implements PinCodeRepository {
    ...
    public Observable<Result<String>> readPasswordByPinCode(String pinCode, String uuid, boolean z) {
        Intrinsics.checkNotNullParameter(pinCode, "pinCode");
        Intrinsics.checkNotNullParameter(uuid, "uuid");
        Observable<R> flatMap = this.pinCodeApi.getCryptoWalletPassword(new ReadPasswordByPinCodeRequest(pinCode, uuid)).flatMap(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1827x9951f988(this.firebaseErrorHandler, this, z, uuid)));
        Intrinsics.checkNotNullExpressionValue(flatMap, "errorHandler: FirebaseFu…R>().toObservable()\n    }");
        Observable<Result<String>> onErrorReturn = flatMap.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1829xfd7cbd92(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
}
