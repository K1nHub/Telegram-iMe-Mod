...
public final class CryptoLocalWalletRepositoryImpl implements CryptoLocalWalletRepository {
    ...
    public Observable<Result<Wallet>> createWallet(BlockchainType blockchainType) {
        ...
        Observable<Result<Wallet>> onErrorReturn = this.walletManager.createWallet(blockchainType).onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1779x26a9e708(this.errorHandler)));
    ...
    public Observable<Result<List<Wallet>>> unlockAllWallets(String oldGuid, String newGuid, String password) {
        ...
        if (CollectionExtKt.isSingletonList(arrayList2)) {
            Observable<Result<List<Wallet>>> map = ((Observable) CollectionsKt.first((List<? extends Object>) arrayList2)).map(new ObservableExtKt$sam$i$io_reactivex_functions_Function$0(new C1784x36424441(this, password)));
    ...
    public Observable<Result<Boolean>> isValidSeed(String seed, BlockchainType blockchainType) {
        ...
        Intrinsics.checkNotNullParameter(blockchainType, "blockchainType");
        Observable flatMap = this.walletManager.generateAddressByMnemonic(seed, blockchainType).flatMap(new ObservableExtKt$sam$i$io_reactivex_functions_Function$0(new C1783xf78a2b72(this, blockchainType)));
    ...
    public Observable<Result<Boolean>> isValidRestoredAddress(String seed, String linkedAddress, BlockchainType blockchainType) {
        ...
        Intrinsics.checkNotNullParameter(blockchainType, "blockchainType");
        Observable map = this.walletManager.generateAddressByMnemonic(seed, blockchainType).map(new ObservableExtKt$sam$i$io_reactivex_functions_Function$0(new C1782x41e0c8d0(linkedAddress)));
        Intrinsics.checkNotNullExpressionValue(map, "crossinline body: (T) ->…ult as? R\n        }\n    }");
        return map;
    }
    ...
    public Observable<Result<Wallet>> importWallet(String guid, String seed, String password, BlockchainType blockchainType) {
        ...
        Intrinsics.checkNotNullParameter(password, "password");
        Intrinsics.checkNotNullParameter(blockchainType, "blockchainType");
        Observable<R> map = this.walletManager.importWallet(guid, seed, password, blockchainType).map(new ObservableExtKt$sam$i$io_reactivex_functions_Function$0(new C1781x966593e2(this, password)));
        Intrinsics.checkNotNullExpressionValue(map, "crossinline body: (T) ->…ult as? R\n        }\n    }");
        Observable<Result<Wallet>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1780x1954a0ff(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
}
