...
public final class SwapRepositoryImpl implements SwapRepository {
    ...
    public Observable<Result<List<TokenInfo>>> getAvailableTokensToSwap(NetworkType networkType, SwapProtocol protocol) {
        ...
        Observable<R> map = this.swapApi.getAvailableTokensToSwap(new NetworkAndProtocolSwapRequest(protocol.name(), networkType.name())).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1899xd64aae2f(this.firebaseErrorHandler)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<List<TokenInfo>>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1898xd612d052(this.errorHandler)));
