...
public final class SwapInteractor {
    ...
    public final Observable<Result<CursoredData<TokenDetailed>>> getAvailableTokensToSwap(SwapProtocol protocol, TransactionDirection direction, String networkId, String str, String str2, Integer num) {
        ...
        Observable<Result<CursoredData<TokenDetailed>>> subscribeOn = this.swapRepository.getAvailableTokensToSwap(protocol, direction, networkId, str, str2, num).subscribeOn(this.schedulersProvider.mo717io());
    ...
    public final Observable<Result<List<CryptoTokenApproveMetadata>>> getApproveTokensInfo(String networkId, SwapProtocol protocol, List<Token> tokens) {
        ...
        Observable<Result<List<CryptoTokenApproveMetadata>>> startWith = this.swapRepository.getApproveTokensInfo(protocol, networkId, tokens).subscribeOn(this.schedulersProvider.mo717io()).startWith((Observable<Result<List<CryptoTokenApproveMetadata>>>) Result.Companion.loading$default(Result.Companion, null, 1, null));
    ...
    public final Observable<Result<String>> approveToken(ApproveArgs args) {
        ...
        Observable<Result<String>> subscribeOn = this.swapRepository.approveToken(args).subscribeOn(this.schedulersProvider.mo717io());
        Intrinsics.checkNotNullExpressionValue(subscribeOn, "swapRepository\n         …(schedulersProvider.io())");
        return subscribeOn;
    }
    ...
    public final Observable<Result<String>> swap(SwapArgs args) {
        Intrinsics.checkNotNullParameter(args, "args");
        Observable<Result<String>> subscribeOn = this.swapRepository.swap(args).subscribeOn(this.schedulersProvider.mo717io());
        Intrinsics.checkNotNullExpressionValue(subscribeOn, "swapRepository\n         …(schedulersProvider.io())");
        return subscribeOn;
    }
    ...
    public final Observable<Result<CryptoSwapMetadata>> getQuoteToSwap(SwapProtocol protocol, Token inputToken, Token outputToken, String amount, TradeType tradeType, float f) {
        ...
        Observable<Result<CryptoSwapMetadata>> startWith = this.swapRepository.getQuoteToSwap(protocol, inputToken, outputToken, amount, tradeType, f).subscribeOn(this.schedulersProvider.mo717io()).startWith((Observable<Result<CryptoSwapMetadata>>) Result.Companion.loading$default(Result.Companion, null, 1, null));
        Intrinsics.checkNotNullExpressionValue(startWith, "swapRepository\n         …artWith(Result.loading())");
        return startWith;
    }
}
