...
public final class TranslationInteractor {
    ...
    public final Observable<Result<Translation>> translate(String text, String targetLanguage, String str) {
        ...
        Observable<Result<Translation>> subscribeOn = this.translationRepository.translate(text, str, targetLanguage).startWith((Observable<Result<Translation>>) Result.Companion.loading$default(Result.Companion, null, 1, null)).subscribeOn(this.schedulersProvider.mo1010io());
    ...
    public final Observable<Result<List<TranslationLanguage>>> languages() {
        Observable<Result<List<TranslationLanguage>>> subscribeOn = this.translationRepository.languages().startWith((Observable<Result<List<TranslationLanguage>>>) Result.Companion.loading$default(Result.Companion, null, 1, null)).subscribeOn(this.schedulersProvider.mo1010io());
        Intrinsics.checkNotNullExpressionValue(subscribeOn, "translationRepository\n  …(schedulersProvider.io())");
        return subscribeOn;
    }
}
