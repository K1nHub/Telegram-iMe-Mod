...
public final class NotificationUiMappingKt {
    ...
    public static final PushNotificationModel mapToPushNotification(Notification notification) {
        ...
        if (notification instanceof Notification.CryptoTransfer.C1958In) {
            Notification.CryptoTransfer.C1958In c1958In = (Notification.CryptoTransfer.C1958In) notification;
            String tokenCode = c1958In.getTokenCode();
            String amount = c1958In.getAmount();
            String toAddress = c1958In.getToAddress();
            return new PushNotificationModel.CryptoTransfer.C1534In(notification.getUserId(), notification.getType().name(), tokenCode, c1958In.getFromAddress(), toAddress, amount);
        ...
        } else if (notification instanceof Notification.Donation.C1959In) {
            Notification.Donation.C1959In c1959In = (Notification.Donation.C1959In) notification;
            String tokenCode3 = c1959In.getTokenCode();
            String amount3 = c1959In.getAmount();
            String toAddress3 = c1959In.getToAddress();
            return new PushNotificationModel.Donation.C1535In(notification.getUserId(), notification.getType().name(), tokenCode3, c1959In.getFromAddress(), toAddress3, amount3);
