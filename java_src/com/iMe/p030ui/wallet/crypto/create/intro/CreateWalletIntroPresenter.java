...
public final class CreateWalletIntroPresenter extends BasePresenter<CreateWalletIntroView> {
    ...
    public final void prepareCreateWalletFlow() {
        ...
        if (Intrinsics.areEqual(initial, WalletCreationType.Initial.Create.INSTANCE)) {
            Observable<Result<Wallet>> observeOn = this.cryptoWalletInteractor.createLocalWallet(this.currentBlockchainType).observeOn(this.schedulersProvider.mo1009ui());
    ...
    private final void showCreateNewWalletConfirmationDialog() {
        ((CreateWalletIntroView) getViewState()).showCreateNewWalletConfirmationDialog(new DialogModel(this.resourceManager.getString(C3630R.string.wallet_creation_intro_new_wallet_confirmation_title), this.resourceManager.getString(C3630R.string.wallet_creation_intro_new_wallet_confirmation_description), this.resourceManager.getString(C3630R.string.common_cancel), this.resourceManager.getString(this.walletCreationType.getButtonTextResId())));
    }
}
