...
public final class StakingPercentageView extends FrameLayout implements KoinComponent {
    ...
    public final void setupViewData(StakingAnnualPercentageData stakingAnnualPercentageData, final Callbacks$Callback onInfoClickAction) {
        ...
        appCompatTextView.setText(LocaleController.getInternalString(C3632R.string.staking_programme_apr) + getAnnualPercentageText(stakingAnnualPercentageData.getApr()));
        AppCompatTextView appCompatTextView2 = forkContentStakingCompoundBinding.textApy;
        appCompatTextView2.setText(LocaleController.getInternalString(C3632R.string.staking_programme_apy) + getAnnualPercentageText(stakingAnnualPercentageData.getApy()));
    ...
    private final void setupProgressListener() {
        Observable<Float> observeOn = this.progressSubject.debounce(200L, TimeUnit.MILLISECONDS).subscribeOn(getSchedulersProvider().mo1014io()).observeOn(getSchedulersProvider().mo1013ui());
