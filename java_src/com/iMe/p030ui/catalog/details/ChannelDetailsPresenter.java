...
public final class ChannelDetailsPresenter extends BasePresenter<ChannelDetailsView> {
    ...
    public ChannelDetailsPresenter(CampaignItem campaign, TLRPC$Chat chat, ChatType chatType, SchedulersProvider schedulersProvider, TelegramApi telegramApi, TelegramControllersGateway telegramControllersGateway) {
    ...
    public final void onSubscribeClick() {
        ...
        Observable<TLRPC$Chat> observeOn = telegramApi.getChatInfoByUsername(str).observeOn(this.schedulersProvider.mo1013ui());
    ...
    public void onFirstViewAttach() {
        ...
        String lowerCase = this.chatType.name().toLowerCase(Locale.ROOT);
        Intrinsics.checkNotNullExpressionValue(lowerCase, "this as java.lang.String).toLowerCase(Locale.ROOT)");
        AnalyticsManager.trackEvent(new AnalyticsEvent.CatalogEvent.PreviewCatalogItem(title, lowerCase));
    ...
    public final Observable<Boolean> getSubscriptionStatusChangeObservable(TLRPC$Chat tLRPC$Chat) {
        ...
        if (canSubscribe) {
            ...
            String lowerCase = this.chatType.name().toLowerCase(Locale.ROOT);
            Intrinsics.checkNotNullExpressionValue(lowerCase, "this as java.lang.String).toLowerCase(Locale.ROOT)");
            AnalyticsManager.trackEvent(new AnalyticsEvent.CatalogEvent.SubscribeCatalogItem(title, lowerCase));
            unsubscribeFromChannel = this.telegramApi.subscribeToChannel(tLRPC$Chat);
        } else {
            AnalyticsManagerKt.getAnalytics();
            String title2 = this.campaign.getTitle();
            String lowerCase2 = this.chatType.name().toLowerCase(Locale.ROOT);
            Intrinsics.checkNotNullExpressionValue(lowerCase2, "this as java.lang.String).toLowerCase(Locale.ROOT)");
            AnalyticsManager.trackEvent(new AnalyticsEvent.CatalogEvent.UnsubscribeCatalogItem(title2, lowerCase2));
