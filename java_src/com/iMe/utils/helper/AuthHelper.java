...
public final class AuthHelper implements KoinComponent, NotificationCenter.NotificationCenterDelegate {
    ...
    private final void runHiddenAuthWithRemoteConfigCheck() {
        Single<Result<Boolean>> observeOn = getRemoteConfigInteractor().getBoolean("auto_auth_enabled").observeOn(getSchedulersProvider().mo1013ui());
        ...
        Disposable subscribe = observeOn.subscribe(new RxExtKt$sam$i$io_reactivex_functions_Consumer$0(new Function1<Result<? extends Boolean>, Unit>() { // from class: com.iMe.utils.helper.AuthHelper$runHiddenAuthWithRemoteConfigCheck$$inlined$subscribeWithErrorHandle$default$1
            ...
            public final void m1878invoke(Result<? extends Boolean> it) {
    ...
    public static final void runAuthButtonAction$lambda$3(String str, TLRPC$TL_messages_requestUrlAuth tLRPC$TL_messages_requestUrlAuth) {
        authSubject.onNext(Result.Companion.success(TuplesKt.m149to(str, tLRPC$TL_messages_requestUrlAuth)));
    }
    ...
    public final void prepareAndStartAuthFlow() {
        Observable<TLRPC$User> observeOn = getTelegramApi().getUserInfoByUsername(AppConfiguration$Bots.getAuthBotUsername()).observeOn(getSchedulersProvider().mo1013ui());
    ...
    private final void startAuthFlow() {
        ...
        long j = authBotId;
        isBotMutedByOriginal = messagesController.isDialogMuted(j, 0L);
        getNotificationsController().muteDialog(j, 0L, true);
        getMessagesController().setAuthBotHidden(true);
        getNotificationCenter().addObserver(this, NotificationCenter.didReceiveNewMessages);
        getSendMessagesHelper().sendMessage(SendMessagesHelper.SendMessageParams.m94of("/start", j, null, null, null, false, null, null, null, true, 0, null, false, null));
    }
    ...
    private final void subscribeToAuthSubject() {
        ...
        Observable observeOn = flatMap.observeOn(getSchedulersProvider().mo1013ui());
    ...
    public final void onAuthFlowFinished() {
        if (!isBotMutedByOriginal) {
            getNotificationsController().muteDialog(authBotId, 0L, false);
        }
        MessagesController messagesController = getMessagesController();
