...
public final class NewsAndUserActivityHelper implements KoinComponent, NotificationCenter.NotificationCenterDelegate {
    ...
    public static final void markActivity() {
        ...
        if (DateExtKt.isDayAgo(newsAndUserActivityHelper.getPreferenceHelper().getLastMarkActivityTime())) {
            ...
            Disposable subscribe = newsAndUserActivityHelper.getNotificationInteractor().sendActivityMark().observeOn(newsAndUserActivityHelper.getSchedulersProvider().mo1013ui()).subscribe();
    ...
    private final void loadBotMessages() {
        ...
        getMessagesController().loadMessages(newsBotId, 0L, true, 10, 0, 0, false, 0, classGuid, 0, 0, 0, 0L, 0, 0, false);
    }

    private final void prepareAndStartSubscribeFlow() {
        Observable<TLRPC$User> observeOn = getTelegramApi().getUserInfoByUsername(AppConfiguration$Bots.getNewsBotUsername()).observeOn(getSchedulersProvider().mo1013ui());
    ...
    public final void onSubscribeFlowFinished() {
        ...
        if (!isBotMutedByOriginal) {
            getNotificationsController().muteDialog(newsBotId, 0L, false);
        }
