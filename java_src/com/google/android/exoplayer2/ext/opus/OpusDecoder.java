...
public final class OpusDecoder extends SimpleDecoder<DecoderInputBuffer, SimpleDecoderOutputBuffer, OpusDecoderException> {
    ...
    static int getPreSkipSamples(List<byte[]> list) {
        if (list.size() == 3) {
            return (int) ((ByteBuffer.wrap(list.get(1)).order(ByteOrder.nativeOrder()).getLong() * 48000) / C0483C.NANOS_PER_SECOND);
        }
        byte[] bArr = list.get(0);
    ...
    static int getSeekPreRollSamples(List<byte[]> list) {
        return list.size() == 3 ? (int) ((ByteBuffer.wrap(list.get(2)).order(ByteOrder.nativeOrder()).getLong() * 48000) / C0483C.NANOS_PER_SECOND) : DEFAULT_SEEK_PRE_ROLL_SAMPLES;
    }
    ...
    static int getDiscardPaddingSamples(ByteBuffer byteBuffer) {
        ...
        return (int) ((j * 48000) / C0483C.NANOS_PER_SECOND);
    }
