...
public final class Id3Decoder extends SimpleMetadataDecoder {
    ...
    public Metadata decode(byte[] bArr, int i) {
        ...
        if (!validateFrames(parsableByteArray, decodeHeader.majorVersion, i2, false)) {
            if (decodeHeader.majorVersion != 4 || !validateFrames(parsableByteArray, 4, i2, true)) {
                Log.m806w(TAG, "Failed to validate ID3 tag with majorVersion=" + decodeHeader.majorVersion);
                return null;
            }
            z = true;
        }
    ...
    private static Id3Header decodeHeader(ParsableByteArray parsableByteArray) {
        ...
        if (parsableByteArray.bytesLeft() < 10) {
            Log.m806w(TAG, "Data too short to be an ID3 tag");
            return null;
        }
        boolean z = false;
        if (parsableByteArray.readUnsignedInt24() != 4801587) {
            Log.m806w(TAG, "Unexpected first three bytes of ID3 tag header: 0x" + String.format("%06X", Integer.valueOf(readUnsignedInt24)));
            return null;
        }
        ...
        if (readUnsignedByte == 2) {
            if ((readUnsignedByte2 & 64) != 0) {
                Log.m806w(TAG, "Skipped ID3 tag with majorVersion=2 and undefined compression scheme");
                return null;
            }
            ...
            Log.m806w(TAG, "Skipped ID3 tag with unsupported majorVersion=" + readUnsignedByte);
            return null;
        } else {
            if ((readUnsignedByte2 & 64) != 0) {
    ...
    */
    ...
    private static ImmutableList<String> decodeTextInformationFrameValues(byte[] bArr, int i, int i2) {
        ...
        return build.isEmpty() ? ImmutableList.m753of("") : build;
    }
