...
final class AudioTrackPositionTracker {
    ...
    public void setAudioTrack(AudioTrack audioTrack, boolean z, int i, int i2, int i3) {
        ...
        this.hasData = false;
        this.stopTimestampUs = C0483C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C0483C.TIME_UNSET;
    ...
    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C0483C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean pause() {
        resetSyncParams();
        if (this.stopTimestampUs == C0483C.TIME_UNSET) {
            ((AudioTimestampPoller) Assertions.checkNotNull(this.audioTimestampPoller)).reset();
            return true;
        }
        return false;
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C0483C.TIME_UNSET) {
