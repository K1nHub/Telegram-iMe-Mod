...
final class RtpH263Reader implements RtpPayloadReader {
    ...
    public void consume(ParsableByteArray parsableByteArray, long j, int i, boolean z) {
        ...
        if ((readUnsignedShort & 512) != 0 || (readUnsignedShort & 504) != 0 || (readUnsignedShort & 7) != 0) {
            Log.m792w(TAG, "Dropping packet: video reduncancy coding is not supported, packet header VRC, or PLEN or PEBIT is non-zero");
            return;
        }
        if (z2) {
            ...
            if ((parsableByteArray.peekUnsignedByte() & 252) < 128) {
                Log.m792w(TAG, "Picture start Code (PSC) missing, dropping packet.");
                return;
            }
            ...
            if (i < nextSequenceNumber) {
                Log.m792w(TAG, Util.formatInvariant("Received RTP packet with unexpected sequence number. Expected: %d; received: %d. Dropping packet.", Integer.valueOf(nextSequenceNumber), Integer.valueOf(i)));
                return;
            }
        } else {
            Log.m792w(TAG, "First payload octet of the H263 packet is not the beginning of a new H263 partition, Dropping current packet.");
            return;
        }
    ...
    private void outputSampleMetadataForFragmentedPackets() {
        ...
        this.fragmentedSampleSizeBytes = 0;
        this.fragmentedSampleTimeUs = C0470C.TIME_UNSET;
        this.isKeyFrame = false;
