...
final class RtpH263Reader implements RtpPayloadReader {
    ...
    public void consume(ParsableByteArray parsableByteArray, long j, int i, boolean z) {
        ...
        if ((readUnsignedShort & 512) != 0 || (readUnsignedShort & 504) != 0 || (readUnsignedShort & 7) != 0) {
            Log.m1106w(TAG, "Dropping packet: video reduncancy coding is not supported, packet header VRC, or PLEN or PEBIT is non-zero");
            return;
        }
        if (z2) {
            ...
            if ((parsableByteArray.peekUnsignedByte() & 252) < 128) {
                Log.m1106w(TAG, "Picture start Code (PSC) missing, dropping packet.");
                return;
            }
            ...
            Log.m1106w(TAG, "First payload octet of the H263 packet is not the beginning of a new H263 partition, Dropping current packet.");
            return;
        }
