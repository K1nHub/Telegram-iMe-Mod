...
final class RtpExtractor implements Extractor {
    ...
    private volatile long firstTimestamp = C0480C.TIME_UNSET;
    private volatile int firstSequenceNumber = -1;
    private long nextRtpTimestamp = C0480C.TIME_UNSET;
    private long playbackStartTimeUs = C0480C.TIME_UNSET;
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        ...
        if (!this.firstPacketRead) {
            if (this.firstTimestamp == C0480C.TIME_UNSET) {
        ...
        synchronized (this.lock) {
            ...
            } else if (this.nextRtpTimestamp != C0480C.TIME_UNSET && this.playbackStartTimeUs != C0480C.TIME_UNSET) {
                this.reorderingQueue.reset();
                this.payloadReader.seek(this.nextRtpTimestamp, this.playbackStartTimeUs);
                this.isSeekPending = false;
                this.nextRtpTimestamp = C0480C.TIME_UNSET;
                this.playbackStartTimeUs = C0480C.TIME_UNSET;
            }
        }
        return 0;
    }
