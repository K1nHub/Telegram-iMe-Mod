...
public final class RtspMessageUtil {
    ...
    /* renamed from: LF */
    private static final String f217LF = new String(new byte[]{10});
    ...
    public static byte[] convertMessageToByteArray(List<String> list) {
        return Joiner.m1058on(CRLF).join(list).getBytes(RtspMessageChannel.CHARSET);
    }
    ...
    public static RtspResponse parseResponse(List<String> list) {
        ...
        return new RtspResponse(parseInt, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m1058on(CRLF).join(list.subList(indexOf + 1, list.size())));
    }
    ...
    public static RtspRequest parseRequest(List<String> list) {
        ...
        int indexOf = list.indexOf("");
        Assertions.checkArgument(indexOf > 0);
        return new RtspRequest(parse, parseMethodString, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m1058on(CRLF).join(list.subList(indexOf + 1, list.size())));
    }
