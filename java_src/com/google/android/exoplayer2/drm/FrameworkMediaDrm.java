...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    ...
    public static /* synthetic */ ExoMediaDrm lambda$static$0(UUID uuid) {
        try {
            ...
            Log.m1111e(TAG, "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
    ...
    private FrameworkMediaDrm(UUID uuid) throws UnsupportedSchemeException {
        ...
        if (C0485C.WIDEVINE_UUID.equals(uuid) && needsForceWidevineL3Workaround()) {
    ...
    public void setPlayerIdForSession(byte[] bArr, PlayerId playerId) {
        if (Util.SDK_INT >= 31) {
            try {
                ...
                Log.m1107w(TAG, "setLogSessionId failed.");
            }
        }
    }
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws NotProvisionedException, DeniedByServerException {
        if (C0485C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public FrameworkCryptoConfig createCryptoConfig(byte[] bArr) throws MediaCryptoException {
        return new FrameworkCryptoConfig(adjustUuid(this.uuid), bArr, Util.SDK_INT < 21 && C0485C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel")));
    }

    private static DrmInitData.SchemeData getSchemeData(UUID uuid, List<DrmInitData.SchemeData> list) {
        boolean z;
        if (!C0485C.WIDEVINE_UUID.equals(uuid)) {
            return list.get(0);
        }
    ...
    private static UUID adjustUuid(UUID uuid) {
        return (Util.SDK_INT >= 27 || !C0485C.CLEARKEY_UUID.equals(uuid)) ? uuid : C0485C.COMMON_PSSH_UUID;
    }
    ...
    */
    private static byte[] adjustRequestInitData(java.util.UUID r3, byte[] r4) {
        /*
            java.util.UUID r0 = com.google.android.exoplayer2.C0485C.PLAYREADY_UUID
        ...
        L18:
            ...
            r2 = 23
            if (r1 >= r2) goto L26
            java.util.UUID r1 = com.google.android.exoplayer2.C0485C.WIDEVINE_UUID
    ...
    private static String adjustRequestMimeType(UUID uuid, String str) {
        return (Util.SDK_INT < 26 && C0485C.CLEARKEY_UUID.equals(uuid) && (MimeTypes.VIDEO_MP4.equals(str) || MimeTypes.AUDIO_MP4.equals(str))) ? "cenc" : str;
    }

    private static byte[] adjustRequestData(UUID uuid, byte[] bArr) {
        return C0485C.CLEARKEY_UUID.equals(uuid) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (readLittleEndianShort != 1 || readLittleEndianShort2 != 1) {
            Log.m1109i(TAG, "Unexpected record count or type. Skipping LA_URL workaround.");
            return bArr;
        }
        ...
        if (readString.indexOf("</DATA>") == -1) {
            Log.m1107w(TAG, "Could not find the </DATA> tag. Skipping LA_URL workaround.");
        }
