...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    public Tx3gDecoder(List<byte[]> list) {
        ...
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultFontFamily = TX3G_SERIF.equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? C0483C.SERIF_NAME : "sans-serif";
            int i = bArr[25] * 20;
    ...
    protected Subtitle decode(byte[] bArr, int i, boolean z) throws SubtitleDecoderException {
        ...
        while (this.parsableByteArray.bytesLeft() >= 8) {
            ...
            if (readInt2 == TYPE_STYL) {
                ...
                assertTrue(this.parsableByteArray.bytesLeft() >= 2);
                f = Util.constrainValue(this.parsableByteArray.readUnsignedShort() / this.calculatedVideoTrackHeight, (float) BitmapDescriptorFactory.HUE_RED, 0.95f);
            }
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.m1107w(TAG, "Truncating styl end (" + readUnsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            i = spannableStringBuilder.length();
        } else {
            i = readUnsignedShort2;
        }
        if (readUnsignedShort >= i) {
            Log.m1107w(TAG, "Ignoring styl with start (" + readUnsignedShort + ") >= end (" + i + ").");
            return;
        }
        int i2 = i;
