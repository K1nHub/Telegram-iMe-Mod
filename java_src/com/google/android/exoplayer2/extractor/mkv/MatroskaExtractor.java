...
public class MatroskaExtractor implements Extractor {
    ...
    MatroskaExtractor(EbmlReader ebmlReader, int i) {
        ...
        this.timecodeScale = C0470C.TIME_UNSET;
        this.durationTimecode = C0470C.TIME_UNSET;
        this.durationUs = C0470C.TIME_UNSET;
        this.cuesContentPosition = -1L;
        this.seekPositionAfterBuildingCues = -1L;
        this.clusterTimecodeUs = C0470C.TIME_UNSET;
    ...
    protected void endMasterElement(int i) throws ParserException {
        ...
        if (i == ID_BLOCK_GROUP) {
            ...
            if (track3.hasContentEncryption) {
                ...
                track3.drmInitData = new DrmInitData(new DrmInitData.SchemeData(C0470C.UUID_NIL, MimeTypes.VIDEO_WEBM, this.currentTrack.cryptoData.encryptionKey));
            }
            ...
            if (this.timecodeScale == C0470C.TIME_UNSET) {
    ...
    */
        ...
        if (trueHdSampleRechunker != null) {
            ...
            if (CODEC_ID_SUBRIP.equals(track.codecId) || CODEC_ID_ASS.equals(track.codecId) || CODEC_ID_VTT.equals(track.codecId)) {
                if (this.blockSampleCount > 1) {
                    Log.m792w(TAG, "Skipping subtitle sample in laced block.");
                } else {
                    long j2 = this.blockDurationUs;
                    if (j2 == C0470C.TIME_UNSET) {
                        Log.m792w(TAG, "Skipping subtitle sample with no duration.");
                    } else {
    ...
    private static byte[] formatSubtitleTimecode(long j, String str, long j2) {
        Assertions.checkArgument(j != C0470C.TIME_UNSET);
    ...
    private SeekMap buildSeekMap(LongArray longArray, LongArray longArray2) {
        int i;
        if (this.segmentContentPosition == -1 || this.durationUs == C0470C.TIME_UNSET || longArray == null || longArray.size() == 0 || longArray2 == null || longArray2.size() != longArray.size()) {
        ...
        long j = jArr2[i];
        if (j <= 0) {
            Log.m792w(TAG, "Discarding last cue point with unexpected duration: " + j);
        ...
        */
        ...
        private static Pair<String, List<byte[]>> parseFourCcPrivate(ParsableByteArray parsableByteArray) throws ParserException {
            try {
                ...
                Log.m792w(MatroskaExtractor.TAG, "Unknown FourCC. Setting mimeType to video/x-unknown");
