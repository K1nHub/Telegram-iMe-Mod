...
public class MatroskaExtractor implements Extractor {
    ...
    MatroskaExtractor(EbmlReader ebmlReader, int i) {
        ...
        this.timecodeScale = C0474C.TIME_UNSET;
        this.durationTimecode = C0474C.TIME_UNSET;
        this.durationUs = C0474C.TIME_UNSET;
        this.cuesContentPosition = -1L;
        this.seekPositionAfterBuildingCues = -1L;
        this.clusterTimecodeUs = C0474C.TIME_UNSET;
    ...
    protected void endMasterElement(int i) throws ParserException {
        ...
        if (i == ID_BLOCK_GROUP) {
            ...
            if (track3.hasContentEncryption) {
                ...
                track3.drmInitData = new DrmInitData(new DrmInitData.SchemeData(C0474C.UUID_NIL, MimeTypes.VIDEO_WEBM, this.currentTrack.cryptoData.encryptionKey));
            }
            ...
            if (this.timecodeScale == C0474C.TIME_UNSET) {
                this.timecodeScale = 1000000L;
            }
            long j3 = this.durationTimecode;
            if (j3 != C0474C.TIME_UNSET) {
    ...
    */
    ...
    private SeekMap buildSeekMap(LongArray longArray, LongArray longArray2) {
        int i;
        if (this.segmentContentPosition == -1 || this.durationUs == C0474C.TIME_UNSET || longArray == null || longArray.size() == 0 || longArray2 == null || longArray2.size() != longArray.size()) {
        ...
        long j = jArr2[i];
        if (j <= 0) {
            Log.m806w(TAG, "Discarding last cue point with unexpected duration: " + j);
    ...
    public static final class Track {
            ...
            switch (c) {
                ...
                case 2:
                    if (parseMsAcmCodecPrivate(new ParsableByteArray(getCodecPrivate(this.codecId)))) {
                        ...
                        if (pcmEncoding == 0) {
                            Log.m806w(MatroskaExtractor.TAG, "Unsupported PCM bit depth: " + this.audioBitDepth + ". Setting mimeType to " + MimeTypes.AUDIO_UNKNOWN);
                        }
                        i2 = pcmEncoding;
                        singletonList = null;
                        str = null;
                        ...
                        Log.m806w(MatroskaExtractor.TAG, "Non-PCM MS/ACM is unsupported. Setting mimeType to " + MimeTypes.AUDIO_UNKNOWN);
                    }
                    singletonList = null;
                    str = null;
                ...
                case '\b':
                    ...
                    singletonList = ImmutableList.m753of(bArr2);
                ...
                case '\n':
                    ...
                    break;
                case 11:
                    singletonList = ImmutableList.m753of(getCodecPrivate(this.codecId));
                    str = null;
                ...
                case 22:
                    if (this.audioBitDepth == 32) {
                        ...
                        Log.m806w(MatroskaExtractor.TAG, "Unsupported floating point PCM bit depth: " + this.audioBitDepth + ". Setting mimeType to " + MimeTypes.AUDIO_UNKNOWN);
                        singletonList = null;
                        str = null;
                        str4 = MimeTypes.AUDIO_UNKNOWN;
                        i2 = -1;
                ...
                case 23:
                    ...
                    if (i8 == 8) {
                        ...
                        Log.m806w(MatroskaExtractor.TAG, "Unsupported big endian PCM bit depth: " + this.audioBitDepth + ". Setting mimeType to " + MimeTypes.AUDIO_UNKNOWN);
                        singletonList = null;
                        str = null;
                        str4 = MimeTypes.AUDIO_UNKNOWN;
                        i2 = -1;
                ...
                case 24:
                    pcmEncoding = Util.getPcmEncoding(this.audioBitDepth);
                    if (pcmEncoding == 0) {
                        Log.m806w(MatroskaExtractor.TAG, "Unsupported little endian PCM bit depth: " + this.audioBitDepth + ". Setting mimeType to " + MimeTypes.AUDIO_UNKNOWN);
                        singletonList = null;
                        str = null;
                        str4 = MimeTypes.AUDIO_UNKNOWN;
                        i2 = -1;
                    ...
                    i3 = -1;
                case 25:
                    singletonList = ImmutableList.m752of(MatroskaExtractor.SSA_DIALOGUE_FORMAT, getCodecPrivate(this.codecId));
                    str = null;
        ...
        private static Pair<String, List<byte[]>> parseFourCcPrivate(ParsableByteArray parsableByteArray) throws ParserException {
            try {
                ...
                Log.m806w(MatroskaExtractor.TAG, "Unknown FourCC. Setting mimeType to video/x-unknown");
