...
public final class MediaMetricsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    private void maybeUpdateTimelineMetadata(Timeline timeline, MediaSource.MediaPeriodId mediaPeriodId) {
        ...
        if (window.durationUs != C0485C.TIME_UNSET && !window.isPlaceholder && !window.isDynamic && !window.isLive()) {
    ...
    private static int getDrmType(DrmInitData drmInitData) {
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if (uuid.equals(C0485C.WIDEVINE_UUID)) {
                return 3;
            }
            if (uuid.equals(C0485C.PLAYREADY_UUID)) {
                return 2;
            }
            if (uuid.equals(C0485C.CLEARKEY_UUID)) {
                return 6;
            }
        }
        return 1;
