...
public class C0688Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0688Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0688Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0688Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0688Storage(context.getApplicationContext());
            }
            C0688Storage c0688Storage = zab;
            lock.unlock();
            return c0688Storage;
        } catch (Throwable th) {
            zaa.unlock();
