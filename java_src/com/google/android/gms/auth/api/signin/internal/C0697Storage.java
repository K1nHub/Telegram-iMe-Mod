...
public class C0697Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0697Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0697Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0697Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0697Storage(context.getApplicationContext());
            }
            C0697Storage c0697Storage = zab;
            lock.unlock();
            return c0697Storage;
        } catch (Throwable th) {
            zaa.unlock();
