...
public class C0689Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0689Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0689Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0689Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0689Storage(context.getApplicationContext());
            }
            C0689Storage c0689Storage = zab;
            lock.unlock();
            return c0689Storage;
        } catch (Throwable th) {
            zaa.unlock();
