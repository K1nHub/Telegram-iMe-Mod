...
public class C0696Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0696Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0696Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0696Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0696Storage(context.getApplicationContext());
            }
            C0696Storage c0696Storage = zab;
            lock.unlock();
            return c0696Storage;
        } catch (Throwable th) {
            zaa.unlock();
