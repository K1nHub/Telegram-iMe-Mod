...
public class C0698Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0698Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0698Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0698Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0698Storage(context.getApplicationContext());
            }
            C0698Storage c0698Storage = zab;
            lock.unlock();
            return c0698Storage;
        } catch (Throwable th) {
            zaa.unlock();
