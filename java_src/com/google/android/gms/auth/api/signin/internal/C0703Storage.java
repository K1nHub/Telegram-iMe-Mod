...
public class C0703Storage {
    private static final Lock zaa = new ReentrantLock();
    private static C0703Storage zab;
    private final Lock zac = new ReentrantLock();
    private final SharedPreferences zad;

    @VisibleForTesting
    C0703Storage(Context context) {
        this.zad = context.getSharedPreferences("com.google.android.gms.signin", 0);
    }

    @KeepForSdk
    public static C0703Storage getInstance(Context context) {
        Preconditions.checkNotNull(context);
        Lock lock = zaa;
        lock.lock();
        try {
            if (zab == null) {
                zab = new C0703Storage(context.getApplicationContext());
            }
            C0703Storage c0703Storage = zab;
            lock.unlock();
            return c0703Storage;
        } catch (Throwable th) {
            zaa.unlock();
