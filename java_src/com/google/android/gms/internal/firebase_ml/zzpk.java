...
public final class zzpk {
    ...
    public final synchronized MappedByteBuffer load() throws FirebaseMLException {
        ...
        gmsLogger.m799d("RemoteModelLoader", "Try to load newly downloaded model file.");
        zzah = zzah(this.zzawe);
        if (zzah == null) {
            gmsLogger.m799d("RemoteModelLoader", "Loading existing model file.");
            zzah = zzmq();
        }
        if (this.zzawe || !this.zzawb.zzmj()) {
            this.zzawe = false;
            gmsLogger.m799d("RemoteModelLoader", "Initial loading, check for model updates.");
    ...
    private final MappedByteBuffer zzah(boolean z) throws FirebaseMLException {
        ...
        if (zzmc == null || zzmd == null) {
            zzass.m799d("RemoteModelLoader", "No new model is downloading.");
            return null;
        }
        ...
        sb.append(valueOf);
        gmsLogger.m799d("RemoteModelLoader", sb.toString());
        if (zzmf.intValue() == 8) {
            gmsLogger.m799d("RemoteModelLoader", "Model downloaded successfully");
            this.zzavp.zza(zzmc.NO_ERROR, true, this.zzavv, zzlu.zzw.zza.SUCCEEDED);
            ParcelFileDescriptor zzmg = this.zzawb.zzmg();
            if (zzmg == null) {
                return null;
            }
            gmsLogger.m799d("RemoteModelLoader", "moving downloaded model from external storage to private folder.");
            try {
                ...
                gmsLogger.m799d("RemoteModelLoader", valueOf2.length() != 0 ? "Moved the downloaded model to private folder successfully: ".concat(valueOf2) : new String("Moved the downloaded model to private folder successfully: "));
                this.zzawb.zze(zzmd, this.zzavv);
                if (z && this.zzawc.zzd(zza)) {
                    gmsLogger.m799d("RemoteModelLoader", "All old models are deleted.");
    ...
    private final MappedByteBuffer zzmq() throws FirebaseMLException {
        ...
        if (zzmm == null) {
            zzass.m799d("RemoteModelLoader", "No existing model file");
            return null;
        }
        try {
            return zzbw(zzmm);
