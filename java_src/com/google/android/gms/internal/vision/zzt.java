    ...
    public final T zzd() {
        synchronized (this.zzb) {
            ...
            try {
                ...
                C0717L.m768d("Cannot load thick client module, fall back to load optional module %s", format);
                try {
                    dynamiteModule = DynamiteModule.load(this.zza, DynamiteModule.PREFER_REMOTE, format);
                } catch (DynamiteModule.LoadingException e) {
                    C0717L.m765e(e, "Error loading optional module %s", format);
                    if (!this.zzf) {
                        C0717L.m768d("Broadcasting download intent for dependency %s", this.zze);
                        String str = this.zze;
