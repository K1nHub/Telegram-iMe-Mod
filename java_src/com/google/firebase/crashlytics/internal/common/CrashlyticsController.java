...
public class CrashlyticsController {
    ...
    synchronized void handleUncaughtException(final SettingsDataProvider settingsDataProvider, final Thread thread, final Throwable th) {
        ...
        logger.m746d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                ...
                public Task<Void> call() throws Exception {
                            ...
                            return settingsDataProvider.getAppSettings().onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                                ...
                                public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                                    if (appSettingsData == null) {
                                        Logger.getLogger().m738w("Received null app settings, cannot send reports at crash time.");
                    ...
                    Logger.getLogger().m744e("Tried to write a fatal exception while no session was open.");
                    return Tasks.forResult(null);
                }
            }));
        } catch (Exception e) {
            Logger.getLogger().m743e("Error handling uncaught exception", e);
        }
    }

    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m746d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(Boolean.TRUE);
        }
        Logger.getLogger().m746d("Automatic data collection is disabled.");
        Logger.getLogger().m740v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m746d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m740v("Found previous crash marker.");
    ...
    public Task<Void> submitAllReports(Task<AppSettingsData> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m740v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(null);
        }
        Logger.getLogger().m740v("Crash reports are available to be sent.");
        return waitForReportAction().onSuccessTask(new C09534(task));
    }
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m740v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m746d("Sending cached crash reports...");
                    CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                    final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                    return C09534.this.val$appSettingsDataTask.onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                        @Override // com.google.android.gms.tasks.SuccessContinuation
                        public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                            if (appSettingsData != null) {
                            ...
                            Logger.getLogger().m738w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
            ...
            public void run() {
                ...
                if (currentSessionId != null) {
                    ...
                    Logger.getLogger().m738w("Tried to write a non-fatal exception while no session was open.");
                }
            }
        });
    }
    ...
    public boolean finalizeSessions(SettingsDataProvider settingsDataProvider) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m738w("Skipping session finalization because a crash has already occurred.");
            return false;
        }
        Logger.getLogger().m740v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsDataProvider);
            Logger.getLogger().m740v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m743e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession() {
        ...
        Logger logger = Logger.getLogger();
        logger.m746d("Opening a new session with ID " + clsuuid);
    ...
    private void doCloseSessions(boolean z, SettingsDataProvider settingsDataProvider) {
        ...
        if (listSortedOpenSessionIds.size() <= z) {
            Logger.getLogger().m740v("No open sessions to be closed.");
            return;
        }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger logger = Logger.getLogger();
        logger.m740v("Finalizing native report for session " + str);
        NativeSessionFileProvider sessionFileProvider = this.nativeComponent.getSessionFileProvider(str);
        File minidumpFile = sessionFileProvider.getMinidumpFile();
        if (minidumpFile == null || !minidumpFile.exists()) {
            Logger logger2 = Logger.getLogger();
            logger2.m738w("No minidump data found for session " + str);
            return;
        }
        ...
        if (!file.mkdirs()) {
            Logger.getLogger().m738w("Couldn't create directory to store native session files, aborting.");
            return;
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m737w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m738w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m738w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m746d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        Logger logger = Logger.getLogger();
        logger.m740v("ANR feature enabled, but device is API " + i);
    }
}
