...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            logger.m1039d("Requesting settings from " + this.url);
            Logger logger2 = this.logger;
            logger2.m1033v("Settings query params were: " + queryParamsFor);
            return handleResponse(applyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m1036e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m1033v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger2 = this.logger;
        logger2.m1037e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger = this.logger;
            logger.m1030w("Failed to parse settings JSON from " + this.url, e);
            Logger logger2 = this.logger;
            logger2.m1031w("Settings response " + str);
            return null;
        }
    }
