...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            logger.m738d("Requesting settings from " + this.url);
            Logger logger2 = this.logger;
            logger2.m732v("Settings query params were: " + queryParamsFor);
            return handleResponse(applyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m735e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m732v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger2 = this.logger;
        logger2.m736e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger = this.logger;
            logger.m729w("Failed to parse settings JSON from " + this.url, e);
            Logger logger2 = this.logger;
            logger2.m730w("Settings response " + str);
            return null;
        }
    }
