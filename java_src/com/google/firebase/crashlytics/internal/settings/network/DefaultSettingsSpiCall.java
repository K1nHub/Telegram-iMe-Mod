...
public class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            logger.m724d("Requesting settings from " + this.url);
            Logger logger2 = this.logger;
            logger2.m718v("Settings query params were: " + queryParamsFor);
            return handleResponse(applyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m721e("Settings request failed.", e);
            return null;
        }
    }
    ...
    JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        Logger logger = this.logger;
        logger.m718v("Settings response code was: " + code);
        if (requestWasSuccessful(code)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        Logger logger2 = this.logger;
        logger2.m722e("Settings request failed; (status: " + code + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            Logger logger = this.logger;
            logger.m715w("Failed to parse settings JSON from " + this.url, e);
            Logger logger2 = this.logger;
            logger2.m716w("Settings response " + str);
            return null;
        }
    }
