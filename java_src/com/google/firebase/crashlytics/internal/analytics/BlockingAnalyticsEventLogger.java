...
public class BlockingAnalyticsEventLogger implements AnalyticsEventReceiver, AnalyticsEventLogger {
    ...
    public void logEvent(String str, Bundle bundle) {
        synchronized (this.latchLock) {
            ...
            logger.m1035v("Logging event " + str + " to Firebase Analytics with params " + bundle);
            this.eventLatch = new CountDownLatch(1);
            this.baseAnalyticsEventLogger.logEvent(str, bundle);
            Logger.getLogger().m1035v("Awaiting app exception callback from Analytics...");
            try {
                if (this.eventLatch.await(this.timeout, this.timeUnit)) {
                    Logger.getLogger().m1035v("App exception callback received from Analytics listener.");
                } else {
                    Logger.getLogger().m1033w("Timeout exceeded while awaiting app exception callback from Analytics listener.");
                }
            } catch (InterruptedException unused) {
                Logger.getLogger().m1039e("Interrupted while awaiting app exception callback from Analytics listener.");
            }
