...
public class CrashlyticsReportPersistence {
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().m1041d("Could not get session for report");
            return;
        }
        ...
        try {
            ...
            logger.m1040d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1032w("Could not persist event for session " + str, e);
        }
    ...
    public void finalizeReports(String str, long j) {
        for (File file : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            logger.m1035v("Finalizing report for session " + file.getName());
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : getAllFinalizedReportFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.m1032w("Could not load report file " + file + "; deleting", e);
                file.delete();
            }
        }
        return arrayList;
    ...
    private void synthesizeReport(File file, long j) {
        ...
        if (filesInDirectory.isEmpty()) {
            Logger.getLogger().m1035v("Session " + file.getName() + " has no events.");
            return;
        }
        ...
        loop0: while (true) {
            ...
            for (File file2 : filesInDirectory) {
                try {
                    ...
                    Logger.getLogger().m1032w("Could not add event to report for " + file2, e);
                }
        ...
        if (arrayList.isEmpty()) {
            Logger.getLogger().m1033w("Could not parse event files for session " + file.getName());
            return;
        }
        String str = null;
        ...
        if (file3.isFile()) {
            try {
                ...
                Logger.getLogger().m1032w("Could not read user ID file in " + file.getName(), e2);
            }
        }
    ...
    private static void synthesizeNativeReportFile(File file, File file2, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1032w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private static void synthesizeReportFile(File file, File file2, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.m1032w("Could not synthesize final report file for " + file, e);
        }
    }
