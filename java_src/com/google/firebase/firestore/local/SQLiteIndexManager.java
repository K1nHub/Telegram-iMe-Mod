...
final class SQLiteIndexManager implements IndexManager {
    ...
    public void addToCollectionParentIndex(ResourcePath resourcePath) {
        ...
        if (this.collectionParentsCache.add(resourcePath)) {
            this.f257db.execute("INSERT OR REPLACE INTO collection_parents (collection_id, parent) VALUES (?, ?)", resourcePath.getLastSegment(), EncodedPath.encode(resourcePath.popLast()));
        }
    }
    ...
    public List<ResourcePath> getCollectionParents(String str) {
        ...
        this.f257db.query("SELECT parent FROM collection_parents WHERE collection_id = ?").binding(str).forEach(new Consumer() { // from class: com.google.firebase.firestore.local.SQLiteIndexManager$$ExternalSyntheticLambda0
