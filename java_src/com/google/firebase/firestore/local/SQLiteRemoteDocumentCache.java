...
public final class SQLiteRemoteDocumentCache implements RemoteDocumentCache {
    ...
    public void add(MutableDocument mutableDocument, SnapshotVersion snapshotVersion) {
        ...
        this.f175db.execute("INSERT OR REPLACE INTO remote_documents (path, read_time_seconds, read_time_nanos, contents) VALUES (?, ?, ?, ?)", pathForKey, Long.valueOf(timestamp.getSeconds()), Integer.valueOf(timestamp.getNanoseconds()), this.serializer.encodeMaybeDocument(mutableDocument).toByteArray());
        this.f175db.getIndexManager().addToCollectionParentIndex(mutableDocument.getKey().getPath().popLast());
    }

    @Override // com.google.firebase.firestore.local.RemoteDocumentCache
    public void remove(DocumentKey documentKey) {
        this.f175db.execute("DELETE FROM remote_documents WHERE path = ?", pathForKey(documentKey));
    }

    @Override // com.google.firebase.firestore.local.RemoteDocumentCache
    public MutableDocument get(DocumentKey documentKey) {
        MutableDocument mutableDocument = (MutableDocument) this.f175db.query("SELECT contents FROM remote_documents WHERE path = ?").binding(pathForKey(documentKey)).firstValue(new Function() { // from class: com.google.firebase.firestore.local.SQLiteRemoteDocumentCache$$ExternalSyntheticLambda2
    ...
    public Map<DocumentKey, MutableDocument> getAll(Iterable<DocumentKey> iterable) {
        ...
        SQLitePersistence.LongQuery longQuery = new SQLitePersistence.LongQuery(this.f175db, "SELECT contents FROM remote_documents WHERE path IN (", arrayList, ") ORDER BY path");
    ...
    public ImmutableSortedMap<DocumentKey, MutableDocument> getAllDocumentsMatchingQuery(final Query query, SnapshotVersion snapshotVersion) {
        ...
        if (snapshotVersion.equals(SnapshotVersion.NONE)) {
            binding = this.f175db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?").binding(encode, prefixSuccessor);
        } else {
            binding = this.f175db.query("SELECT path, contents FROM remote_documents WHERE path >= ? AND path < ?AND (read_time_seconds > ? OR (read_time_seconds = ? AND read_time_nanos > ?))").binding(encode, prefixSuccessor, Long.valueOf(timestamp.getSeconds()), Long.valueOf(timestamp.getSeconds()), Integer.valueOf(timestamp.getNanoseconds()));
        }
