...
public final class Egloo {
    ...
    public static final void checkGlError(String opName) {
        int m1975constructorimpl;
        Intrinsics.checkNotNullParameter(opName, "opName");
        if (UInt.m1975constructorimpl(GLES20.glGetError()) == GlKt.getGL_NO_ERROR()) {
            return;
        }
        String str = "Error during " + opName + ": glError 0x" + MiscKt.intToHexString(m1975constructorimpl) + ": " + MiscKt.gluErrorString(m1975constructorimpl);
        Log.e("Egloo", str);
        throw new RuntimeException(str);
    }
}
