...
import com.smedialink.utils.extentions.model.wallet.StatusExtKt;
import com.smedialink.utils.extentions.model.wallet.TransactionExtKt;
...
public final class TransactionItem extends NoChildNode {
    ...
    public final String getTransactionStatusColor() {
        return StatusExtKt.color(this.transaction.getStatus());
    }
    ...
    public final String getAmount(ResourceManager resourceManager) {
        ...
        if (transaction instanceof Transaction.Unsupported) {
            return resourceManager.getString(C3158R.string.wallet_unsupported_transaction_message);
        }
        if (transaction instanceof Transaction.Crypto.Approve) {
            return resourceManager.getString(C3158R.string.wallet_transactions_type_approve_title, resourceManager.getString(TokenInfo.Companion.map(transaction.getTokenCode()).getShortName()));
        }
    ...
    public final String getAmountWithSymbol(boolean z) {
        if (z) {
            return Intrinsics.stringPlus(this.transaction.getDirection().getSymbol(), TransactionExtKt.getFormattedAmount(this.transaction));
        }
