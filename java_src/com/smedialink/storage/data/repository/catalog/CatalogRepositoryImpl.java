...
public final class CatalogRepositoryImpl implements CatalogRepository {
    ...
    public Observable<Result<List<CatalogLanguage>>> getLanguages(boolean z) {
        if (!DateExtKt.isDayAgo(this.preferenceHelper.getCatalogLanguagesLastUpdateTime())) {
            ...
            Observable<R> flatMap = observable.flatMap(new Function() { // from class: com.smedialink.storage.data.repository.catalog.CatalogRepositoryImpl$$ExternalSyntheticLambda0
    ...
    public Observable<Result<List<CategoryWithCounter>>> getCategories(ChatType chatType, boolean z) {
        ...
        Observable<ApiBaseResponse<List<CategoryWithCounterResponse>>> subscribeOn = this.catalogApi.getCategories(chatType.name(), this.preferenceHelper.getCatalogSelectedLanguageId()).subscribeOn(this.schedulersProvider.mo707io());
        ...
        if (!z) {
            ...
            flatMap = Observable.concat(onErrorResumeNext.map(new Function() { // from class: com.smedialink.storage.data.repository.catalog.CatalogRepositoryImpl$$ExternalSyntheticLambda1
                @Override // io.reactivex.functions.Function
                public final Object apply(Object obj) {
    ...
    public Observable<Result<CategoriesPreviewsCursored>> getCatalogPreview(ChatType chatType, String str) {
        Intrinsics.checkNotNullParameter(chatType, "chatType");
        Observable map = CatalogApi.DefaultImpls.getCatalogPreview$default(this.catalogApi, chatType.name(), str, null, null, this.preferenceHelper.getCatalogSelectedLanguageId(), 12, null).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new CatalogRepositoryImpl$getCatalogPreview$$inlined$mapSuccess$1(this.firebaseErrorHandler)));
    ...
    public Observable<Result<CampaignsCursored>> getAllCampaigns(ChatType chatType, String str) {
        Intrinsics.checkNotNullParameter(chatType, "chatType");
        Observable map = CatalogApi.DefaultImpls.getAllCampaigns$default(this.catalogApi, chatType.name(), str, null, this.preferenceHelper.getCatalogSelectedLanguageId(), 4, null).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new CatalogRepositoryImpl$getAllCampaigns$$inlined$mapSuccess$1(this.firebaseErrorHandler)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
    ...
    public Observable<Result<CampaignsCursored>> getCampaignsByCategoryId(long j, ChatType chatType, String str) {
        Intrinsics.checkNotNullParameter(chatType, "chatType");
        Observable map = CatalogApi.DefaultImpls.getCampaignsByCategoryId$default(this.catalogApi, j, chatType.name(), str, null, this.preferenceHelper.getCatalogSelectedLanguageId(), 8, null).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1730x5d040d4b(this.firebaseErrorHandler)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<CampaignsCursored>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1729x268554b6(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
}
