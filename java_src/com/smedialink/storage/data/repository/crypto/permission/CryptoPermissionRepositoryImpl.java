...
public final class CryptoPermissionRepositoryImpl implements CryptoPermissionRepository {
    ...
    public Observable<Result<String>> requestPermission(String userId, PermissionAction action) {
        ...
        Observable<R> map = this.permissionApi.requestPermission(new RequestPermissionRequest(userId, action.name())).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1760x7df61441(this.firebaseErrorHandler)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<String>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1759x23d42c80(this.errorHandler)));
    ...
    public Observable<Result<AccountInfo>> getAccountInfo(String userId) {
        Intrinsics.checkNotNullParameter(userId, "userId");
        Observable<R> map = this.permissionApi.getAccountInfo(new GetAccountInfoRequest(userId)).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1754x4a2c931c(this.firebaseErrorHandler)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<AccountInfo>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1753xde6d8905(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
    ...
    public Observable<Result<CryptoWalletInfo>> getPermissionSettings(boolean z) {
        ...
        ObservableSource map = this.cryptoWalletApi.getCryptoWalletInfo().map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1756x4c9e810b(this.firebaseErrorHandler, this)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable concat = Observable.concat(empty, map);
        Intrinsics.checkNotNullExpressionValue(concat, "concat(\n                …              }\n        )");
        Observable<Result<CryptoWalletInfo>> onErrorReturn = concat.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1755x2a3958f6(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
    ...
    public Observable<Result<Boolean>> managePermissionSettings(TokenCode type, boolean z, List<Long> whitelistUsers) {
        ...
        Observable<R> map = permissionApi.manageCryptoPermissions(new ManageCryptoPermissionRequest(arrayList, z, name)).map(new FirebaseExtKt$sam$i$io_reactivex_functions_Function$0(new C1758x4e08b32e(this.firebaseErrorHandler, this, z, whitelistUsers)));
        Intrinsics.checkNotNullExpressionValue(map, "errorHandler: FirebaseFu…response).toError()\n    }");
        Observable<Result<Boolean>> onErrorReturn = map.onErrorReturn(new RxExtKt$sam$i$io_reactivex_functions_Function$0(new C1757x56156b33(this.errorHandler)));
        Intrinsics.checkNotNullExpressionValue(onErrorReturn, "errorHandler: ErrorHandl…ndleError(it).toError() }");
        return onErrorReturn;
    }
}
