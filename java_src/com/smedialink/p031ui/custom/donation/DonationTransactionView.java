...
public final class DonationTransactionView extends FrameLayout implements KoinComponent {
    ...
    public final void setTransaction(TransactionItem item) {
        ...
        if (item.getTransaction() instanceof Transaction.Crypto.Donation) {
            ...
            forkRecycleItemWalletDonationTransactionBinding.imageUserAvatar.getImageReceiver().setRoundRadius(getResources().getDimensionPixelSize(C3301R.dimen.telegram_avatar_size_medium) / 2);
            if (user != null) {
                ...
                forkRecycleItemWalletDonationTransactionBinding.textUserFullName.setText(getResourceManager().getString(C3301R.string.common_id, Long.valueOf(Long.parseLong(((Transaction.Crypto.Donation) item.getTransaction()).getSenderAccountId()))));
            }
