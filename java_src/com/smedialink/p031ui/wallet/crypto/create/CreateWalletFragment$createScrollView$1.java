...
public final class CreateWalletFragment$createScrollView$1 extends ScrollView {
    ...
    public void scrollToDescendant(View child) {
        ...
        rect.bottom = i + AndroidUtilities.m50dp(((CreateWalletFragment.NumericEditText) list.get(lastIndex)).getEditText() == child ? 90 : 10);
        int computeScrollDeltaToGetChildRectOnScreen = computeScrollDeltaToGetChildRectOnScreen(this.tempRect);
        if (computeScrollDeltaToGetChildRectOnScreen < 0) {
            this.scrollingUp = 2;
            Unit unit = Unit.INSTANCE;
            computeScrollDeltaToGetChildRectOnScreen -= (getMeasuredHeight() - child.getMeasuredHeight()) / 2;
        } else {
            this.scrollingUp = 0;
        }
        if (computeScrollDeltaToGetChildRectOnScreen != 0) {
    ...
    public boolean requestChildRectangleOnScreen(View child, Rect rectangle, boolean z) {
        ...
        if (Build.VERSION.SDK_INT < 23) {
            int i = rectangle.bottom;
            List list = this.this$0.editTexts;
            lastIndex = CollectionsKt__CollectionsKt.getLastIndex(this.this$0.editTexts);
            int m50dp = i + AndroidUtilities.m50dp(((CreateWalletFragment.NumericEditText) list.get(lastIndex)).getEditText() == this.scrollingToChild ? 90 : 16);
            rectangle.bottom = m50dp;
            int i2 = this.scrollingUp;
            if (i2 != 0) {
                rectangle.top -= i2;
                rectangle.bottom = m50dp - i2;
                this.scrollingUp = 0;
            }
        }
