package p033j$.util;

import java.util.NoSuchElementException;
import java.util.Objects;
import p033j$.util.Iterator;
import p033j$.util.Spliterator;
import p033j$.util.function.C2825o;
import p033j$.util.function.Consumer;
import p033j$.util.function.InterfaceC2826p;
/* renamed from: j$.util.t */
/* loaded from: classes2.dex */
class C3152t implements InterfaceC2845p, InterfaceC2826p, Iterator {

    /* renamed from: a */
    boolean f1240a = false;

    /* renamed from: b */
    long f1241b;

    /* renamed from: c */
    final /* synthetic */ Spliterator.InterfaceC2774c f1242c;

    /* JADX INFO: Access modifiers changed from: package-private */
    public C3152t(Spliterator.InterfaceC2774c interfaceC2774c) {
        this.f1242c = interfaceC2774c;
    }

    @Override // p033j$.util.function.InterfaceC2826p
    public void accept(long j) {
        this.f1240a = true;
        this.f1241b = j;
    }

    @Override // p033j$.util.InterfaceC2842n
    /* renamed from: d */
    public void forEachRemaining(InterfaceC2826p interfaceC2826p) {
        Objects.requireNonNull(interfaceC2826p);
        while (hasNext()) {
            interfaceC2826p.accept(nextLong());
        }
    }

    @Override // p033j$.util.function.InterfaceC2826p
    /* renamed from: f */
    public InterfaceC2826p mo212f(InterfaceC2826p interfaceC2826p) {
        Objects.requireNonNull(interfaceC2826p);
        return new C2825o(this, interfaceC2826p);
    }

    @Override // p033j$.util.InterfaceC2845p, p033j$.util.Iterator
    public void forEachRemaining(Consumer consumer) {
        if (consumer instanceof InterfaceC2826p) {
            forEachRemaining((InterfaceC2826p) consumer);
            return;
        }
        Objects.requireNonNull(consumer);
        if (!AbstractC2771H.f696a) {
            while (hasNext()) {
                consumer.accept(Long.valueOf(nextLong()));
            }
            return;
        }
        AbstractC2771H.m671a(C3152t.class, "{0} calling PrimitiveIterator.OfLong.forEachRemainingLong(action::accept)");
        throw null;
    }

    @Override // java.util.Iterator, p033j$.util.Iterator
    public boolean hasNext() {
        if (!this.f1240a) {
            this.f1242c.mo185i(this);
        }
        return this.f1240a;
    }

    @Override // java.util.Iterator, p033j$.util.Iterator
    public Long next() {
        if (AbstractC2771H.f696a) {
            AbstractC2771H.m671a(C3152t.class, "{0} calling PrimitiveIterator.OfLong.nextLong()");
            throw null;
        }
        return Long.valueOf(nextLong());
    }

    @Override // p033j$.util.InterfaceC2845p
    public long nextLong() {
        if (this.f1240a || hasNext()) {
            this.f1240a = false;
            return this.f1241b;
        }
        throw new NoSuchElementException();
    }

    @Override // java.util.Iterator, p033j$.util.Iterator
    public /* synthetic */ void remove() {
        Iterator.CC.m670a(this);
        throw null;
    }
}
