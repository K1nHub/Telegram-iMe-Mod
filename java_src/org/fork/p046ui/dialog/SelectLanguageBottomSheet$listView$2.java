package org.fork.p046ui.dialog;

import kotlin.jvm.functions.Function0;
import kotlin.jvm.internal.Lambda;
/* JADX INFO: Access modifiers changed from: package-private */
/* compiled from: SelectLanguageBottomSheet.kt */
/* renamed from: org.fork.ui.dialog.SelectLanguageBottomSheet$listView$2 */
/* loaded from: classes4.dex */
public final class SelectLanguageBottomSheet$listView$2 extends Lambda implements Function0<SelectLanguageBottomSheet$initListView$1> {
    final /* synthetic */ SelectLanguageBottomSheet this$0;

    /* JADX INFO: Access modifiers changed from: package-private */
    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public SelectLanguageBottomSheet$listView$2(SelectLanguageBottomSheet selectLanguageBottomSheet) {
        super(0);
        this.this$0 = selectLanguageBottomSheet;
    }

    @Override // kotlin.jvm.functions.Function0
    public final SelectLanguageBottomSheet$initListView$1 invoke() {
        SelectLanguageBottomSheet$initListView$1 initListView;
        initListView = this.this$0.initListView();
        return initListView;
    }
}
