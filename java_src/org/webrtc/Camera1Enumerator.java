...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m18d(TAG, "Index: " + i + ". " + deviceName);
            } else {
                Logging.m17e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m16e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m18d(TAG, "Get supported formats for camera index " + i + ".");
        long elapsedRealtime = SystemClock.elapsedRealtime();
        Camera camera = null;
        try {
            try {
                Logging.m18d(TAG, "Opening camera with index " + i);
                ...
                try {
                    ...
                    Logging.m16e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                }
                long elapsedRealtime2 = SystemClock.elapsedRealtime();
                Logging.m18d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (elapsedRealtime2 - elapsedRealtime) + " ms.");
                return arrayList;
            } catch (RuntimeException e2) {
                Logging.m16e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    public static int getCameraIndex(String str) {
        Logging.m18d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
