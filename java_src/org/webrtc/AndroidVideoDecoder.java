...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
    ...
    public AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecMimeType videoCodecMimeType, int i, EglBase.Context context) {
        ...
        Logging.m20d(TAG, "ctor name: " + str + " type: " + videoCodecMimeType + " color format: " + i + " context: " + context);
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m20d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2);
        if (this.outputThread != null) {
            Logging.m19e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        try {
            ...
            try {
                ...
                Logging.m20d(TAG, "initDecodeInternal done");
                return VideoCodecStatus.OK;
            } catch (IllegalArgumentException | IllegalStateException e) {
                Logging.m18e(TAG, "initDecode failed", e);
                release();
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            }
        } catch (IOException | IllegalArgumentException | IllegalStateException unused) {
            Logging.m19e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (byteBuffer == null) {
            Logging.m19e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        int remaining = byteBuffer.remaining();
        if (remaining == 0) {
            Logging.m19e(TAG, "decode() - input buffer empty");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        ...
        if (i3 * i4 <= 0 || ((i3 == i && i4 == i2) || (reinitDecode = reinitDecode(i3, i4)) == VideoCodecStatus.OK)) {
            if (this.keyFrameRequired && encodedImage.frameType != EncodedImage.FrameType.VideoFrameKey) {
                Logging.m19e(TAG, "decode() - key frame required first");
                return VideoCodecStatus.NO_OUTPUT;
            }
            try {
                int dequeueInputBuffer = this.codec.dequeueInputBuffer(500000L);
                if (dequeueInputBuffer < 0) {
                    Logging.m19e(TAG, "decode() - no HW buffers available; decoder falling behind");
                    return VideoCodecStatus.ERROR;
                }
                try {
                    ByteBuffer byteBuffer2 = this.codec.getInputBuffers()[dequeueInputBuffer];
                    if (byteBuffer2.capacity() < remaining) {
                        Logging.m19e(TAG, "decode() - HW buffer too small");
                        return VideoCodecStatus.ERROR;
                    }
    ...
    private VideoCodecStatus releaseInternal() {
        if (!this.running) {
            Logging.m20d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m18e(TAG, "Media decoder release timeout", new RuntimeException());
                return VideoCodecStatus.TIMEOUT;
            } else if (this.shutdownException != null) {
                Logging.m18e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    protected void deliverDecodedFrame() {
        ...
        try {
            ...
            if (dequeueOutputBuffer == -2) {
                ...
                Logging.m17v(TAG, "dequeueOutputBuffer returned " + dequeueOutputBuffer);
            } else {
            ...
            Logging.m18e(TAG, "deliverDecodedFrame failed", e);
        }
    }
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) {
        ...
        if (i7 < ((i3 * i4) * 3) / 2) {
            Logging.m19e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        int integer2;
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m20d(TAG, "Decoder format changed: " + mediaFormat.toString());
        ...
        synchronized (this.dimensionLock) {
            if (integer != this.width || integer2 != this.height) {
                ...
                Logging.m16w(TAG, "Unexpected format dimensions. Configured " + this.width + "*" + this.height + ". New " + integer + "*" + integer2 + ". Skip it");
                return;
            }
            if (this.surfaceTextureHelper == null && mediaFormat.containsKey("color-format")) {
                this.colorFormat = mediaFormat.getInteger("color-format");
                Logging.m20d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
            ...
            synchronized (this.dimensionLock) {
                ...
                Logging.m20d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
    ...
    public void releaseCodecOnOutputThread() {
        ...
        try {
            ...
            Logging.m18e(TAG, "Media decoder stop failed", e);
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m18e(TAG, "Media decoder release failed", e2);
