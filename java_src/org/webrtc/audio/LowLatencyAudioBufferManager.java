        ...
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            if (underrunCount > this.prevUnderrunCount) {
                if (this.bufferIncreaseCounter < 5) {
                    ...
                    Logging.m18d(TAG, "Underrun detected! Increasing AudioTrack buffer size from " + bufferSizeInFrames + " to " + playbackRate);
                ...
                if (i <= 0) {
                    ...
                    if (max != bufferSizeInFrames2) {
                        Logging.m18d(TAG, "Lowering AudioTrack buffer size from " + bufferSizeInFrames2 + " to " + max);
