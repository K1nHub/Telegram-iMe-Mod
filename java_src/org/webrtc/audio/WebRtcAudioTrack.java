...
public class WebRtcAudioTrack {
    ...
    private boolean stopPlayout() {
        ...
        if (this.audioTrack != null) {
            Logging.m20d(TAG, "Calling AudioTrack.stop...");
            try {
                this.audioTrack.stop();
                Logging.m20d(TAG, "AudioTrack.stop is done.");
                doAudioTrackStateCallback(1);
            } catch (IllegalStateException e) {
                Logging.m19e(TAG, "AudioTrack.stop failed: " + e.getMessage());
            }
        }
        releaseAudioResources();
        return true;
    ...
    private static AudioTrack createAudioTrackOnOreoOrHigher(int i, int i2, int i3, AudioAttributes audioAttributes) {
        Logging.m20d(TAG, "createAudioTrackOnOreoOrHigher");
        logNativeOutputSampleRate(i);
    ...
    public void doAudioTrackStateCallback(int i) {
        Logging.m20d(TAG, "doAudioTrackStateCallback: " + i);
        ...
        if (audioTrackStateCallback != null) {
            if (i == 0) {
                ...
                Logging.m19e(TAG, "Invalid audio state");
            }
        }
    }
}
