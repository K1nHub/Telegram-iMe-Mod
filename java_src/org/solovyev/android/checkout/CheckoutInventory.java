...
public final class CheckoutInventory extends BaseInventory {
    ...
    public class Worker implements Checkout.Listener, Runnable {
        ...
        private void loadPurchases(BillingRequests billingRequests, final Inventory.Product product) {
            billingRequests.getAllPurchases(product.f1416id, CheckoutInventory.this.synchronizedListener(new RequestListener<Purchases>() { // from class: org.solovyev.android.checkout.CheckoutInventory.Worker.1
        ...
        private void loadSkus(BillingRequests billingRequests, final Inventory.Product product) {
            List<String> skus = this.mTask.getRequest().getSkus(product.f1416id);
            if (skus.isEmpty()) {
                Billing.warning("There are no SKUs for \"" + product.f1416id + "\" product. No SKU information will be loaded");
                synchronized (CheckoutInventory.this.mLock) {
                    countDown();
                }
                return;
            }
            billingRequests.getSkus(product.f1416id, skus, CheckoutInventory.this.synchronizedListener(new RequestListener<Skus>() { // from class: org.solovyev.android.checkout.CheckoutInventory.Worker.2
                @Override // org.solovyev.android.checkout.RequestListener
