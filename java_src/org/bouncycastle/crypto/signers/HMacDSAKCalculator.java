...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    /* renamed from: K */
    private final byte[] f1304K;

    /* renamed from: V */
    private final byte[] f1305V;
    private final HMac hMac;

    /* renamed from: n */
    private BigInteger f1306n;

    public HMacDSAKCalculator(Digest digest) {
        HMac hMac = new HMac(digest);
        this.hMac = hMac;
        this.f1305V = new byte[hMac.getMacSize()];
        this.f1304K = new byte[hMac.getMacSize()];
    }

    private BigInteger bitsToInt(byte[] bArr) {
        BigInteger bigInteger = new BigInteger(1, bArr);
        return bArr.length * 8 > this.f1306n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f1306n.bitLength()) : bigInteger;
    }

    @Override // org.bouncycastle.crypto.signers.DSAKCalculator
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f1306n = bigInteger;
        Arrays.fill(this.f1305V, (byte) 1);
        Arrays.fill(this.f1304K, (byte) 0);
        ...
        this.hMac.init(new KeyParameter(this.f1304K));
        HMac hMac = this.hMac;
        byte[] bArr4 = this.f1305V;
        ...
        this.hMac.doFinal(this.f1304K, 0);
        this.hMac.init(new KeyParameter(this.f1304K));
        HMac hMac2 = this.hMac;
        byte[] bArr5 = this.f1305V;
        hMac2.update(bArr5, 0, bArr5.length);
        this.hMac.doFinal(this.f1305V, 0);
        HMac hMac3 = this.hMac;
        byte[] bArr6 = this.f1305V;
    ...
    public BigInteger nextK() {
        int bitLength = (this.f1306n.bitLength() + 7) / 8;
        byte[] bArr = new byte[bitLength];
        while (true) {
            int i = 0;
            while (i < bitLength) {
                HMac hMac = this.hMac;
                byte[] bArr2 = this.f1305V;
                hMac.update(bArr2, 0, bArr2.length);
                this.hMac.doFinal(this.f1305V, 0);
                int min = Math.min(bitLength - i, this.f1305V.length);
                System.arraycopy(this.f1305V, 0, bArr, i, min);
                i += min;
            }
            BigInteger bitsToInt = bitsToInt(bArr);
            if (bitsToInt.compareTo(ZERO) > 0 && bitsToInt.compareTo(this.f1306n) < 0) {
                return bitsToInt;
            }
            HMac hMac2 = this.hMac;
            byte[] bArr3 = this.f1305V;
