...
public final class CheckProofUtils {
    ...
    public final BlockHeaderResult checkBlockHeaderProof(Cell root, TonNodeBlockIdExt blockId, boolean z) {
        ...
        return new BlockHeaderResult(loadTlb.getInfo().getValue().m5256getGenUtimepVg5ArA(), loadTlb.getInfo().getValue().m5254getEndLtsVKNKU(), z ? CellRef.DefaultImpls.toCell$default(loadTlb.getStateUpdate(), null, 1, null).getRefs().get(1).hash(0) : null);
    }
    ...
    public final FullAccountState checkAccountProof(byte[] proof, TonNodeBlockIdExt shardBlock, AddrStd address, Cell root) {
        ...
        HashmapAugNode.AhmnLeaf ahmnLeaf = ((ShardStateUnsplit) loadTlb).getAccounts().getValue().m5322unboximpl().get(address.address());
        ...
        if (shardAccount == null) {
            ...
            return new FullAccountState(shardBlock, address, new TransactionId(shardAccount.getLastTransHash(), shardAccount.m5315getLastTransLtsVKNKU()), valueOf);
        }
    }
}
