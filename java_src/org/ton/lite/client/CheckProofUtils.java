...
public final class CheckProofUtils {
    ...
    public final BlockHeaderResult checkBlockHeaderProof(Cell root, TonNodeBlockIdExt blockId, boolean z) {
        ...
        return new BlockHeaderResult(loadTlb.getInfo().getValue().m4935getGenUtimepVg5ArA(), loadTlb.getInfo().getValue().m4933getEndLtsVKNKU(), z ? CellRef.DefaultImpls.toCell$default(loadTlb.getStateUpdate(), null, 1, null).getRefs().get(1).hash(0) : null);
    }
    ...
    public final FullAccountState checkAccountProof(byte[] proof, TonNodeBlockIdExt shardBlock, AddrStd address, Cell root) {
        ...
        HashmapAugNode.AhmnLeaf ahmnLeaf = ((ShardStateUnsplit) loadTlb).getAccounts().getValue().m5001unboximpl().get(address.address());
        ...
        if (shardAccount == null) {
            ...
            return new FullAccountState(shardBlock, address, new TransactionId(shardAccount.getLastTransHash(), shardAccount.m4994getLastTransLtsVKNKU()), valueOf);
        }
    }
}
