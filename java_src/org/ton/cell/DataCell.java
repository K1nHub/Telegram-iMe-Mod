...
public final class DataCell implements Cell {
    ...
    public BitString hash(int i) {
        byte[] first = this.hashes.get(LevelMask.m5399getHashIndeximpl(LevelMask.m5393applyspGXSBY(mo5373getLevelMaskKat384U(), i))).getFirst();
        return BitString.Companion.m43of(first, first.length * 8);
    }

    @Override // org.ton.cell.Cell
    public int depth(int i) {
        return this.hashes.get(LevelMask.m5399getHashIndeximpl(LevelMask.m5393applyspGXSBY(mo5373getLevelMaskKat384U(), i))).getSecond().intValue();
    }

    @Override // org.ton.cell.Cell
    public Cell virtualize(int i) {
        return LevelMask.m5402isEmptyimpl(mo5373getLevelMaskKat384U()) ? this : new VirtualCell(this, i);
    }
