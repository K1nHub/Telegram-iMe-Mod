...
public abstract class TlConstructor<T> implements TlCodec<T> {
    ...
    public T decodeBoxed(TlReader reader) {
        ...
        if (!(readInt == getId())) {
            padStart = StringsKt__StringsKt.padStart(UStringsKt.m2053toStringV7xB4Y4(UInt.m1967constructorimpl(Integer.reverseBytes(getId())), 16), 8, '0');
            padStart2 = StringsKt__StringsKt.padStart(UStringsKt.m2053toStringV7xB4Y4(UInt.m1967constructorimpl(Integer.reverseBytes(readInt)), 16), 8, '0');
