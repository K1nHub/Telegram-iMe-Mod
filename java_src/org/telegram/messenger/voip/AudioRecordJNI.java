...
public class AudioRecordJNI {
    ...
    /* renamed from: ns */
    private NoiseSuppressor f1485ns;
            ...
            if (Build.VERSION.SDK_INT >= 16) {
                try {
                    if (AutomaticGainControl.isAvailable()) {
                        ...
                        VLog.m35w("AutomaticGainControl is not available on this device :(");
                    }
                } catch (Throwable th) {
                    VLog.m39e("error creating AutomaticGainControl", th);
                }
                try {
                    if (NoiseSuppressor.isAvailable()) {
                        NoiseSuppressor create2 = NoiseSuppressor.create(this.audioRecord.getAudioSessionId());
                        this.f1485ns = create2;
                        if (create2 != null) {
                            create2.setEnabled(Instance.getGlobalServerConfig().useSystemNs && isGoodAudioEffect(this.f1485ns));
                        }
                    } else {
                        VLog.m35w("NoiseSuppressor is not available on this device :(");
                    }
                } catch (Throwable th2) {
                    VLog.m39e("error creating NoiseSuppressor", th2);
                }
                try {
                    if (AcousticEchoCanceler.isAvailable()) {
                        ...
                        VLog.m35w("AcousticEchoCanceler is not available on this device");
                    }
                } catch (Throwable th3) {
                    VLog.m39e("error creating AcousticEchoCanceler", th3);
                }
            }
    ...
    private boolean tryInit(int i, int i2) {
        ...
        VLog.m37i("Trying to initialize AudioRecord with source=" + i + " and sample rate=" + i2);
        try {
            this.audioRecord = new AudioRecord(i, i2, 16, 2, getBufferSize(this.bufferSize, OpusUtil.SAMPLE_RATE));
        } catch (Exception e) {
            VLog.m39e("AudioRecord init failed!", e);
        }
    ...
    public void release() {
        ...
        NoiseSuppressor noiseSuppressor = this.f1485ns;
    ...
    public boolean start() {
        ...
        if (audioRecord != null && audioRecord.getState() == 1) {
            try {
                ...
                VLog.m39e("Error initializing AudioRecord", e);
            }
        }
        return false;
    }
    ...
    public /* synthetic */ void lambda$startThread$0(ByteBuffer byteBuffer) {
        ...
        VLog.m37i("audiorecord thread exits");
    }
    ...
    private static boolean isGoodAudioEffect(AudioEffect audioEffect) {
        ...
        VLog.m41d(audioEffect.getClass().getSimpleName() + ": implementor=" + descriptor.implementor + ", name=" + descriptor.name);
