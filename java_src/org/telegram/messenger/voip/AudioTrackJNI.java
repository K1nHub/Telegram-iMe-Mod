...
public class AudioTrackJNI {
    ...
    public void init(int i, int i2, int i3, int i4) {
        ...
        if (audioTrack.getState() != 1) {
            VLog.m74w("Error initializing AudioTrack with 48k, trying 44.1k with resampling");
            try {
                this.audioTrack.release();
            } catch (Throwable unused) {
            }
            int bufferSize = getBufferSize(i4 * 6, 44100);
            VLog.m80d("buffer size: " + bufferSize);
    ...
    public /* synthetic */ void lambda$startThread$0() {
        try {
            ...
            VLog.m76i("audiotrack thread exits");
        } catch (Exception e2) {
            VLog.m78e("error starting AudioTrack", e2);
        }
    }
}
