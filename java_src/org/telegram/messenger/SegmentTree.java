...
public class SegmentTree {
    ...
    private int rMaxQ(int i, int i2, int i3) {
        ...
        if (node.pendingVal != null && contains(node.from, node.f1464to, i2, i3)) {
            return node.pendingVal.intValue();
        }
        if (contains(i2, i3, node.from, node.f1464to)) {
            return this.heap[i].max;
        }
        if (intersects(i2, i3, node.from, node.f1464to)) {
            propagate(i);
            int i4 = i * 2;
    ...
    public static class Node {
        ...
        int size() {
            return (this.f1464to - this.from) + 1;
        }
    }
}
