    ...
    public static /* synthetic */ void lambda$startJob$0() {
        ...
        try {
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m105d("starting keep-alive job");
            }
            synchronized (sync) {
                startingJob = true;
            }
    ...
    public static void finishJobInternal() {
        synchronized (sync) {
            if (countDownLatch != null) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("finish keep-alive job");
                }
                countDownLatch.countDown();
            }
            if (startingJob) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("finish queued keep-alive job");
                }
                startingJob = false;
            }
        }
    ...
    protected void onHandleWork(Intent intent) {
        synchronized (sync) {
            if (startingJob) {
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("started keep-alive job");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("ended keep-alive job");
                }
            }
        }
    }
