...
public class ChatsWidgetProvider extends AppWidgetProvider {
    ...
    public void onDeleted(Context context, int[] iArr) {
        ...
        for (int i = 0; i < iArr.length; i++) {
            ...
            edit.remove(SessionDescription.ATTR_TYPE + iArr[i]);
    ...
    public static void updateWidget(Context context, AppWidgetManager appWidgetManager, int i) {
        ...
        if (!sharedPreferences.getBoolean("deleted" + i, false)) {
            ...
            if (i3 == -1) {
                ...
                edit.putInt(SessionDescription.ATTR_TYPE + i, 0).commit();
            }
            ...
            if (cellsForSize == 1 || arrayList.size() <= 1) {
                i2 = C3634R.layout.shortcut_widget_layout_1;
            } else if (cellsForSize == 2 || arrayList.size() <= 2) {
                i2 = C3634R.layout.shortcut_widget_layout_2;
            } else if (cellsForSize == 3 || arrayList.size() <= 3) {
                i2 = C3634R.layout.shortcut_widget_layout_3;
            } else {
                i2 = C3634R.layout.shortcut_widget_layout_4;
            }
        } else {
            i2 = C3634R.layout.shortcut_widget_layout_1;
        }
        RemoteViews remoteViews = new RemoteViews(context.getPackageName(), i2);
        int i4 = C3634R.C3637id.list_view;
        remoteViews.setRemoteAdapter(i, i4, intent);
        remoteViews.setEmptyView(i4, C3634R.C3637id.empty_view);
