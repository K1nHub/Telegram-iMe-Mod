...
public class SRPHelper {
    ...
    public static byte[] getX(byte[] bArr, C3380x72c667f c3380x72c667f) {
        byte[] bArr2 = c3380x72c667f.salt1;
        byte[] computeSHA256 = Utilities.computeSHA256(bArr2, bArr, bArr2);
        byte[] bArr3 = c3380x72c667f.salt2;
        byte[] computePBKDF2 = Utilities.computePBKDF2(Utilities.computeSHA256(bArr3, computeSHA256, bArr3), c3380x72c667f.salt1);
        byte[] bArr4 = c3380x72c667f.salt2;
        return Utilities.computeSHA256(bArr4, computePBKDF2, bArr4);
    }

    public static BigInteger getV(byte[] bArr, C3380x72c667f c3380x72c667f) {
        BigInteger valueOf = BigInteger.valueOf(c3380x72c667f.f1622g);
        getBigIntegerBytes(valueOf);
        return valueOf.modPow(new BigInteger(1, getX(bArr, c3380x72c667f)), new BigInteger(1, c3380x72c667f.f1623p));
    }

    public static byte[] getVBytes(byte[] bArr, C3380x72c667f c3380x72c667f) {
        if (Utilities.isGoodPrime(c3380x72c667f.f1623p, c3380x72c667f.f1622g)) {
            return getBigIntegerBytes(getV(bArr, c3380x72c667f));
        }
        return null;
    }

    public static TLRPC$TL_inputCheckPasswordSRP startCheck(byte[] bArr, long j, byte[] bArr2, C3380x72c667f c3380x72c667f) {
        if (bArr != null && bArr2 != null && bArr2.length != 0 && Utilities.isGoodPrime(c3380x72c667f.f1623p, c3380x72c667f.f1622g)) {
            BigInteger valueOf = BigInteger.valueOf(c3380x72c667f.f1622g);
            byte[] bigIntegerBytes = getBigIntegerBytes(valueOf);
            BigInteger bigInteger = new BigInteger(1, c3380x72c667f.f1623p);
            BigInteger bigInteger2 = new BigInteger(1, Utilities.computeSHA256(c3380x72c667f.f1623p, bigIntegerBytes));
            ...
            if (bigInteger5.compareTo(bigInteger6) > 0 && bigInteger5.compareTo(bigInteger) < 0) {
                ...
                if (Utilities.isGoodGaAndGb(subtract, bigInteger)) {
                    ...
                    byte[] computeSHA2562 = Utilities.computeSHA256(c3380x72c667f.f1623p);
                    ...
                    tLRPC$TL_inputCheckPasswordSRP.f1564M1 = Utilities.computeSHA256(computeSHA2562, Utilities.computeSHA256(c3380x72c667f.salt1), Utilities.computeSHA256(c3380x72c667f.salt2), bigIntegerBytes2, bigIntegerBytes3, computeSHA256);
                    tLRPC$TL_inputCheckPasswordSRP.f1563A = bigIntegerBytes2;
