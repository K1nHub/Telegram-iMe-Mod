...
public class DialogObject {
    ...
    public static void initDialog(TLRPC$Dialog tLRPC$Dialog) {
        if (tLRPC$Dialog == null || tLRPC$Dialog.f1506id != 0) {
            return;
        }
        if (tLRPC$Dialog instanceof TLRPC$TL_dialog) {
            ...
            if (j != 0) {
                tLRPC$Dialog.f1506id = j;
                return;
            }
            long j2 = tLRPC$Peer.chat_id;
            if (j2 != 0) {
                tLRPC$Dialog.f1506id = -j2;
            } else {
                tLRPC$Dialog.f1506id = -tLRPC$Peer.channel_id;
            }
        } else if (tLRPC$Dialog instanceof TLRPC$TL_dialogFolder) {
            tLRPC$Dialog.f1506id = makeFolderDialogId(((TLRPC$TL_dialogFolder) tLRPC$Dialog).folder.f1552id);
        }
    }
    ...
    public static String setDialogPhotoTitle(ImageReceiver imageReceiver, AvatarDrawable avatarDrawable, TLObject tLObject) {
        ...
        if (tLObject instanceof TLRPC$User) {
            ...
            if (UserObject.isReplyUser(tLRPC$User)) {
                String string = LocaleController.getString("RepliesTitle", C3301R.string.RepliesTitle);
                ...
                String string2 = LocaleController.getString("SavedMessages", C3301R.string.SavedMessages);
                if (avatarDrawable != null) {
