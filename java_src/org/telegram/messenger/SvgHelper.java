    ...
    public static class SvgDrawable extends Drawable {
            ...
            if (this.placeholderGradient[i] != null) {
                ...
                if (f5 > BitmapDescriptorFactory.HUE_RED && lite) {
                    ...
                    if (matrixArr[i4] != null) {
                        matrixArr[i4].reset();
                        if (z) {
                            this.placeholderMatrix[i4].postTranslate(((-i2) + totalTranslation) - f, BitmapDescriptorFactory.HUE_RED);
                        } else {
                            this.placeholderMatrix[i4].postTranslate(((-i2) + totalTranslation) - f, BitmapDescriptorFactory.HUE_RED);
                        }
            ...
            for (int i5 = 0; i5 < size; i5++) {
                ...
                if (obj instanceof Matrix) {
                    ...
                    if (obj instanceof Path) {
                        ...
                        Line line = (Line) obj;
                        canvas.drawLine(line.f1565x1, line.f1567y1, line.f1566x2, line.f1568y2, paint);
                    } else if (obj instanceof Circle) {
                        Circle circle = (Circle) obj;
                        canvas.drawCircle(circle.f1563x1, circle.f1564y1, circle.rad, paint);
        ...
        public void setupGradient(int i, Theme.ResourcesProvider resourcesProvider, float f, boolean z) {
            ...
            if (iArr[z ? 1 : 0] != color) {
                ...
                float m102dp = AndroidUtilities.m102dp(180) / gradientWidth;
                int argb = Color.argb((int) ((Color.alpha(color) / 2) * this.colorAlpha), Color.red(color), Color.green(color), Color.blue(color));
                float f2 = (1.0f - m102dp) / 2.0f;
                float f3 = m102dp / 2.0f;
                this.placeholderGradient[z ? 1 : 0] = new LinearGradient((float) BitmapDescriptorFactory.HUE_RED, (float) BitmapDescriptorFactory.HUE_RED, gradientWidth, (float) BitmapDescriptorFactory.HUE_RED, new int[]{0, 0, argb, 0, 0}, new float[]{BitmapDescriptorFactory.HUE_RED, f2 - f3, f2, f2 + f3, 1.0f}, Shader.TileMode.REPEAT);
                int i2 = Build.VERSION.SDK_INT;
                if (i2 >= 28) {
                    bitmapShader = new LinearGradient((float) BitmapDescriptorFactory.HUE_RED, (float) BitmapDescriptorFactory.HUE_RED, gradientWidth, (float) BitmapDescriptorFactory.HUE_RED, new int[]{argb, argb}, (float[]) null, Shader.TileMode.REPEAT);
                } else {
    ...
    public static Matrix parseTransform(String str) {
        ...
        if (startsWith) {
            ...
            if (parseNumbers.numbers.size() == 6) {
                ...
                matrix.setValues(new float[]{((Float) parseNumbers.numbers.get(0)).floatValue(), ((Float) parseNumbers.numbers.get(2)).floatValue(), ((Float) parseNumbers.numbers.get(4)).floatValue(), ((Float) parseNumbers.numbers.get(1)).floatValue(), ((Float) parseNumbers.numbers.get(3)).floatValue(), ((Float) parseNumbers.numbers.get(5)).floatValue(), BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, 1.0f});
                return matrix;
            }
            return null;
    ...
    */
    ...
    public static class Properties {
        ...
        private Properties(Attributes attributes, HashMap<String, StyleSet> hashMap) {
            ...
            String stringAttr = SvgHelper.getStringAttr(TtmlNode.TAG_STYLE, attributes);
            if (stringAttr == null) {
            ...
            if (!this.boundsMode || str2.equals(TtmlNode.TAG_STYLE)) {
                ...
                switch (c) {
                    ...
                    case '\n':
                        ...
                        return;
                    case 3:
                        if ("bounds".equalsIgnoreCase(SvgHelper.getStringAttr(TtmlNode.ATTR_ID, attributes))) {
                    ...
                    case '\b':
                        ...
                        if (floatAttr14 == null) {
                            floatAttr14 = Float.valueOf((float) BitmapDescriptorFactory.HUE_RED);
                        }
                        Float floatAttr15 = SvgHelper.getFloatAttr("y", attributes);
                        if (floatAttr15 == null) {
                            floatAttr15 = Float.valueOf((float) BitmapDescriptorFactory.HUE_RED);
                        }
    ...
    public static class ParserHelper {
        ...
        /* renamed from: s */
        private CharSequence f1570s;

        public ParserHelper(CharSequence charSequence, int i) {
            this.f1570s = charSequence;
            this.pos = i;
            this.f1569n = charSequence.length();
        ...
        private char read() {
            ...
            return this.f1570s.charAt(i3);
        }

        public void skipWhitespace() {
            while (true) {
                int i = this.pos;
                if (i >= this.f1569n || !Character.isWhitespace(this.f1570s.charAt(i))) {
                    return;
                }
                advance();
            }
        ...
        public void skipNumberSeparator() {
            while (true) {
                ...
                char charAt = this.f1570s.charAt(i);
                if (charAt != '\t' && charAt != '\n' && charAt != ' ' && charAt != ',') {
                    return;
                }
                advance();
