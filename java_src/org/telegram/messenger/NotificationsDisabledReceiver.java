    ...
    public void onReceive(Context context, Intent intent) {
        if ("android.app.action.NOTIFICATION_CHANNEL_BLOCK_STATE_CHANGED".equals(intent.getAction())) {
            ...
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m105d("received disabled notification channel event for " + stringExtra + " state = " + booleanExtra);
            }
            if (SystemClock.elapsedRealtime() - AccountInstance.getInstance(intValue).getNotificationsController().lastNotificationChannelCreateTime <= 1000) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("received disable notification event right after creating notification channel, ignoring");
                    return;
                }
                return;
            }
            ...
            if (split[1].startsWith("channel")) {
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("apply channel{channel} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("apply channel{groups} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("apply channel{private} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("apply channel{stories} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m105d("apply channel{else} " + stringExtra + " state");
                }
