...
public class ImageLocation {
    ...
    public static ImageLocation getForUser(TLRPC$User tLRPC$User, int i) {
        ...
        if (tLRPC$User != null && tLRPC$User.access_hash != 0 && (tLRPC$UserProfilePhoto = tLRPC$User.photo) != null) {
            ...
            if (MessagesController.getInstance(i3).isPremiumUser(tLRPC$User) && tLRPC$User.photo.has_video && ForkCommonController.getInstance(i3).isPremiumAnimateAvatars() && (userFull = MessagesController.getInstance(i3).getUserFull(tLRPC$User.f1751id)) != null && (tLRPC$Photo = userFull.profile_photo) != null && (arrayList = tLRPC$Photo.video_sizes) != null && !arrayList.isEmpty()) {
                if (i == 4) {
