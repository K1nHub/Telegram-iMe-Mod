...
public class NotificationImageProvider extends ContentProvider implements NotificationCenter.NotificationCenterDelegate {
    ...
    public String[] getStreamTypes(Uri uri, String str) {
        if (str.startsWith("*/") || str.startsWith("image/")) {
            return new String[]{MimeTypes.IMAGE_JPEG, "image/png", "image/webp"};
        }
        return null;
    }
    ...
    public ParcelFileDescriptor openFile(Uri uri, String str) throws FileNotFoundException {
        ...
        if (getUriMatcher().match(uri) == 1) {
            ...
            if (!file.exists()) {
                ...
                while (!file.exists()) {
                    if (System.currentTimeMillis() - longValue >= C0483C.DEFAULT_MAX_SEEK_TO_PREVIOUS_POSITION_MS) {
                        if (BuildVars.LOGS_ENABLED) {
                            FileLog.m100w("Waiting for " + str2 + " to download timed out");
                        }
