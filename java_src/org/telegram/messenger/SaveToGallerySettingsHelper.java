...
public class SaveToGallerySettingsHelper {
    ...
    public static class SharedSettings extends Settings {
        ...
        public CharSequence createDescription(int i) {
            ...
            if (enabled()) {
                if (this.savePhoto) {
                    sb.append(LocaleController.getString("SaveToGalleryPhotos", C3290R.string.SaveToGalleryPhotos));
                }
                if (this.saveVideo) {
                    if (sb.length() != 0) {
                        sb.append(", ");
                    }
                    sb.append(LocaleController.getString("SaveToGalleryVideos", C3290R.string.SaveToGalleryVideos));
                    long j = this.limitVideo;
                    if (j > 0 && j < 4194304000L) {
                        sb.append(" (");
                        sb.append(AndroidUtilities.formatFileSize(this.limitVideo, true));
                        sb.append(")");
                    }
                }
            } else {
                sb.append(LocaleController.getString("SaveToGalleryOff", C3290R.string.SaveToGalleryOff));
            }
        ...
        public CharSequence createDescription(int i) {
            ...
            if (enabled()) {
                ...
                if (this.saveVideo) {
                    ...
                    long j = this.limitVideo;
                    if (j <= 0 || j >= 4194304000L) {
                        sb.append(LocaleController.formatString("SaveToGalleryVideos", C3290R.string.SaveToGalleryVideos, new Object[0]));
                    } else {
                        sb.append(LocaleController.formatString("SaveToGalleryVideosUpTo", C3290R.string.SaveToGalleryVideosUpTo, AndroidUtilities.formatFileSize(j, true)));
                    }
                }
            } else {
                sb.append(LocaleController.getString("SaveToGalleryOff", C3290R.string.SaveToGalleryOff));
