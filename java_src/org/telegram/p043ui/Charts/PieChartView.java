...
public class PieChartView extends StackLinearChartView<PieChartViewData> {
            ...
            while (i4 < length) {
                ...
                while (i6 < size) {
                    ...
                    if (z || lineViewData.alpha != BitmapDescriptorFactory.HUE_RED) {
                        float f6 = lineViewData.line.f1721y[i4] * lineViewData.alpha;
                        f5 += f6;
                ...
                while (i9 < size) {
                    ...
                    if (lineViewData2.enabled || lineViewData2.alpha != f4) {
                        int[] iArr = lineViewData2.line.f1721y;
                        if (i7 == i5) {
                            if (iArr[i4] != 0) {
    ...
    public void updatePicker(ChartData chartData, long j) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            if (j2 >= chartData.f1719x[i2]) {
                i = i2;
            }
        }
        float length2 = chartData.xPercentage.length < 2 ? 0.5f : 1.0f / chartData.f1719x.length;
        if (i == 0) {
            ChartPickerDelegate chartPickerDelegate = this.pickerDelegate;
            chartPickerDelegate.pickerStart = BitmapDescriptorFactory.HUE_RED;
            chartPickerDelegate.pickerEnd = length2;
        } else if (i >= chartData.f1719x.length - 1) {
    ...
    private void updateCharValues(float f, float f2, boolean z) {
        ...
        while (i3 <= i2) {
            for (int i6 = 0; i6 < size; i6++) {
                ...
                fArr[i6] = fArr[i6] + ((StackLinearChartData) this.chartData).lines.get(i6).f1721y[i3];
                this.sum += ((StackLinearChartData) this.chartData).lines.get(i6).f1721y[i3];
                if (this.isEmpty && ((PieChartViewData) this.lines.get(i6)).enabled && ((StackLinearChartData) this.chartData).lines.get(i6).f1721y[i3] > 0) {
                    this.isEmpty = false;
                }
            }
            i3++;
