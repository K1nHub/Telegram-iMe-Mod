...
public class StackLinearChartData extends ChartData {
    ...
    public StackLinearChartData(JSONObject jSONObject, boolean z) throws JSONException {
        ...
        if (z) {
            ...
            for (int i = 0; i < this.lines.size(); i++) {
                int length = this.f1800x.length;
                for (int i2 = 0; i2 < length; i2++) {
                    int i3 = this.lines.get(i).f1802y[i2];
                    jArr[i] = jArr[i] + i3;
                    if (i3 == 0) {
                        iArr[i] = iArr[i] + 1;
                    }
            ...
            for (int i4 = 0; i4 < this.lines.size(); i4++) {
                if (jArr[i4] / j < 0.01d && iArr[i4] > this.f1800x.length / 2.0f) {
        ...
        int length2 = this.lines.get(0).f1802y.length;
        ...
        for (int i5 = 0; i5 < length2; i5++) {
            ...
            for (int i6 = 0; i6 < size; i6++) {
                int[] iArr2 = this.ySum;
                iArr2[i5] = iArr2[i5] + this.lines.get(i6).f1802y[i5];
            }
        }
    ...
    public StackLinearChartData(ChartData chartData, long j) {
        int binarySearch = Arrays.binarySearch(chartData.f1800x, j);
        ...
        int i5 = 0;
        while (i <= i2) {
            this.f1800x[i5] = chartData.f1800x[i];
            for (int i6 = 0; i6 < this.lines.size(); i6++) {
                this.lines.get(i6).f1802y[i5] = chartData.lines.get(i6).f1802y[i];
            }
            i5++;
            i++;
        }
    ...
    public void measure() {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            for (int i3 = 0; i3 < size; i3++) {
                int[] iArr2 = this.lines.get(i3).f1802y;
                if (iArr2[i2] > iArr[i3]) {
                    iArr[i3] = iArr2[i2];
                }
            }
