            ...
            if (tLRPC$TL_error == null) {
                ...
                while (true) {
                    ...
                    while (i6 < arrayList.size()) {
                        ...
                        if (this.topicMode) {
                            if (tLRPC$Chat2 != null) {
                                tLRPC$Dialog2 = MessagesController.getInstance(this.currentAccount).dialogs_dict.get(-tLRPC$Chat2.f1602id);
                                i2 = i6;
                                ...
                                i2 = i6;
                                tLRPC$Dialog2 = MessagesController.getInstance(this.currentAccount).dialogs_dict.get(tLRPC$User2.f1749id);
                            }
                        ...
                        if (tLRPC$Chat2 != null) {
                            if (!hiddenChatsController.isChatHidden(-tLRPC$Chat2.f1602id) && z && ((!z2 || ChatObject.canAddBotsToChat(tLRPC$Chat2)) && ((this.allowGlobalResults || !ChatObject.isNotInChat(tLRPC$Chat2)) && filter(tLRPC$Chat2)))) {
                                MessagesController.DialogFilter dialogFilter = this.filter;
                                if (dialogFilter == null || dialogFilter.includesDialog(this.accountInstance, -tLRPC$Chat2.f1602id)) {
                                    this.globalSearch.add(tLRPC$Chat2);
                                }
                                this.globalSearchMap.put(-tLRPC$Chat2.f1602id, tLRPC$Chat2);
                            }
                        } else if (tLRPC$User2 != null && !hiddenChatsController.isChatHidden(tLRPC$User2.f1749id) && !z2 && ((z3 || !tLRPC$User2.bot) && ((z4 || !tLRPC$User2.self) && ((this.allowGlobalResults || i5 != 1 || tLRPC$User2.contact) && filter(tLRPC$User2))))) {
                            if (this.filter == null) {
                                this.globalSearch.add(tLRPC$User2);
                            }
                            this.globalSearchMap.put(tLRPC$User2.f1749id, tLRPC$User2);
                        }
                        i6 = i2 + 1;
                        j2 = 0;
                    }
                ...
                if (!this.allResultsAreGlobal) {
                    for (int i7 = 0; i7 < tLRPC$TL_contacts_found.my_results.size(); i7++) {
                        ...
                        if (this.topicMode) {
                            if (tLRPC$Chat != null) {
                                tLRPC$Dialog = MessagesController.getInstance(this.currentAccount).dialogs_dict.get(-tLRPC$Chat.f1602id);
                            } else if (tLRPC$User != null) {
                                if (!UserObject.isUserSelf(tLRPC$User) && !UserObject.isReplyUser(tLRPC$User) && !MessagesController.isSupportUser(tLRPC$User)) {
                                    tLRPC$Dialog = MessagesController.getInstance(this.currentAccount).dialogs_dict.get(tLRPC$User.f1749id);
                                }
                            } else {
                                tLRPC$Dialog = null;
                            }
                        ...
                        if (tLRPC$Chat != null) {
                            if (!hiddenChatsController.isChatHidden(-tLRPC$Chat.f1602id) && z && ((!z2 || ChatObject.canAddBotsToChat(tLRPC$Chat)) && (-tLRPC$Chat.f1602id) != j && filter(tLRPC$Chat))) {
                                MessagesController.DialogFilter dialogFilter2 = this.filter;
                                if (dialogFilter2 == null || dialogFilter2.includesDialog(this.accountInstance, -tLRPC$Chat.f1602id)) {
                                    this.localServerSearch.add(tLRPC$Chat);
                                }
                                this.globalSearchMap.put(-tLRPC$Chat.f1602id, tLRPC$Chat);
                            }
                        } else if (tLRPC$User != null && !hiddenChatsController.isChatHidden(tLRPC$User.f1749id) && !z2 && ((z3 || !tLRPC$User.bot) && ((z4 || !tLRPC$User.self) && tLRPC$User.f1749id != j && filter(tLRPC$User)))) {
                            if (this.filter == null) {
                                this.localServerSearch.add(tLRPC$User);
                            }
                            this.globalSearchMap.put(tLRPC$User.f1749id, tLRPC$User);
                        }
                    }
                }
    ...
    private void removeGroupSearchFromGlobal() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (tLRPC$User != null) {
                ...
                this.globalSearchMap.remove(tLRPC$User.f1749id);
            }
        }
    }
        ...
        if (this.globalSearchMap.size() != 0) {
            ...
            while (i < size2) {
                ...
                if (obj instanceof TLRPC$User) {
                    ...
                    TLRPC$User tLRPC$User2 = (TLRPC$User) this.globalSearchMap.get(tLRPC$User.f1749id);
                    if (tLRPC$User2 != null) {
                        this.globalSearch.remove(tLRPC$User2);
                        this.localServerSearch.remove(tLRPC$User2);
                        this.globalSearchMap.remove(tLRPC$User2.f1749id);
                    }
                    TLObject tLObject = this.groupSearchMap.get(tLRPC$User.f1749id);
                    if (tLObject != null) {
                        this.groupSearch.remove(tLObject);
                        this.groupSearchMap.remove(tLRPC$User.f1749id);
                    }
                    TLObject tLObject2 = this.phoneSearchMap.get(tLRPC$User.f1749id);
                    if (tLObject2 != null) {
                        this.phonesSearch.remove(tLObject2);
                        this.phoneSearchMap.remove(tLRPC$User.f1749id);
                    }
                } else if ((obj instanceof TLRPC$Chat) && (tLRPC$Chat = (TLRPC$Chat) this.globalSearchMap.get(-((TLRPC$Chat) obj).f1602id)) != null) {
                    this.globalSearch.remove(tLRPC$Chat);
                    this.localServerSearch.remove(tLRPC$Chat);
                    this.globalSearchMap.remove(-tLRPC$Chat.f1602id);
                }
                i++;
            }
        }
