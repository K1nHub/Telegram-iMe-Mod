...
public class ChatPullingDownDrawable implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ChatPullingDownDrawable(int i, View view, long j, int i2, int i3, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.textPaint2.setTextSize(AndroidUtilities.m72dp(14));
    ...
    public void updateDialog() {
        ...
        if (nextUnreadDialog != null) {
            long j = nextUnreadDialog.f1528id;
            this.nextDialogId = j;
            ...
            if (DialogObject.isUserDialog(j)) {
                this.nextUser = MessagesController.getInstance(this.currentAccount).getUser(Long.valueOf(nextUnreadDialog.f1528id));
            } else {
                this.nextChat = MessagesController.getInstance(this.currentAccount).getChat(Long.valueOf(-nextUnreadDialog.f1528id));
            }
            ...
            MessagesController.getInstance(this.currentAccount).ensureMessagesLoaded(nextUnreadDialog.f1528id, 0, null);
    ...
    public void setWidth(int i) {
        ...
        int i2;
        if (i != this.lastWidth) {
            this.circleRadius = AndroidUtilities.m72dp(56) / 2.0f;
            this.lastWidth = i;
            TLRPC$Chat tLRPC$Chat = this.nextChat;
            String string3 = tLRPC$Chat != null ? tLRPC$Chat.title : LocaleController.getString("SwipeToGoNextChannelEnd", C3473R.string.SwipeToGoNextChannelEnd);
            if (this.nextUser != null) {
                ContactsController contactsController = ContactsController.getInstance(this.currentAccount);
                if (contactsController.contactsDict.get(Long.valueOf(this.nextUser.f1685id)) == null && ((contactsController.contactsDict.size() != 0 || !contactsController.isLoadingContacts()) && !TextUtils.isEmpty(this.nextUser.phone))) {
                    C3389PhoneFormat c3389PhoneFormat = C3389PhoneFormat.getInstance();
                    string3 = c3389PhoneFormat.format("+" + this.nextUser.phone);
                } else {
            ...
            this.chatNameWidth = Math.min(measureText, this.lastWidth - AndroidUtilities.m72dp(60));
            ...
            if (z && (i2 = this.dialogFolderId) != this.folderId && i2 != 0) {
                string = LocaleController.getString("SwipeToGoNextArchive", C3473R.string.SwipeToGoNextArchive);
                string2 = LocaleController.getString("ReleaseToGoNextArchive", C3473R.string.ReleaseToGoNextArchive);
                ...
                string = LocaleController.getString("SwipeToGoNextFolder", C3473R.string.SwipeToGoNextFolder);
                string2 = LocaleController.getString("ReleaseToGoNextFolder", C3473R.string.ReleaseToGoNextFolder);
            } else {
                string = LocaleController.getString("SwipeToGoNextChannel", C3473R.string.SwipeToGoNextChannel);
                string2 = LocaleController.getString("ReleaseToGoNextChannel", C3473R.string.ReleaseToGoNextChannel);
            ...
            this.layout1Width = Math.min(measureText2, this.lastWidth - AndroidUtilities.m72dp(60));
            ...
            this.layout2Width = Math.min(measureText3, this.lastWidth - AndroidUtilities.m72dp(60));
            ...
            this.imageReceiver.setImageCoords((this.lastWidth / 2.0f) - (AndroidUtilities.m72dp(40) / 2.0f), (AndroidUtilities.m72dp(12) + this.circleRadius) - (AndroidUtilities.m72dp(40) / 2.0f), AndroidUtilities.m72dp(40), AndroidUtilities.m72dp(40));
            this.imageReceiver.setRoundRadius((int) (AndroidUtilities.m72dp(40) / 2.0f));
            this.counterDrawable.setSize(AndroidUtilities.m72dp(28), AndroidUtilities.m72dp(100));
        }
    }
    ...
    public void draw(Canvas canvas, View view, float f, float f2) {
        ...
        float m72dp = AndroidUtilities.m72dp(110) * f;
        if (m72dp < AndroidUtilities.m72dp(8)) {
            return;
        }
        float f4 = f < 0.2f ? 5.0f * f * f2 : f2;
        Theme.applyServiceShaderMatrix(this.lastWidth, view.getMeasuredHeight(), BitmapDescriptorFactory.HUE_RED, view.getMeasuredHeight() - m72dp);
        TextPaint textPaint = this.textPaint;
        ...
        float f6 = this.bounceProgress * (-AndroidUtilities.m72dp(4));
        if (this.emptyStub) {
            m72dp -= f6;
        }
        float f7 = m72dp / 2.0f;
        float max = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(this.circleRadius, (f7 - (AndroidUtilities.m72dp(16) * f)) - AndroidUtilities.m72dp(4)));
        float max2 = ((Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(this.circleRadius * f, f7 - (AndroidUtilities.m72dp(8) * f))) * 2.0f) - AndroidUtilities.dp2(16.0f)) * (1.0f - this.swipeToReleaseProgress);
        float f8 = this.swipeToReleaseProgress;
        float m72dp2 = max2 + (AndroidUtilities.m72dp(56) * f8);
        if (f8 < 1.0f || this.emptyStub) {
            float f9 = -m72dp;
            i3 = alpha2;
            i4 = alpha;
            float m72dp3 = ((-AndroidUtilities.m72dp(8)) * (1.0f - this.swipeToReleaseProgress)) + ((AndroidUtilities.m72dp(56) + f9) * this.swipeToReleaseProgress);
            RectF rectF = AndroidUtilities.rectTmp;
            rectF.set(f5 - max, f9, max + f5, m72dp3);
            if (this.swipeToReleaseProgress > BitmapDescriptorFactory.HUE_RED && !this.emptyStub) {
                float m72dp4 = AndroidUtilities.m72dp(16) * this.swipeToReleaseProgress;
                rectF.inset(m72dp4, m72dp4);
            }
            drawBackground(canvas, rectF);
            float m72dp5 = ((AndroidUtilities.m72dp(24) + f9) + (AndroidUtilities.m72dp(8) * (1.0f - f))) - (AndroidUtilities.m72dp(36) * this.swipeToReleaseProgress);
            canvas.save();
            ...
            drawArrow(canvas, f5, m72dp5, AndroidUtilities.m72dp(24) * f);
            if (this.emptyStub) {
                i5 = 2;
                float dp2 = ((((-AndroidUtilities.m72dp(8)) - (AndroidUtilities.dp2(8.0f) * f)) - m72dp2) * (1.0f - this.swipeToReleaseProgress)) + ((f9 - AndroidUtilities.m72dp(2)) * this.swipeToReleaseProgress) + f6;
                this.arrowPaint.setAlpha(alpha4);
                canvas.save();
                canvas.scale(f, f, f5, AndroidUtilities.m72dp(28) + dp2);
                drawCheck(canvas, f5, dp2 + AndroidUtilities.m72dp(28));
                canvas.restore();
            } else {
                i5 = 2;
            }
        ...
        if (this.chatNameLayout != null && this.swipeToReleaseProgress > BitmapDescriptorFactory.HUE_RED) {
            ...
            this.textPaint.setAlpha(i8);
            float m72dp6 = ((AndroidUtilities.m72dp(20) * (1.0f - this.swipeToReleaseProgress)) - (AndroidUtilities.m72dp(36) * this.swipeToReleaseProgress)) + f6;
            RectF rectF2 = AndroidUtilities.rectTmp;
            int i10 = this.lastWidth;
            int i11 = this.chatNameWidth;
            rectF2.set((i10 - i11) / 2.0f, m72dp6, i10 - ((i10 - i11) / 2.0f), this.chatNameLayout.getHeight() + m72dp6);
            rectF2.inset(-AndroidUtilities.m72dp(8), -AndroidUtilities.m72dp(4));
            canvas.drawRoundRect(rectF2, AndroidUtilities.m72dp(15), AndroidUtilities.m72dp(15), getThemedPaint("paintChatActionBackground"));
            if (hasGradientService()) {
                canvas.drawRoundRect(rectF2, AndroidUtilities.m72dp(15), AndroidUtilities.m72dp(15), Theme.chat_actionBackgroundGradientDarkenPaint);
            }
            canvas.save();
            canvas.translate((this.lastWidth - this.chatNameWidth) / 2.0f, m72dp6);
            this.chatNameLayout.draw(canvas);
            canvas.restore();
        }
        if (this.emptyStub || m72dp2 <= BitmapDescriptorFactory.HUE_RED) {
            f3 = 1.0f;
        } else {
            f3 = 1.0f;
            float dp22 = ((((-AndroidUtilities.m72dp(8)) - (AndroidUtilities.dp2(8.0f) * f)) - m72dp2) * (1.0f - this.swipeToReleaseProgress)) + (((-m72dp) + AndroidUtilities.m72dp(4)) * this.swipeToReleaseProgress) + f6;
            float f11 = m72dp2 / 2.0f;
            this.imageReceiver.setRoundRadius((int) f11);
            this.imageReceiver.setImageCoords(f5 - f11, dp22, m72dp2, m72dp2);
            if (this.swipeToReleaseProgress > BitmapDescriptorFactory.HUE_RED) {
                ...
                canvas.scale(f12, f12, f5 + AndroidUtilities.m72dp(12) + this.counterDrawable.getCenterX(), (dp22 - AndroidUtilities.m72dp(6)) + AndroidUtilities.m72dp(14));
                canvas.translate(f5 + AndroidUtilities.m72dp(12), dp22 - AndroidUtilities.m72dp(6));
                this.counterDrawable.updateBackgroundRect();
                this.counterDrawable.rectF.inset(-AndroidUtilities.m72dp(i12), -AndroidUtilities.m72dp(i12));
                ...
                canvas.scale(f13, f13, f5 + AndroidUtilities.m72dp(12) + this.counterDrawable.getCenterX(), (dp22 - AndroidUtilities.m72dp(6)) + AndroidUtilities.m72dp(14));
                canvas.translate(f5 + AndroidUtilities.m72dp(12), dp22 - AndroidUtilities.m72dp(6));
    ...
    private void drawCheck(Canvas canvas, float f, float f2) {
        if (this.animateCheck) {
            ...
            canvas.translate(f - AndroidUtilities.m72dp(24), f2 - AndroidUtilities.m72dp(24));
            float m72dp = AndroidUtilities.m72dp(16);
            float m72dp2 = AndroidUtilities.m72dp(26);
            float m72dp3 = AndroidUtilities.m72dp(22);
            float m72dp4 = AndroidUtilities.m72dp(32);
            float m72dp5 = AndroidUtilities.m72dp(32);
            float m72dp6 = AndroidUtilities.m72dp(20);
            float f8 = 1.0f - f6;
            canvas.drawLine(m72dp, m72dp2, (m72dp * f8) + (m72dp3 * f6), (f8 * m72dp2) + (f6 * m72dp4), this.arrowPaint);
            if (f7 > BitmapDescriptorFactory.HUE_RED) {
                float f9 = 1.0f - f7;
                canvas.drawLine(m72dp3, m72dp4, (m72dp3 * f9) + (m72dp5 * f7), (f9 * m72dp4) + (m72dp6 * f7), this.arrowPaint);
            }
            canvas.restore();
        }
    }
    ...
    private void drawArrow(Canvas canvas, float f, float f2, float f3) {
        ...
        canvas.scale(dpf2, dpf2, f, f2 - AndroidUtilities.m72dp(20));
        canvas.translate(f - AndroidUtilities.dp2(12.0f), f2 - AndroidUtilities.m72dp(12));
    ...
    public static TLRPC$Dialog getNextUnreadDialog(long j, int i, int i2, boolean z, int[] iArr) {
        ...
        while (i3 < dialogs.size()) {
            ...
            TLRPC$Chat chat = DialogObject.isChatDialog(tLRPC$Dialog.f1528id) ? messagesController.getChat(Long.valueOf(-tLRPC$Dialog.f1528id)) : tLRPC$Chat;
            ToolsController toolsController2 = toolsController;
            TLRPC$User user = DialogObject.isUserDialog(tLRPC$Dialog.f1528id) ? messagesController.getUser(Long.valueOf(tLRPC$Dialog.f1528id)) : null;
            if ((chat != null || (user != null && !user.self && !UserObject.isReplyUser(user) && UserObject.isLive(user))) && tLRPC$Dialog.f1528id != j && tLRPC$Dialog.unread_count > 0) {
                if (dialogType == DialogType.getDialogType(user != null ? user : chat) && !messagesController.isPromoDialog(tLRPC$Dialog.f1528id, false) && (user != null || MessagesController.getRestrictionReason(chat.restriction_reason) == null)) {
                    return tLRPC$Dialog;
                }
            }
            i3++;
        ...
        if (z) {
            if (i2 != 0) {
                for (int i4 = 0; i4 < messagesController.dialogFilters.size(); i4++) {
                    int i5 = messagesController.dialogFilters.get(i4).f1459id;
