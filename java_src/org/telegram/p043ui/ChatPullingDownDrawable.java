...
public class ChatPullingDownDrawable implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void updateDialog() {
        ...
        if (nextUnreadDialog != null) {
            long j = nextUnreadDialog.f1524id;
            this.nextDialogId = j;
            ...
            if (DialogObject.isUserDialog(j)) {
                this.nextUser = MessagesController.getInstance(this.currentAccount).getUser(Long.valueOf(nextUnreadDialog.f1524id));
            } else {
                this.nextChat = MessagesController.getInstance(this.currentAccount).getChat(Long.valueOf(-nextUnreadDialog.f1524id));
            }
            ...
            MessagesController.getInstance(this.currentAccount).ensureMessagesLoaded(nextUnreadDialog.f1524id, 0, null);
    ...
    public void setWidth(int i) {
        ...
        if (i != this.lastWidth) {
            ...
            String string3 = tLRPC$Chat != null ? tLRPC$Chat.title : LocaleController.getString("SwipeToGoNextChannelEnd", C3419R.string.SwipeToGoNextChannelEnd);
            if (this.nextUser != null) {
                ContactsController contactsController = ContactsController.getInstance(this.currentAccount);
                if (contactsController.contactsDict.get(Long.valueOf(this.nextUser.f1675id)) == null && ((contactsController.contactsDict.size() != 0 || !contactsController.isLoadingContacts()) && !TextUtils.isEmpty(this.nextUser.phone))) {
                    C3334PhoneFormat c3334PhoneFormat = C3334PhoneFormat.getInstance();
                    string3 = c3334PhoneFormat.format("+" + this.nextUser.phone);
                } else {
            ...
            if (z && (i2 = this.dialogFolderId) != this.folderId && i2 != 0) {
                string = LocaleController.getString("SwipeToGoNextArchive", C3419R.string.SwipeToGoNextArchive);
                string2 = LocaleController.getString("ReleaseToGoNextArchive", C3419R.string.ReleaseToGoNextArchive);
                ...
                string = LocaleController.getString("SwipeToGoNextFolder", C3419R.string.SwipeToGoNextFolder);
                string2 = LocaleController.getString("ReleaseToGoNextFolder", C3419R.string.ReleaseToGoNextFolder);
            } else {
                string = LocaleController.getString("SwipeToGoNextChannel", C3419R.string.SwipeToGoNextChannel);
                string2 = LocaleController.getString("ReleaseToGoNextChannel", C3419R.string.ReleaseToGoNextChannel);
    ...
    public static TLRPC$Dialog getNextUnreadDialog(long j, int i, int i2, boolean z, int[] iArr) {
        ...
        while (i3 < dialogs.size()) {
            ...
            TLRPC$Chat chat = DialogObject.isChatDialog(tLRPC$Dialog.f1524id) ? messagesController.getChat(Long.valueOf(-tLRPC$Dialog.f1524id)) : tLRPC$Chat;
            ToolsController toolsController2 = toolsController;
            TLRPC$User user = DialogObject.isUserDialog(tLRPC$Dialog.f1524id) ? messagesController.getUser(Long.valueOf(tLRPC$Dialog.f1524id)) : null;
            if ((chat != null || (user != null && !user.self && !UserObject.isReplyUser(user) && UserObject.isLive(user))) && tLRPC$Dialog.f1524id != j && tLRPC$Dialog.unread_count > 0) {
                if (dialogType == DialogType.getDialogType(user != null ? user : chat) && !messagesController.isPromoDialog(tLRPC$Dialog.f1524id, false) && (user != null || MessagesController.getRestrictionReason(chat.restriction_reason) == null)) {
                    return tLRPC$Dialog;
                }
            }
            i3++;
        ...
        if (z) {
            if (i2 != 0) {
                for (int i4 = 0; i4 < messagesController.dialogFilters.size(); i4++) {
                    int i5 = messagesController.dialogFilters.get(i4).f1458id;
