...
public class DraftSavedHint extends View {
    ...
    public DraftSavedHint(Context context) {
        ...
        paint.setPathEffect(new CornerPathEffect(AndroidUtilities.m72dp(6)));
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        canvas.save();
        canvas.translate(BitmapDescriptorFactory.HUE_RED, (this.shown ? CubicBezierInterpolator.EASE_OUT_BACK.getInterpolation(f) : 1.0f) * AndroidUtilities.m72dp(12));
        float interpolation = CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(f);
        float measuredWidth = getMeasuredWidth();
        float measuredHeight = getMeasuredHeight();
        float m72dp = AndroidUtilities.m72dp(22) + this.layoutWidth;
        float min = (measuredWidth / 2.0f) - Math.min(AndroidUtilities.m72dp((int) TsExtractor.TS_STREAM_TYPE_E_AC3), 0.35f * measuredWidth);
        float max = Math.max(AndroidUtilities.m72dp(8), min - (m72dp / 2.0f));
        this.path.rewind();
        this.path.moveTo(max, BitmapDescriptorFactory.HUE_RED);
        float f2 = m72dp + max;
        this.path.lineTo(f2, BitmapDescriptorFactory.HUE_RED);
        this.path.lineTo(f2, measuredHeight - AndroidUtilities.m72dp(18));
        this.path.lineTo(AndroidUtilities.m72dp(7) + min, measuredHeight - AndroidUtilities.m72dp(18));
        this.path.lineTo(AndroidUtilities.m72dp(1) + min, measuredHeight - AndroidUtilities.m72dp(12));
        this.path.lineTo(min - AndroidUtilities.m72dp(1), measuredHeight - AndroidUtilities.m72dp(12));
        this.path.lineTo(min - AndroidUtilities.m72dp(7), measuredHeight - AndroidUtilities.m72dp(18));
        this.path.lineTo(max, measuredHeight - AndroidUtilities.m72dp(18));
        this.path.close();
        this.backgroundPaint.setAlpha((int) (204.0f * interpolation));
        canvas.drawPath(this.path, this.backgroundPaint);
        canvas.save();
        canvas.translate((max + AndroidUtilities.m72dp(11)) - this.layoutLeft, ((measuredHeight - AndroidUtilities.m72dp(18)) - this.layout.getHeight()) / 2.0f);
    ...
    protected void onMeasure(int i, int i2) {
        setMeasuredDimension(View.MeasureSpec.getSize(i), AndroidUtilities.m72dp(50));
    }
}
