...
public class RecordControl extends View implements FlashViews.Invertable {
    ...
    public void setInvert(float f) {
        ...
        this.hintLinePaintBlack.setColor(ColorUtils.blendARGB(402653184, C0485C.ENCODING_PCM_32BIT, f));
    ...
    */
    ...
    private void getVector(float f, float f2, double d, float f3, Point point) {
        double d2 = f3;
        point.f1892x = (float) (f + (Math.cos(d) * d2));
        point.f1893y = (float) (f2 + (Math.sin(d) * d2));
    }

    private float dist(Point point, Point point2) {
        return MathUtils.distance(point.f1892x, point.f1893y, point2.f1892x, point2.f1893y);
    }
    ...
    private boolean isPressed(float f, float f2, float f3, float f4, float f5, boolean z) {
        return this.recording ? (!z || f4 - f2 <= ((float) AndroidUtilities.m104dp(100))) && Math.abs(f3 - f) <= f5 : MathUtils.distance(f, f2, f3, f4) <= f5;
    }
