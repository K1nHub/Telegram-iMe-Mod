...
public class DualCameraView extends CameraView implements CameraController.ErrorCallback {
    ...
    /* renamed from: cx */
    private float f1884cx;

    /* renamed from: cy */
    private float f1885cy;
            ...
            if (motionEvent.getAction() == 2 && this.down) {
                ...
                if (motionEvent.getPointerCount() > 1) {
                    if (this.lastTouchDistance != BitmapDescriptorFactory.HUE_RED) {
                        ...
                        if (this.f1887w * f7 > getWidth() * 0.7f) {
                            width = getWidth() * 0.7f;
                            f2 = this.f1887w;
                        } else {
                            if (this.f1887w * f7 < getWidth() * 0.2f) {
                                width = getWidth() * 0.2f;
                                f2 = this.f1887w;
                            }
                ...
                if (this.allowRotation && !this.doNotSpanRotation) {
                    if (Math.abs(round) < 5.0f) {
                        this.finalMatrix.postRotate(round, this.f1884cx, this.f1885cy);
                        if (!this.snappedRotation) {
                            try {
                                performHapticFeedback(9, 1);
                ...
                float f9 = this.f1884cx;
                if (f9 < BitmapDescriptorFactory.HUE_RED) {
                    this.finalMatrix.postTranslate(-f9, BitmapDescriptorFactory.HUE_RED);
                } else if (f9 > getWidth()) {
                    this.finalMatrix.postTranslate(getWidth() - this.f1884cx, BitmapDescriptorFactory.HUE_RED);
                }
                float f10 = this.f1885cy;
                if (f10 < BitmapDescriptorFactory.HUE_RED) {
                    ...
                    this.finalMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, (getHeight() - AndroidUtilities.m72dp((int) ImageReceiver.DEFAULT_CROSSFADE_DURATION)) - this.f1885cy);
                }
                this.finalMatrix.postConcat(this.toGL);
                dualPosition.set(this.finalMatrix);
                updateDualPosition();
                float f11 = this.f1885cy;
                boolean z5 = Math.min(f11, f11 - (this.f1886h / 2.0f)) < ((float) AndroidUtilities.m72dp(66));
                float f12 = this.f1885cy;
                boolean z6 = Math.max(f12, (this.f1886h / 2.0f) + f12) > ((float) (getHeight() - AndroidUtilities.m72dp(66)));
                if (this.atTop != z5) {
                    this.atTop = z5;
    ...
    private void extractPointsData(Matrix matrix) {
        ...
        this.f1884cx = fArr2[0];
        this.f1885cy = fArr2[1];
        fArr2[0] = 1.0f;
        fArr2[1] = 0.0f;
        matrix.mapPoints(fArr2);
        float[] fArr3 = this.vertices;
        this.angle = (float) Math.toDegrees(Math.atan2(fArr3[1] - this.f1885cy, fArr3[0] - this.f1884cx));
        float f = this.f1884cx;
        float f2 = this.f1885cy;
        float[] fArr4 = this.vertices;
        this.f1887w = MathUtils.distance(f, f2, fArr4[0], fArr4[1]) * 2.0f;
        float[] fArr5 = this.vertices;
        fArr5[0] = 0.0f;
        fArr5[1] = 1.0f;
        matrix.mapPoints(fArr5);
        float f3 = this.f1884cx;
        float f4 = this.f1885cy;
        float[] fArr6 = this.vertices;
        this.f1886h = MathUtils.distance(f3, f4, fArr6[0], fArr6[1]) * 2.0f;
    }
    ...
    public void onError(int i, Camera camera, CameraSession cameraSession) {
        if (isDual()) {
            if (!dualAvailableDefault(getContext(), false)) {
                ...
                new AlertDialog.Builder(getContext()).setTitle(LocaleController.getString(C3419R.string.DualErrorTitle)).setMessage(LocaleController.getString(C3419R.string.DualErrorMessage)).setPositiveButton(LocaleController.getString(C3419R.string.OK), null).show();
            }
            log(false);
            toggleDual();
        }
