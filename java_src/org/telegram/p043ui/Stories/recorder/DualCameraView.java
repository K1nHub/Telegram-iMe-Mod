...
public class DualCameraView extends CameraView implements CameraController.ErrorCallback {
    ...
    /* renamed from: cx */
    private float f2007cx;

    /* renamed from: cy */
    private float f2008cy;
    ...
    private boolean checkTap(MotionEvent motionEvent) {
        if (motionEvent.getAction() == 0) {
            ...
            if (System.currentTimeMillis() - this.tapTime <= ViewConfiguration.getTapTimeout() && MathUtils.distance(this.tapX, this.tapY, motionEvent.getX(), motionEvent.getY()) < AndroidUtilities.m107dp(10)) {
                if (isAtDual(this.tapX, this.tapY)) {
                    switchCamera();
                    this.lastFocusToPoint = null;
                } else {
    ...
    private boolean touchEvent(MotionEvent motionEvent) {
        ...
        if (isDual()) {
            ...
            if (motionEvent.getAction() == 2 && this.down) {
                if (MathUtils.distance(f3, f4, f5, f6) > AndroidUtilities.m107dp(2) && (runnable = this.longpressRunnable) != null) {
                    AndroidUtilities.cancelRunOnUIThread(runnable);
                    this.longpressRunnable = null;
                }
                if (motionEvent.getPointerCount() > 1) {
                    if (this.lastTouchDistance != BitmapDescriptorFactory.HUE_RED) {
                        ...
                        if (this.f2010w * f7 > getWidth() * 0.7f) {
                            width = getWidth() * 0.7f;
                            f2 = this.f2010w;
                        } else {
                            if (this.f2010w * f7 < getWidth() * 0.2f) {
                                width = getWidth() * 0.2f;
                                f2 = this.f2010w;
                            }
                ...
                if (this.allowRotation && !this.doNotSpanRotation) {
                    if (Math.abs(round) < 5.0f) {
                        this.finalMatrix.postRotate(round, this.f2007cx, this.f2008cy);
                        if (!this.snappedRotation) {
                            try {
                                performHapticFeedback(9, 1);
                ...
                float f9 = this.f2007cx;
                if (f9 < BitmapDescriptorFactory.HUE_RED) {
                    this.finalMatrix.postTranslate(-f9, BitmapDescriptorFactory.HUE_RED);
                } else if (f9 > getWidth()) {
                    this.finalMatrix.postTranslate(getWidth() - this.f2007cx, BitmapDescriptorFactory.HUE_RED);
                }
                float f10 = this.f2008cy;
                if (f10 < BitmapDescriptorFactory.HUE_RED) {
                    this.finalMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, -f10);
                } else if (f10 > getHeight() - AndroidUtilities.m107dp((int) ImageReceiver.DEFAULT_CROSSFADE_DURATION)) {
                    this.finalMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, (getHeight() - AndroidUtilities.m107dp((int) ImageReceiver.DEFAULT_CROSSFADE_DURATION)) - this.f2008cy);
                }
                this.finalMatrix.postConcat(this.toGL);
                dualPosition.set(this.finalMatrix);
                updateDualPosition();
                float f11 = this.f2008cy;
                boolean z5 = Math.min(f11, f11 - (this.f2009h / 2.0f)) < ((float) AndroidUtilities.m107dp(66));
                float f12 = this.f2008cy;
                boolean z6 = Math.max(f12, (this.f2009h / 2.0f) + f12) > ((float) (getHeight() - AndroidUtilities.m107dp(66)));
                if (this.atTop != z5) {
                    this.atTop = z5;
    ...
    private void extractPointsData(Matrix matrix) {
        ...
        this.f2007cx = fArr2[0];
        this.f2008cy = fArr2[1];
        fArr2[0] = 1.0f;
        fArr2[1] = 0.0f;
        matrix.mapPoints(fArr2);
        float[] fArr3 = this.vertices;
        this.angle = (float) Math.toDegrees(Math.atan2(fArr3[1] - this.f2008cy, fArr3[0] - this.f2007cx));
        float f = this.f2007cx;
        float f2 = this.f2008cy;
        float[] fArr4 = this.vertices;
        this.f2010w = MathUtils.distance(f, f2, fArr4[0], fArr4[1]) * 2.0f;
        float[] fArr5 = this.vertices;
        fArr5[0] = 0.0f;
        fArr5[1] = 1.0f;
        matrix.mapPoints(fArr5);
        float f3 = this.f2007cx;
        float f4 = this.f2008cy;
        float[] fArr6 = this.vertices;
        this.f2009h = MathUtils.distance(f3, f4, fArr6[0], fArr6[1]) * 2.0f;
    }
