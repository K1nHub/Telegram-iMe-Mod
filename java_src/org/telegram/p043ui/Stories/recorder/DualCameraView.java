...
public class DualCameraView extends CameraView implements CameraController.ErrorCallback {
    ...
    private boolean checkTap(MotionEvent motionEvent) {
        if (motionEvent.getAction() == 0) {
            ...
            if (System.currentTimeMillis() - this.tapTime <= ViewConfiguration.getTapTimeout() && MathUtils.distance(this.tapX, this.tapY, motionEvent.getX(), motionEvent.getY()) < AndroidUtilities.m104dp(10)) {
                if (isAtDual(this.tapX, this.tapY)) {
                    switchCamera();
                    this.lastFocusToPoint = null;
                } else {
    ...
    private boolean touchEvent(MotionEvent motionEvent) {
        ...
        if (isDual()) {
            ...
            if (motionEvent.getAction() == 2 && this.down) {
                if (MathUtils.distance(f3, f4, f5, f6) > AndroidUtilities.m104dp(2) && (runnable = this.longpressRunnable) != null) {
                    AndroidUtilities.cancelRunOnUIThread(runnable);
                    this.longpressRunnable = null;
                }
