...
public class StoryPrivacyButton extends View {
    ...
    public boolean set(boolean z, TLRPC$StoryItem tLRPC$StoryItem, boolean z2) {
        ...
        if (tLRPC$StoryItem == null) {
            ...
            setIcon(C3419R.C3421drawable.msg_stories_closefriends, 15.0f);
            setupGradient(-7808710, -13781445);
            this.crossfadeT.set(z2, true);
        } else if (tLRPC$StoryItem.contacts) {
            setIcon(C3419R.C3421drawable.msg_folders_private, 17.33f);
            setupGradient(-3905294, -6923014);
            this.crossfadeT.set(z2, true);
        } else if (tLRPC$StoryItem.selected_contacts) {
            setIcon(C3419R.C3421drawable.msg_folders_groups, 17.33f);
            setupGradient(-18621, -618956);
            this.crossfadeT.set(z2, true);
        } else if (z) {
            setIcon(C3419R.C3421drawable.msg_folders_channels, 17.33f);
    ...
    private void setupGradient(int i, int i2) {
        ...
        this.topColor = i;
        this.bottomColor = i2;
        LinearGradient linearGradient = new LinearGradient((float) BitmapDescriptorFactory.HUE_RED, (float) BitmapDescriptorFactory.HUE_RED, (float) BitmapDescriptorFactory.HUE_RED, AndroidUtilities.m72dp(23), new int[]{i, i2}, new float[]{BitmapDescriptorFactory.HUE_RED, 1.0f}, Shader.TileMode.CLAMP);
        this.gradientMatrix.reset();
        this.gradientMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, AndroidUtilities.m72dp(8));
    ...
    protected void onDraw(Canvas canvas) {
        if (this.draw) {
            ...
            if (f > BitmapDescriptorFactory.HUE_RED) {
                ...
                canvas.drawRoundRect(rectF, AndroidUtilities.m72dp(12), AndroidUtilities.m72dp(12), this.backgroundPaint[1]);
            }
            if (f < 1.0f) {
                this.backgroundPaint[0].setAlpha((int) ((1.0f - f) * 255.0f));
                canvas.drawRoundRect(rectF, AndroidUtilities.m72dp(12), AndroidUtilities.m72dp(12), this.backgroundPaint[0]);
            }
    ...
    protected void onMeasure(int i, int i2) {
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m72dp(60), 1073741824), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m72dp(40), 1073741824));
    }

    public float getCenterX() {
        return getX() + (getWidth() / 2.0f) + (this.drawArrow ? 0 : AndroidUtilities.m72dp(14));
    }
}
