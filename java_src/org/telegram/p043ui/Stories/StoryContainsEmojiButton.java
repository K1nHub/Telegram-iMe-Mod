...
public class StoryContainsEmojiButton extends View {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        int m72dp = AndroidUtilities.m72dp(29);
        StaticLayout staticLayout = this.layout;
        setMeasuredDimension(z ? View.MeasureSpec.getSize(i) : getMinimumWidth(), paddingTop + AndroidUtilities.lerp(m72dp, staticLayout == null ? AndroidUtilities.m72dp(29) : staticLayout.getHeight(), this.loadT) + getPaddingBottom());
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f < 1.0f) {
            ...
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop(), getMeasuredWidth() - getPaddingRight(), getPaddingTop() + AndroidUtilities.m72dp(12), Path.Direction.CW);
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop() + AndroidUtilities.m72dp(16), getPaddingLeft() + (((getMeasuredWidth() - getPaddingRight()) - getPaddingLeft()) * 0.46f), getPaddingTop() + AndroidUtilities.m72dp(28), Path.Direction.CW);
    ...
    public /* synthetic */ void lambda$load$1(TLObject tLObject, Object obj, ArrayList arrayList, boolean[] zArr, int i) {
        ...
        if (this.inputSets != null && arrayList != null && !arrayList.isEmpty()) {
            for (int i3 = 0; i3 < arrayList.size(); i3++) {
                ...
                long j = tLRPC$InputStickerSet.f1541id;
                int i4 = 0;
                while (true) {
                    if (i4 >= this.inputSets.size()) {
                        z = false;
                        break;
                    } else if (this.inputSets.get(i4).f1541id == j) {
                        z = true;
                        break;
                    } else {
                        i4++;
    ...
    private void set(TLRPC$TL_messages_stickerSet tLRPC$TL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                break;
            } else if (arrayList.get(i).f1526id == tLRPC$TL_messages_stickerSet.set.thumb_document_id) {
        ...
        if (z && this.stickers) {
            string = LocaleController.getString(C3419R.string.StoryContainsStickersEmojiFrom);
        } else if (z) {
            string = LocaleController.getString(C3419R.string.StoryContainsEmojiFrom);
        } else {
            string = LocaleController.getString(C3419R.string.StoryContainsStickersFrom);
        }
    ...
    private void set(TLRPC$StickerSetCovered tLRPC$StickerSetCovered) {
        ...
        if (tLRPC$Document == null && (tLRPC$StickerSetCovered instanceof TLRPC$TL_stickerSetFullCovered)) {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                if (arrayList.get(i).f1526id == tLRPC$StickerSetCovered.set.thumb_document_id) {
                    tLRPC$Document = arrayList.get(i);
                }
            }
            if (tLRPC$Document == null && !arrayList.isEmpty()) {
    ...
    private void set(int i) {
        boolean z = this.emoji;
        if (z && this.stickers) {
            setText(AndroidUtilities.replaceTags(LocaleController.formatPluralString("StoryContainsStickersEmoji", i, new Object[0])));
        } else if (z) {
            setText(AndroidUtilities.replaceTags(LocaleController.formatPluralString("StoryContainsEmoji", i, new Object[0])));
        } else {
            setText(AndroidUtilities.replaceTags(LocaleController.formatPluralString("StoryContainsStickers", i, new Object[0])));
        }
    }
    ...
    private void animateLoad(boolean z) {
        ...
        if (z) {
            ...
            if (this.layout == null || Math.abs(getMeasuredHeight() - ((getPaddingTop() + this.layout.getHeight()) + getPaddingBottom())) > AndroidUtilities.m72dp(3)) {
                z2 = true;
            }
