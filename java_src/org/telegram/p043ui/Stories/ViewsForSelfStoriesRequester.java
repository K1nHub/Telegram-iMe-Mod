...
/* renamed from: org.telegram.ui.Stories.ViewsForSelfStoriesRequester */
...
public class ViewsForSelfStoriesRequester {
    ...
    public boolean lambda$new$0() {
        ...
        for (int i = 0; i < stories.stories.size(); i++) {
            tLRPC$TL_stories_getStoriesViews.f1657id.add(Integer.valueOf(stories.stories.get(i).f1553id));
        }
        ...
        if (tLObject != null) {
            ...
            for (int i = 0; i < tLRPC$TL_stories_storyViews.views.size(); i++) {
                for (int i2 = 0; i2 < stories.stories.size(); i2++) {
                    if (stories.stories.get(i2).f1553id == tLRPC$TL_stories_getStoriesViews.f1657id.get(i).intValue()) {
