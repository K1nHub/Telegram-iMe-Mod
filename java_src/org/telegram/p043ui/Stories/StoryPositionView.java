...
public class StoryPositionView {
    ...
    public StoryPositionView() {
        ...
        spannableStringBuilder.append((CharSequence) " ").setSpan(new DialogCell.FixedWidthSpan(AndroidUtilities.m72dp(1)), 0, 1, 0);
        SpannableStringBuilder spannableStringBuilder2 = new SpannableStringBuilder();
        this.rightSpace = spannableStringBuilder2;
        spannableStringBuilder2.append((CharSequence) " ").setSpan(new DialogCell.FixedWidthSpan(AndroidUtilities.m72dp(1)), 0, 1, 0);
    }
    ...
    public void draw(Canvas canvas, float f, int i, int i2, FrameLayout frameLayout, PeerStoriesView.PeerHeaderView peerHeaderView) {
        ...
        canvas.translate((((AndroidUtilities.m72dp(4) + peerHeaderView.getLeft()) + peerHeaderView.titleView.getLeft()) + peerHeaderView.titleView.getTextWidth()) - Utilities.clamp((peerHeaderView.titleView.getTextWidth() + currentWidth) - peerHeaderView.titleView.getWidth(), currentWidth, 0), ((peerHeaderView.getY() + peerHeaderView.titleView.getTop()) + (this.textDrawable.getHeight() / 2.0f)) - 1.0f);
        float m72dp = AndroidUtilities.m72dp(8);
        float m72dp2 = AndroidUtilities.m72dp(2);
        AndroidUtilities.rectTmp.set(-m72dp, -m72dp2, this.textDrawable.getCurrentWidth() + m72dp, this.textDrawable.getHeight() + m72dp2);
