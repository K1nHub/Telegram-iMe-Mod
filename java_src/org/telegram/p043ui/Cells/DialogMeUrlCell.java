...
public class DialogMeUrlCell extends BaseCell {
    ...
    public DialogMeUrlCell(Context context) {
        ...
        this.messageTop = AndroidUtilities.m72dp(40);
        this.avatarTop = AndroidUtilities.m72dp(10);
        this.currentAccount = UserConfig.selectedAccount;
        Theme.createDialogsResources(context);
        this.avatarImage.setRoundRadius(AndroidUtilities.m72dp(26));
    }
    ...
    protected void onMeasure(int i, int i2) {
        setMeasuredDimension(View.MeasureSpec.getSize(i), AndroidUtilities.m72dp(72) + (this.useSeparator ? 1 : 0));
    }
    ...
    public void buildLayout() {
        ...
        if (tLRPC$RecentMeUrl instanceof TLRPC$TL_recentMeUrlChat) {
            ...
            if (!LocaleController.isRTL) {
                ...
                this.nameLeft = AndroidUtilities.m72dp(AndroidUtilities.leftBaseline + 4);
            } else {
                this.nameLockLeft = getMeasuredWidth() - AndroidUtilities.m72dp(AndroidUtilities.leftBaseline);
                this.nameLeft = AndroidUtilities.m72dp(14);
            }
            str = chat.title;
            ...
            if (user != null) {
                if (user.bot) {
                    this.nameLockTop = AndroidUtilities.m73dp(16.5f);
                    if (!LocaleController.isRTL) {
                        this.nameLockLeft = AndroidUtilities.m72dp(AndroidUtilities.leftBaseline);
                        this.nameLeft = AndroidUtilities.m72dp(AndroidUtilities.leftBaseline + 4);
                    } else {
        ...
        if (TextUtils.isEmpty(str)) {
            str = LocaleController.getString("HiddenName", C3473R.string.HiddenName);
        }
        if (!LocaleController.isRTL) {
            ...
            measuredWidth = getMeasuredWidth() - this.nameLeft;
            m72dp = AndroidUtilities.m72dp(AndroidUtilities.leftBaseline);
        }
        int i = measuredWidth - m72dp;
        if (this.drawNameLock) {
            i -= AndroidUtilities.m72dp(4) + Theme.dialogs_lockDrawable.getIntrinsicWidth();
        }
        if (this.drawVerified) {
            int m72dp2 = AndroidUtilities.m72dp(6) + Theme.dialogs_verifiedDrawable.getIntrinsicWidth();
            i -= m72dp2;
            if (LocaleController.isRTL) {
                this.nameLeft += m72dp2;
            }
        }
        int max = Math.max(AndroidUtilities.m72dp(12), i);
        try {
            this.nameLayout = new StaticLayout(TextUtils.ellipsize(str.replace('\n', ' '), textPaint, max - AndroidUtilities.m72dp(12), TextUtils.TruncateAt.END), textPaint, max, Layout.Alignment.ALIGN_NORMAL, 1.0f, BitmapDescriptorFactory.HUE_RED, false);
        } catch (Exception e) {
            FileLog.m67e(e);
        }
        int measuredWidth3 = getMeasuredWidth() - AndroidUtilities.m72dp(AndroidUtilities.leftBaseline + 16);
        if (!LocaleController.isRTL) {
            this.messageLeft = AndroidUtilities.m72dp(AndroidUtilities.leftBaseline);
            measuredWidth2 = AndroidUtilities.m72dp(AndroidUtilities.isTablet() ? 13 : 9);
        } else {
            this.messageLeft = AndroidUtilities.m72dp(16);
            measuredWidth2 = getMeasuredWidth() - AndroidUtilities.m72dp(AndroidUtilities.isTablet() ? 65 : 61);
        }
        this.avatarImage.setImageCoords(measuredWidth2, this.avatarTop, AndroidUtilities.m72dp(52), AndroidUtilities.m72dp(52));
        int max2 = Math.max(AndroidUtilities.m72dp(12), measuredWidth3);
        try {
            this.messageLayout = new StaticLayout(TextUtils.ellipsize(str4, textPaint2, max2 - AndroidUtilities.m72dp(12), TextUtils.TruncateAt.END), textPaint2, max2, Layout.Alignment.ALIGN_NORMAL, 1.0f, BitmapDescriptorFactory.HUE_RED, false);
        } catch (Exception e2) {
            FileLog.m67e(e2);
        }
        if (LocaleController.isRTL) {
            ...
            if (staticLayout != null && staticLayout.getLineCount() > 0) {
                ...
                if (this.drawVerified) {
                    this.nameMuteLeft = (int) (((this.nameLeft + (max - ceil)) - AndroidUtilities.m72dp(6)) - Theme.dialogs_verifiedDrawable.getIntrinsicWidth());
                }
        ...
        if (staticLayout3 != null && staticLayout3.getLineCount() > 0) {
            ...
            if (this.drawVerified) {
                this.nameMuteLeft = (int) (this.nameLeft + lineRight + AndroidUtilities.m72dp(6));
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.nameLayout != null) {
            canvas.save();
            canvas.translate(this.nameLeft, AndroidUtilities.m72dp(13));
        ...
        if (this.drawVerified) {
            BaseCell.setDrawableBounds(Theme.dialogs_verifiedDrawable, this.nameMuteLeft, AndroidUtilities.m73dp(16.5f));
            BaseCell.setDrawableBounds(Theme.dialogs_verifiedCheckDrawable, this.nameMuteLeft, AndroidUtilities.m73dp(16.5f));
