...
public class GroupCreateUserCell extends FrameLayout {
    ...
    public GroupCreateUserCell(Context context, int i, int i2, boolean z, boolean z2, Theme.ResourcesProvider resourcesProvider) {
        ...
        SimpleTextView simpleTextView = new SimpleTextView(this, context) { // from class: org.telegram.ui.Cells.GroupCreateUserCell.1
            ...
            public boolean setText(CharSequence charSequence, boolean z4) {
                return super.setText(Emoji.replaceEmoji(charSequence, getPaint().getFontMetricsInt(), AndroidUtilities.m104dp(14), false), z4);
            }
        };
    ...
    protected void onMeasure(int i, int i2) {
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(View.MeasureSpec.getSize(i), 1073741824), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m104dp(this.currentObject instanceof String ? 50 : 58), 1073741824));
    }
    ...
    */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.checkBoxType == 2 && (this.isChecked || this.checkProgress > BitmapDescriptorFactory.HUE_RED)) {
            ...
            canvas.drawCircle(this.avatarImageView.getLeft() + (this.avatarImageView.getMeasuredWidth() / 2), this.avatarImageView.getTop() + (this.avatarImageView.getMeasuredHeight() / 2), AndroidUtilities.m104dp(18) + (AndroidUtilities.m104dp(4) * this.checkProgress), this.paint);
        }
        if (this.drawDivider) {
            int m104dp = AndroidUtilities.m104dp(LocaleController.isRTL ? 0 : this.padding + 72);
            int measuredWidth = getMeasuredWidth() - AndroidUtilities.m104dp(LocaleController.isRTL ? this.padding + 72 : 0);
            if (this.forceDarkTheme) {
                Theme.dividerExtraPaint.setColor(Theme.getColor(Theme.key_voipgroup_actionBar, this.resourcesProvider));
                canvas.drawRect(m104dp, getMeasuredHeight() - 1, measuredWidth, getMeasuredHeight(), Theme.dividerExtraPaint);
                return;
            }
            canvas.drawRect(m104dp, getMeasuredHeight() - 1, measuredWidth, getMeasuredHeight(), Theme.dividerPaint);
        }
    }
