...
public class ExpiredStoryView {
    ...
    public void measure(ChatMessageCell chatMessageCell) {
        ...
        String string = LocaleController.getString("From", C3473R.string.From);
        ...
        String string2 = LocaleController.getString("FromFormatted", C3473R.string.FromFormatted);
        ...
        this.titleLayout = new StaticLayout(createExpiredStoryString, textPaint2, ((int) (textPaint2.measureText(createExpiredStoryString, 0, createExpiredStoryString.length()) + 1.0f)) + AndroidUtilities.m72dp(10), Layout.Alignment.ALIGN_NORMAL, 1.0f, BitmapDescriptorFactory.HUE_RED, false);
        this.subtitleLayout = new StaticLayout(str2, textPaint2, ((int) (textPaint2.measureText((CharSequence) str2, 0, str2.length()) + 1.0f)) + AndroidUtilities.m72dp(10), Layout.Alignment.ALIGN_NORMAL, 1.0f, BitmapDescriptorFactory.HUE_RED, false);
        this.height = 0;
        this.verticalPadding = AndroidUtilities.m72dp(4);
        this.horizontalPadding = AndroidUtilities.m72dp(12);
        this.height = (int) (this.height + AndroidUtilities.m72dp(4) + this.titleLayout.getHeight() + AndroidUtilities.m72dp(2) + this.subtitleLayout.getHeight() + AndroidUtilities.m72dp(4) + (this.verticalPadding * 2.0f));
        this.width = Math.max(this.titleLayout.getWidth(), this.subtitleLayout.getWidth()) + AndroidUtilities.m72dp(12) + AndroidUtilities.m72dp(20) + chatMessageCell.getExtraTextX();
    }

    public void draw(Canvas canvas, ChatMessageCell chatMessageCell) {
        float m72dp = AndroidUtilities.m72dp(8) + this.verticalPadding;
        this.textY = m72dp;
        if (chatMessageCell.pinnedTop) {
            this.textY = m72dp - AndroidUtilities.m72dp(2);
        }
        RectF rectF = AndroidUtilities.rectTmp;
        if (chatMessageCell.getMessageObject().isOutOwner()) {
            this.textX = (((((-(chatMessageCell.timeWidth + AndroidUtilities.m72dp(12))) + chatMessageCell.getExtraTextX()) + chatMessageCell.getMeasuredWidth()) - this.width) + AndroidUtilities.m72dp(24)) - this.horizontalPadding;
            ...
            float m72dp2 = chatMessageCell.isAvatarVisible ? AndroidUtilities.m72dp(48) : 0.0f;
            this.textX = this.horizontalPadding + m72dp2 + AndroidUtilities.m72dp(12);
            float f = this.horizontalPadding;
            rectF.set(m72dp2 + f, this.verticalPadding, m72dp2 + f + this.width, chatMessageCell.getMeasuredHeight() - this.verticalPadding);
        }
        if (chatMessageCell.getMessageObject().isOutOwner()) {
        ...
        canvas.translate(BitmapDescriptorFactory.HUE_RED, this.titleLayout.getHeight() + AndroidUtilities.m72dp(2));
