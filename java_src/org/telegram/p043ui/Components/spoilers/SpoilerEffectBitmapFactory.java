...
public class SpoilerEffectBitmapFactory {
    ...
    private SpoilerEffectBitmapFactory() {
        new Matrix();
        int m104dp = AndroidUtilities.m104dp(SharedConfig.getDevicePerformanceClass() == 2 ? ImageReceiver.DEFAULT_CROSSFADE_DURATION : 100);
        Point point = AndroidUtilities.displaySize;
        int min = (int) Math.min(Math.min(point.x, point.y) * 0.5f, m104dp);
        this.size = min;
        if (min < AndroidUtilities.m104dp(80)) {
            this.size = AndroidUtilities.m104dp(80);
        }
    }
    ...
    public Paint getPaint() {
        if (this.shaderBitmap == null) {
            ...
            int i2 = this.size;
            int i3 = (int) (i2 / 10.0f);
            int m104dp = (int) ((i2 / AndroidUtilities.m104dp(200)) * 60.0f);
            for (int i4 = 0; i4 < 10; i4++) {
                for (int i5 = 0; i5 < 10; i5++) {
                    ...
                    int i6 = i3 * i4;
                    int i7 = i3 * i5;
                    spoilerEffect.setBounds(i6, i7 - AndroidUtilities.m104dp(5), i6 + i3 + AndroidUtilities.m104dp(3), i7 + i3 + AndroidUtilities.m104dp(5));
                    spoilerEffect.drawPoints = true;
                    spoilerEffect.particlePoints = (float[][]) Array.newInstance(float.class, SpoilerEffect.ALPHAS.length, m104dp * 2);
                    spoilerEffect.setMaxParticlesCount(m104dp);
