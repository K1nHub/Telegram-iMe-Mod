...
public class BotKeyboardView extends LinearLayout {
    ...
    public void updateColors() {
        ...
        for (int i2 = 0; i2 < this.buttonViews.size(); i2++) {
            ...
            this.buttonViews.get(i2).setBackground(Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.m104dp(4), getThemedColor(Theme.key_chat_botKeyboardButtonBackground), getThemedColor(Theme.key_chat_botKeyboardButtonBackgroundPressed)));
    ...
    public void setPanelHeight(int i) {
        ...
        this.buttonHeight = !this.isFullSize ? 42 : (int) Math.max(42.0f, (((this.panelHeight - AndroidUtilities.m104dp(30)) - ((this.botButtons.rows.size() - 1) * AndroidUtilities.m104dp(10))) / this.botButtons.rows.size()) / AndroidUtilities.density);
        int childCount = this.container.getChildCount();
        int m104dp = AndroidUtilities.m104dp(this.buttonHeight);
        for (int i2 = 0; i2 < childCount; i2++) {
            View childAt = this.container.getChildAt(i2);
            LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) childAt.getLayoutParams();
            if (layoutParams.height != m104dp) {
                layoutParams.height = m104dp;
    ...
    public void setButtons(TLRPC$TL_replyKeyboardMarkup tLRPC$TL_replyKeyboardMarkup) {
        ...
        this.isFullSize = z;
        int i3 = 10;
        this.buttonHeight = !z ? 42 : (int) Math.max(42.0f, (((this.panelHeight - AndroidUtilities.m104dp(30)) - ((this.botButtons.rows.size() - 1) * AndroidUtilities.m104dp(10))) / this.botButtons.rows.size()) / AndroidUtilities.density);
        int i4 = 0;
        while (i4 < tLRPC$TL_replyKeyboardMarkup2.rows.size()) {
            ...
            while (i5 < tLRPC$TL_keyboardButtonRow.buttons.size()) {
                ...
                if ((tLRPC$KeyboardButton instanceof TLRPC$TL_keyboardButtonWebView) || (tLRPC$KeyboardButton instanceof TLRPC$TL_keyboardButtonSimpleWebView)) {
                    imageView.setImageResource(C3634R.C3636drawable.bot_webview);
                    i = 0;
    ...
    public class Button extends TextView {
        public Button(BotKeyboardView botKeyboardView, Context context, TLRPC$KeyboardButton tLRPC$KeyboardButton) {
            ...
            setBackground(Theme.createSimpleSelectorRoundRectDrawable(AndroidUtilities.m104dp(4), botKeyboardView.getThemedColor(Theme.key_chat_botKeyboardButtonBackground), botKeyboardView.getThemedColor(Theme.key_chat_botKeyboardButtonBackgroundPressed)));
            setTextSize(1, 16.0f);
            setGravity(17);
            setPadding(AndroidUtilities.m104dp(4), 0, AndroidUtilities.m104dp(4), 0);
    ...
    public int getKeyboardHeight() {
        ...
        return this.isFullSize ? this.panelHeight : (tLRPC$TL_replyKeyboardMarkup.rows.size() * AndroidUtilities.m104dp(this.buttonHeight)) + AndroidUtilities.m104dp(30) + ((this.botButtons.rows.size() - 1) * AndroidUtilities.m104dp(10));
    }
