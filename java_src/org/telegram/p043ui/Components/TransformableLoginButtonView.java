...
public class TransformableLoginButtonView extends View {
    ...
    public TransformableLoginButtonView(Context context) {
        ...
        this.outlinePaint.setStrokeWidth(AndroidUtilities.m104dp(2));
    ...
    protected void onDraw(Canvas canvas) {
        if (this.drawBackground) {
            float m105dp = AndroidUtilities.m105dp(((this.transformType == 0 ? this.progress : 1.0f) * 26.0f) + 6.0f);
            this.rect.set(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight());
            canvas.drawRoundRect(this.rect, m105dp, m105dp, this.backgroundPaint);
        }
        ...
        if (i == IdFabric$CustomType.TRANSFORMABLE_LOGIN_BUTTON_SEARCH) {
            ...
            if (textPaint != null && this.buttonText != null) {
                ...
                canvas.drawText(this.buttonText, (getWidth() - this.buttonWidth) / 2.0f, ((getHeight() / 2.0f) + (this.textPaint.getTextSize() / 2.0f)) - AndroidUtilities.m105dp(1.75f), this.textPaint);
                this.textPaint.setAlpha(alpha);
            }
            float max = (Math.max(0.4f, this.progress) - 0.4f) / 0.6f;
            if (max != BitmapDescriptorFactory.HUE_RED) {
                float m105dp2 = AndroidUtilities.m105dp(21.0f) + ((getWidth() - (AndroidUtilities.m105dp(21.0f) * 2)) * max);
                float height2 = getHeight() / 2.0f;
                canvas.drawLine(AndroidUtilities.m105dp(21.0f), height2, m105dp2, height2, this.outlinePaint);
                double m105dp3 = AndroidUtilities.m105dp(9.0f) * max;
                float cos = (float) (m105dp2 - (Math.cos(0.7853981633974483d) * m105dp3));
                float sin = (float) (Math.sin(0.7853981633974483d) * m105dp3);
                canvas.drawLine(m105dp2, height2, cos, height2 - sin, this.outlinePaint);
                canvas.drawLine(m105dp2, height2, cos, height2 + sin, this.outlinePaint);
            }
        } else if (i == 1) {
            float m105dp4 = AndroidUtilities.m105dp(21.0f);
            float width2 = getWidth() - AndroidUtilities.m105dp(21.0f);
            float height3 = getHeight() / 2.0f;
            canvas.save();
            canvas.translate((-AndroidUtilities.m104dp(2)) * this.progress, BitmapDescriptorFactory.HUE_RED);
            canvas.rotate(this.progress * 90.0f, getWidth() / 2.0f, getHeight() / 2.0f);
            canvas.drawLine(((width2 - m105dp4) * this.progress) + m105dp4, height3, width2, height3, this.outlinePaint);
            int m105dp5 = AndroidUtilities.m105dp((this.progress * (-1.0f)) + 9.0f);
            int m105dp6 = AndroidUtilities.m105dp((this.progress * 7.0f) + 9.0f);
            double d = width2;
            double d2 = m105dp5;
            double d3 = height3;
