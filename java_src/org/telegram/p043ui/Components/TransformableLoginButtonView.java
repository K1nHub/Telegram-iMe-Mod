...
public class TransformableLoginButtonView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (this.drawBackground) {
            float m73dp = AndroidUtilities.m73dp(((this.transformType == 0 ? this.progress : 1.0f) * 26.0f) + 6.0f);
            this.rect.set(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight());
            canvas.drawRoundRect(this.rect, m73dp, m73dp, this.backgroundPaint);
        }
        int i = this.transformType;
        if (i == IdFabric$CustomType.TRANSFORMABLE_LOGIN_BUTTON_SEARCH) {
            Drawable drawable = ContextCompat.getDrawable(getContext(), C3473R.C3475drawable.fork_ic_search_contact_30);
            ...
            if (textPaint != null && this.buttonText != null) {
                ...
                canvas.drawText(this.buttonText, (getWidth() - this.buttonWidth) / 2.0f, ((getHeight() / 2.0f) + (this.textPaint.getTextSize() / 2.0f)) - AndroidUtilities.m73dp(1.75f), this.textPaint);
                this.textPaint.setAlpha(alpha);
            }
            float max = (Math.max(0.4f, this.progress) - 0.4f) / 0.6f;
            if (max != BitmapDescriptorFactory.HUE_RED) {
                float m73dp2 = AndroidUtilities.m73dp(21.0f) + ((getWidth() - (AndroidUtilities.m73dp(21.0f) * 2)) * max);
                float height2 = getHeight() / 2.0f;
                canvas.drawLine(AndroidUtilities.m73dp(21.0f), height2, m73dp2, height2, this.outlinePaint);
                double m73dp3 = AndroidUtilities.m73dp(9.0f) * max;
                float cos = (float) (m73dp2 - (Math.cos(0.7853981633974483d) * m73dp3));
                float sin = (float) (Math.sin(0.7853981633974483d) * m73dp3);
                canvas.drawLine(m73dp2, height2, cos, height2 - sin, this.outlinePaint);
                canvas.drawLine(m73dp2, height2, cos, height2 + sin, this.outlinePaint);
            }
        } else if (i == 1) {
            float m73dp4 = AndroidUtilities.m73dp(21.0f);
            float width2 = getWidth() - AndroidUtilities.m73dp(21.0f);
            float height3 = getHeight() / 2.0f;
            canvas.save();
            canvas.translate((-AndroidUtilities.m72dp(2)) * this.progress, BitmapDescriptorFactory.HUE_RED);
            canvas.rotate(this.progress * 90.0f, getWidth() / 2.0f, getHeight() / 2.0f);
            canvas.drawLine(((width2 - m73dp4) * this.progress) + m73dp4, height3, width2, height3, this.outlinePaint);
            int m73dp5 = AndroidUtilities.m73dp((this.progress * (-1.0f)) + 9.0f);
            int m73dp6 = AndroidUtilities.m73dp((this.progress * 7.0f) + 9.0f);
            double d = width2;
            double d2 = m73dp5;
            double d3 = height3;
