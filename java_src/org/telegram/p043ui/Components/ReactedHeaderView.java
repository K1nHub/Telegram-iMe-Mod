...
public class ReactedHeaderView extends FrameLayout {
    ...
    public ReactedHeaderView(Context context, int i, MessageObject messageObject, long j) {
        ...
        Drawable mutate = ContextCompat.getDrawable(context, C3473R.C3475drawable.msg_reactions).mutate();
    ...
    public static class UserSeen {
        ...
        public UserSeen(TLObject tLObject, int i) {
            ...
            if (tLObject instanceof TLRPC$User) {
                this.dialogId = ((TLRPC$User) tLObject).f1685id;
            } else if (tLObject instanceof TLRPC$Chat) {
                this.dialogId = -((TLRPC$Chat) tLObject).f1522id;
            }
        }
    }
    ...
    public /* synthetic */ void lambda$onAttachedToWindow$5(long j, TLRPC$Chat tLRPC$Chat, TLObject tLObject, TLRPC$TL_error tLRPC$TL_error) {
        if (tLObject instanceof TLRPC$Vector) {
            ...
            if (ChatObject.isChannel(tLRPC$Chat)) {
                ...
                tLRPC$TL_channels_getParticipants.channel = MessagesController.getInstance(this.currentAccount).getInputChannel(tLRPC$Chat.f1522id);
    ...
    public /* synthetic */ void lambda$onAttachedToWindow$1(TLObject tLObject, List list, List list2, List list3, Runnable runnable) {
        if (tLObject != null) {
            ...
            for (int i = 0; i < tLRPC$TL_channels_channelParticipants.users.size(); i++) {
                ...
                int indexOf = list.indexOf(Long.valueOf(tLRPC$User.f1685id));
    ...
    public /* synthetic */ void lambda$loadReactions$6(int i, TLRPC$TL_messages_messageReactionsList tLRPC$TL_messages_messageReactionsList) {
        ...
        while (it.hasNext()) {
            ...
            TLRPC$Peer tLRPC$Peer = this.message.messageOwner.from_id;
            if (tLRPC$Peer != null && next.f1685id != tLRPC$Peer.user_id) {
                int i2 = 0;
                while (true) {
                    if (i2 >= this.users.size()) {
                        z3 = false;
                        break;
                    } else if (this.users.get(i2).dialogId == next.f1685id) {
                        z3 = true;
                        break;
                    } else {
                        i2++;
        ...
        while (it2.hasNext()) {
            ...
            if (tLRPC$Peer2 != null && next2.f1522id != tLRPC$Peer2.user_id) {
                int i3 = 0;
                while (true) {
                    if (i3 >= this.users.size()) {
                        z2 = false;
                        break;
                    } else if (this.users.get(i3).dialogId == (-next2.f1522id)) {
                        z2 = true;
                        break;
                    } else {
                        i3++;
