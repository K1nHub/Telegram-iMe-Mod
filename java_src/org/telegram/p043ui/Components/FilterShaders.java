...
public class FilterShaders {
            ...
            if (loadShader3 != 0 && loadShader4 != 0) {
                ...
                if (loadShader5 != 0 && loadShader6 != 0) {
                    ...
                    if (loadShader7 != 0 && loadShader8 != 0) {
                        ...
                        int loadShader9 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 vTextureCoord;void main() {gl_Position = position;vTextureCoord = inputTexCoord;}");
                        ...
                        if (loadShader9 != 0 && loadShader10 != 0) {
                            ...
                            int loadShader11 = loadShader(35632, "precision lowp float;varying highp vec2 vTextureCoord;varying highp vec2 texCoord2;uniform sampler2D sTexture;uniform sampler2D inputImageTexture2;void main() {vec4 image = texture2D(sTexture, vTextureCoord);vec4 blurredImage = texture2D(inputImageTexture2, texCoord2);gl_FragColor = vec4((image.rgb - blurredImage.rgb + vec3(0.5,0.5,0.5)), image.a);}");
                            int loadShader12 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 vTextureCoord;varying vec2 texCoord2;void main() {gl_Position = position;vTextureCoord = inputTexCoord;texCoord2 = inputTexCoord;}");
                            if (loadShader12 != 0 && loadShader11 != 0) {
                                int glCreateProgram6 = GLES20.glCreateProgram();
                                this.highPassProgram = glCreateProgram6;
                                GLES20.glAttachShader(glCreateProgram6, loadShader12);
                                GLES20.glAttachShader(this.highPassProgram, loadShader11);
    ...
    private boolean setupExternalShaders() {
        ...
        int i2 = 1;
        if (hDRType == 1) {
            readRes = RLottieDrawable.readRes(null, C3419R.raw.yuv_hlg2rgb);
        } else {
            readRes = hDRType == 2 ? RLottieDrawable.readRes(null, C3419R.raw.yuv_pq2rgb) : "";
        }
        ...
        while (true) {
            ...
            if (i3 < (z2 ? i : i2)) {
                ...
                GLES20.glDeleteProgram(this.rgbToHsvShaderProgram[i3]);
                if (loadShader5 == 0 || loadShader6 == 0) {
                    return false;
                }
                this.rgbToHsvShaderProgram[i3] = GLES20.glCreateProgram();
                GLES20.glAttachShader(this.rgbToHsvShaderProgram[i3], loadShader5);
                GLES20.glAttachShader(this.rgbToHsvShaderProgram[i3], loadShader6);
                GLES20.glBindAttribLocation(this.rgbToHsvShaderProgram[i3], 0, "position");
                GLES20.glBindAttribLocation(this.rgbToHsvShaderProgram[i3], 1, "inputTexCoord");
                GLES20.glLinkProgram(this.rgbToHsvShaderProgram[i3]);
                GLES20.glGetProgramiv(this.rgbToHsvShaderProgram[i3], 35714, iArr2, 0);
                if (iArr2[0] == 0) {
                    GLES20.glDeleteProgram(this.rgbToHsvShaderProgram[i3]);
                    this.rgbToHsvShaderProgram[i3] = 0;
                } else {
                    this.rgbToHsvPositionHandle[i3] = GLES20.glGetAttribLocation(this.rgbToHsvShaderProgram[i3], "position");
                    this.rgbToHsvInputTexCoordHandle[i3] = GLES20.glGetAttribLocation(this.rgbToHsvShaderProgram[i3], "inputTexCoord");
                    this.rgbToHsvSourceImageHandle[i3] = GLES20.glGetUniformLocation(this.rgbToHsvShaderProgram[i3], "sTexture");
                    if (i3 == 1) {
                        this.rgbToHsvMatrixHandle = GLES20.glGetUniformLocation(this.rgbToHsvShaderProgram[i3], "videoMatrix");
                        this.rgbToHsvTexSizeHandle = GLES20.glGetUniformLocation(this.rgbToHsvShaderProgram[i3], "texSize");
                    }
                }
                i3++;
                iArr = iArr2;
                ...
                if (this.isVideo) {
                    ...
                    loadShader3 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 vTextureCoord;varying vec2 texCoord2;void main() {gl_Position = position;vTextureCoord = inputTexCoord;texCoord2 = inputTexCoord;}");
                    loadShader4 = loadShader(35632, String.format(Locale.US, "%1$s\nprecision lowp float;varying highp vec2 vTextureCoord;varying highp vec2 texCoord2;uniform %2$s sTexture;uniform sampler2D toneCurveTexture;uniform sampler2D inputImageTexture3;uniform lowp float mixturePercent;void main() {vec4 image = texture2D(sTexture, vTextureCoord);vec4 mask = texture2D(inputImageTexture3, texCoord2);float redCurveValue = texture2D(toneCurveTexture, vec2(image.r, 0.0)).r;float greenCurveValue = texture2D(toneCurveTexture, vec2(image.g, 0.0)).g;float blueCurveValue = texture2D(toneCurveTexture, vec2(image.b, 0.0)).b;vec4 result = vec4(redCurveValue, greenCurveValue, blueCurveValue, image.a);vec4 tone = mix(image, result, mixturePercent);gl_FragColor = vec4(mix(image.rgb, tone.rgb, 1.0 - mask.b), 1.0);}", str, str2));
                }
                ...
                int glCreateProgram2 = GLES20.glCreateProgram();
                this.compositeProgram = glCreateProgram2;
                GLES20.glAttachShader(glCreateProgram2, loadShader3);
                GLES20.glAttachShader(this.compositeProgram, loadShader4);
    ...
    public boolean drawBlurPass() {
        ...
        if (blurType == 1) {
            ...
            GLES20.glUniform2f(this.radialBlurExcludePointHandle, blurExcludePoint.f1797x, blurExcludePoint.f1798y);
            ...
            GLES20.glUniform2f(this.linearBlurExcludePointHandle, blurExcludePoint2.f1797x, blurExcludePoint2.f1798y);
