        ...
        private boolean checkThread() {
            if (Thread.currentThread() != Looper.getMainLooper().getThread()) {
                if (BuildVars.DEBUG_VERSION) {
                    FileLog.m100e("EmojiDocumentFetcher", new IllegalStateException("Wrong thread"));
                    return false;
                }
                return false;
            }
    ...
    private void initDocument(boolean z) {
        ...
        if (this.document != null) {
            ...
            if (imageReceiver == null || z) {
                ...
                this.imageReceiver.setRoundRadius((i9 == 5 || i9 == 6) ? AndroidUtilities.m104dp(6) : i2);
                updateAttachState();
                invalidate();
            }
        }
            ...
            if (this.drawables[0] != null) {
                ...
                if (drawableArr3[0] instanceof AnimatedEmojiDrawable) {
                    if (((AnimatedEmojiDrawable) drawableArr3[0]).imageReceiver != null) {
                        ((AnimatedEmojiDrawable) this.drawables[0]).imageReceiver.setRoundRadius(AndroidUtilities.m104dp(4));
                    }
                    if (f < 1.0f) {
