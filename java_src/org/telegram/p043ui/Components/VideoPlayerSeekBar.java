...
public class VideoPlayerSeekBar {
    ...
    private int lineHeight = AndroidUtilities.m104dp(4);
    private int smallLineHeight = AndroidUtilities.m104dp(2);
    ...
    public VideoPlayerSeekBar(View view) {
        ...
        thumbWidth = AndroidUtilities.m104dp(24);
        this.currentRadius = AndroidUtilities.m104dp(6);
    ...
    public void setProgress(float f, boolean z) {
        ...
        if (z) {
            if (Math.abs(ceil - this.thumbX) > AndroidUtilities.m104dp(10)) {
        ...
        if (Math.abs(this.animatedThumbX - this.thumbX) > AndroidUtilities.m104dp(8)) {
            this.animatedThumbX = this.thumbX;
        }
    }
        ...
        try {
            ...
            for (URLSpanNoUnderline uRLSpanNoUnderline : uRLSpanNoUnderlineArr) {
                if (uRLSpanNoUnderline != null && uRLSpanNoUnderline.getURL() != null && uRLSpanNoUnderline.label != null && uRLSpanNoUnderline.getURL().startsWith("video?") && (parseInt = Utilities.parseInt((CharSequence) uRLSpanNoUnderline.getURL().substring(6))) != null && parseInt.intValue() >= 0) {
                    ...
                    Emoji.replaceEmoji((CharSequence) spannableStringBuilder, this.timestampLabelPaint.getFontMetricsInt(), AndroidUtilities.m104dp(14), false);
    ...
    public void draw(Canvas canvas, View view) {
        ...
        int i5 = this.height;
        rectF.top = AndroidUtilities.lerp((i5 - this.lineHeight) / 2.0f, (i5 - AndroidUtilities.m104dp(3)) - this.smallLineHeight, this.transitionProgress);
        RectF rectF2 = this.rect;
        int i6 = this.height;
        rectF2.bottom = AndroidUtilities.lerp((this.lineHeight + i6) / 2.0f, i6 - AndroidUtilities.m104dp(3), this.transitionProgress);
        float f = this.thumbX;
        ...
        float m104dp = AndroidUtilities.m104dp(this.pressed ? 8 : 6);
        if (this.currentRadius != m104dp) {
            ...
            if (f13 < m104dp) {
                float m104dp2 = f13 + (AndroidUtilities.m104dp(1) * (((float) j) / 60.0f));
                this.currentRadius = m104dp2;
                if (m104dp2 > m104dp) {
                    this.currentRadius = m104dp;
                }
            } else {
                float m104dp3 = f13 - (AndroidUtilities.m104dp(1) * (((float) j) / 60.0f));
                this.currentRadius = m104dp3;
                if (m104dp3 < m104dp) {
                    this.currentRadius = m104dp;
                }
            }
    ...
    private void drawProgressBar(Canvas canvas, RectF rectF, Paint paint2) {
        ...
        int i2;
        int i3 = 1;
        float m104dp = AndroidUtilities.m104dp(AndroidUtilities.lerp(2, 1, this.transitionProgress));
        ArrayList<Pair<Float, CharSequence>> arrayList = this.timestamps;
        if (arrayList == null || arrayList.isEmpty()) {
            canvas.drawRoundRect(rectF, m104dp, m104dp, paint2);
            return;
        }
        ...
        float m105dp = AndroidUtilities.m105dp(this.timestampsAppearing * 1.0f) / 2.0f;
        if (tmpPath == null) {
            tmpPath = new Path();
        }
        tmpPath.reset();
        float m104dp2 = AndroidUtilities.m104dp(4) / (lerp2 - lerp);
        int i4 = 0;
        while (true) {
            i = -1;
            if (i4 >= this.timestamps.size()) {
                i4 = -1;
                break;
            } else if (((Float) this.timestamps.get(i4).first).floatValue() >= m104dp2) {
                break;
            } else {
                i4++;
            }
        ...
        while (true) {
            if (size < 0) {
                break;
            } else if (1.0f - ((Float) this.timestamps.get(size).first).floatValue() >= m104dp2) {
                i = size + 1;
                break;
            } else {
                size--;
        ...
        while (i6 <= i5) {
            ...
            while (i6 != i5 && i6 != 0 && i6 < this.timestamps.size() - i3 && ((Float) this.timestamps.get(i6).first).floatValue() - floatValue <= m104dp2) {
                i6++;
                floatValue2 = ((Float) this.timestamps.get(i6).first).floatValue();
            }
            RectF rectF2 = AndroidUtilities.rectTmp;
            rectF2.left = AndroidUtilities.lerp(lerp, lerp2, floatValue) + (i6 > 0 ? m105dp : BitmapDescriptorFactory.HUE_RED);
            float lerp3 = AndroidUtilities.lerp(lerp, lerp2, floatValue2) - (i6 < i5 ? m105dp : BitmapDescriptorFactory.HUE_RED);
            rectF2.right = lerp3;
            ...
            if (f3 < f4) {
                ...
                if (i6 == i4 || (i7 != 0 && rectF2.left >= rectF.left)) {
                    ...
                    fArr[1] = m104dp;
                    fArr[0] = m104dp;
                    float f5 = 0.7f * m104dp * this.timestampsAppearing;
                    fArr[5] = f5;
                    fArr[4] = f5;
                    fArr[3] = f5;
                    fArr[2] = f5;
                    ...
                    i2 = i5;
                    float f6 = 0.7f * m104dp * this.timestampsAppearing;
                    fArr2[7] = f6;
                    fArr2[6] = f6;
                    fArr2[1] = f6;
                    fArr2[0] = f6;
                    ...
                    float f7 = this.timestampsAppearing * 0.7f * m104dp;
                    fArr3[5] = f7;
                    fArr3[4] = f7;
                    fArr3[3] = f7;
                    fArr3[2] = f7;
    ...
    private void drawTimestampLabel(Canvas canvas) {
        ...
        float abs = Math.abs(lerp - f3) - AndroidUtilities.m104dp(16);
        ...
        canvas.save();
        int i2 = this.height;
        canvas.translate(lerp + ((lerp2 - f) * this.transitionProgress), AndroidUtilities.lerp((this.lineHeight + i2) / 2.0f, i2 - AndroidUtilities.m104dp(3), this.transitionProgress) + AndroidUtilities.m104dp(12));
        if (this.timestampLabel[1] != null) {
            canvas.save();
            if (this.timestampChangeDirection != 0) {
                canvas.translate(AndroidUtilities.m104dp(8) + (AndroidUtilities.m104dp(16) * (-this.timestampChangeDirection) * interpolation), BitmapDescriptorFactory.HUE_RED);
            }
        ...
        if (this.timestampLabel[0] != null) {
            canvas.save();
            if (this.timestampChangeDirection != 0) {
                canvas.translate(AndroidUtilities.m104dp(8) + (AndroidUtilities.m104dp(16) * this.timestampChangeDirection * (1.0f - interpolation)), BitmapDescriptorFactory.HUE_RED);
            }
    ...
    private StaticLayout makeStaticLayout(CharSequence charSequence, int i) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            return StaticLayout.Builder.obtain(str, 0, str.length(), this.timestampLabelPaint, i).setMaxLines(1).setAlignment(Layout.Alignment.ALIGN_CENTER).setEllipsize(TextUtils.TruncateAt.END).setEllipsizedWidth(Math.min(AndroidUtilities.m104dp(400), i)).build();
        }
        return new StaticLayout(str, 0, str.length(), this.timestampLabelPaint, i, Layout.Alignment.ALIGN_CENTER, 1.0f, BitmapDescriptorFactory.HUE_RED, false, TextUtils.TruncateAt.END, Math.min(AndroidUtilities.m104dp(400), i));
    }
