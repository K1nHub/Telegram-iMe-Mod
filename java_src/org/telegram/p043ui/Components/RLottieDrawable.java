...
public class RLottieDrawable extends BitmapDrawable implements Animatable, BitmapsCache.Cacheable {
    ...
    public RLottieDrawable(File file, int i, int i2, BitmapsCache.CacheOptions cacheOptions, boolean z, int[] iArr, int i3) {
        ...
        if (this.precache) {
            ...
            if (this.nativePtr == 0) {
                FileLog.m102d("RLottieDrawable nativePtr == 0 " + file.getAbsolutePath() + " remove file");
                file.delete();
            }
    ...
    private void parseLottieMetadata(File file, String str, int[] iArr) {
        ...
        try {
            ...
            iArr[0] = (int) (lottieMetadata.f1930op - lottieMetadata.f1929ip);
            iArr[1] = (int) lottieMetadata.f1928fr;
        } catch (Exception e) {
    ...
    public void drawInternal(Canvas canvas, Paint paint, boolean z, long j, int i) {
        ...
        if (!z) {
            ...
            if (this.applyTransformation) {
                ...
                if (Math.abs(rectF.width() - this.width) >= AndroidUtilities.m104dp(1) || Math.abs(rectF.height() - this.height) >= AndroidUtilities.m104dp(1)) {
                    z2 = true;
                }
                this.needScale = z2;
            }
