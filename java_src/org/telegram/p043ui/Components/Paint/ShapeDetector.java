...
public class ShapeDetector {
    ...
    public static class Point {
        ...
        /* renamed from: x */
        public double f1873x;

        /* renamed from: y */
        public double f1874y;
        ...
        public double distance(double d, double d2) {
            return Math.sqrt(Math.pow(d - this.f1873x, 2.0d) + Math.pow(d2 - this.f1874y, 2.0d));
        }

        public double distance(Point point) {
            return distance(point.f1873x, point.f1874y);
        }
    }
    ...
    public /* synthetic */ void lambda$new$2() {
        ...
        synchronized (this) {
            ...
            translate(fullClone, -centroid.f1873x, -centroid.f1874y);
    ...
    private ArrayList<Point> resample(ArrayList<Point> arrayList, int i) {
        ...
        while (i4 < arrayList.size()) {
            ...
            if (d3 >= pathLength) {
                ...
                i2 = i3;
                d = pathLength;
                Point point = new Point(arrayList.get(i5).f1873x + ((arrayList.get(i4).f1873x - arrayList.get(i5).f1873x) * d4), arrayList.get(i5).f1874y + (d4 * (arrayList.get(i4).f1874y - arrayList.get(i5).f1874y)));
    ...
    private double distanceAtAngle(ArrayList<Point> arrayList, Point point, ArrayList<Point> arrayList2, double d) {
        ...
        while (i < min) {
            ...
            double d3 = point3.f1873x;
            int i2 = i;
            double d4 = point2.f1873x;
            double d5 = point3.f1874y;
            int i3 = min;
            double d6 = point2.f1874y;
            double d7 = cos;
    ...
    private ArrayList<Point> fullClone(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            arrayList2.add(new Point(point.f1873x, point.f1874y));
        }
        return arrayList2;
    }
    ...
    private void rotate(ArrayList<Point> arrayList, double d, Point point) {
        ...
        while (i < arrayList.size()) {
            ...
            double d2 = point2.f1873x;
            double d3 = point.f1873x;
            double d4 = point2.f1874y;
            int i2 = i;
            double d5 = point.f1874y;
            point2.f1874y = ((d2 - d3) * sin) + ((d4 - d5) * cos) + d5;
            point2.f1873x = (((d2 - d3) * cos) - ((d4 - d5) * sin)) + d3;
            i = i2 + 1;
        }
    }
    ...
    private RectD boundingBox(ArrayList<Point> arrayList) {
        ...
        double d = arrayList.get(0).f1873x;
        double d2 = arrayList.get(0).f1874y;
        RectD rectD = new RectD(d, d2, d, d2);
        for (int i = 1; i < arrayList.size(); i++) {
            Point point = arrayList.get(i);
            rectD.union(point.f1873x, point.f1874y);
        }
        return rectD;
    }
    ...
    private Point centroid(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            Point point2 = arrayList.get(i);
            point.f1873x += point2.f1873x;
            point.f1874y += point2.f1874y;
        }
        point.f1873x /= arrayList.size();
        point.f1874y /= arrayList.size();
        return point;
    }

    private double indicativeAngle(ArrayList<Point> arrayList) {
        Point centroid = centroid(arrayList);
        return Math.atan2(centroid.f1874y - arrayList.get(0).f1874y, centroid.f1873x - arrayList.get(0).f1873x);
    }
    ...
    private Shape constructShape(int i, ArrayList<Point> arrayList) {
        ...
        if (i == 4) {
            ...
            shape.centerX = (float) point.f1873x;
            shape.centerY = (float) point.f1874y;
            shape.middleX = (float) point2.f1873x;
            shape.middleY = (float) point2.f1874y;
            shape.radiusX = (float) point3.f1873x;
            shape.radiusY = (float) point3.f1874y;
            shape.arrowTriangleLength = 16.0f;
        } else {
            Point centroid = centroid(arrayList);
            shape.centerX = (float) centroid.f1873x;
            shape.centerY = (float) centroid.f1874y;
            RectD boundingBox = boundingBox(arrayList);
            ...
            if (i == 2 && (findAnglePoint = findAnglePoint(arrayList, 1)) > 0) {
                ...
                shape.rotation = (float) Math.atan2(point4.f1874y - shape.centerY, point4.f1873x - shape.centerX);
            }
        }
        return shape;
    }
    ...
    public /* synthetic */ void lambda$showSaveLearnDialog$3(ArrayList arrayList, DialogInterface dialogInterface, int i) {
        if (i == 0) {
            ...
            for (int i2 = 0; i2 < this.templates.size(); i2++) {
                ...
                for (int i3 = 0; i3 < template.points.size(); i3++) {
                    ...
                    sb.append("[");
                    sb.append(Math.round(point.f1873x));
                    sb.append(",");
                    sb.append(Math.round(point.f1874y));
                    sb.append("]");
                }
                sb.append("],\n\t\t\"freq\": ");
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            str = str + "," + Math.round(((Point) arrayList.get(i4)).f1873x) + "," + Math.round(((Point) arrayList.get(i4)).f1874y);
        }
