...
public class Input {
    ...
    private void fill(Brush brush, final boolean z, final Runnable runnable) {
        ...
        float distance = MathUtils.distance((float) point.f1864x, (float) point.f1865y, (float) BitmapDescriptorFactory.HUE_RED, (float) BitmapDescriptorFactory.HUE_RED);
        Point point2 = this.lastLocation;
        float max = Math.max(distance, MathUtils.distance((float) point2.f1864x, (float) point2.f1865y, size.width, (float) BitmapDescriptorFactory.HUE_RED));
        Point point3 = this.lastLocation;
        float distance2 = MathUtils.distance((float) point3.f1864x, (float) point3.f1865y, (float) BitmapDescriptorFactory.HUE_RED, size.height);
        Point point4 = this.lastLocation;
        final float max2 = Math.max(max, Math.max(distance2, MathUtils.distance((float) point4.f1864x, (float) point4.f1865y, size.width, size.height))) / 0.84f;
        ...
        final Point point6 = new Point(point5.f1864x, point5.f1865y, 1.0d);
    ...
    */
    ...
    public /* synthetic */ void lambda$process$2(float f, Point point, float f2, float[] fArr, double d, boolean[] zArr, ValueAnimator valueAnimator) {
        ...
        paintPath(new Path(new Point[]{new Point(point.f1864x + (fArr[0] * cos), point.f1865y + (fArr[0] * sin), d), new Point(point.f1864x + (cos * d4), point.f1865y + (sin * d4), d, true)}));
        double cos2 = Math.cos(d2 + 2.5918139392115793d);
        double sin2 = Math.sin(d2 + 2.748893571891069d);
        double d5 = cos2 * d3;
        double d6 = d3 * sin2;
        paintPath(new Path(new Point[]{new Point(point.f1864x + (fArr[0] * d5), point.f1865y + (fArr[0] * d6), d), new Point(point.f1864x + (d5 * d4), point.f1865y + (d6 * d4), d, true)}));
    ...
    private void smoothenAndPaintPoints(boolean z, float f) {
        ...
        if (i > 2) {
            ...
            while (i2 < min) {
                ...
                this.thicknessSum += smoothPoint.f1866z;
    ...
    private Point smoothPoint(Point point, Point point2, Point point3, float f, float f2) {
        ...
        double d3 = f3 * f3;
        double d4 = f;
        return new Point((point.f1864x * d3) + (point3.f1864x * 2.0d * d4 * d) + (point2.f1864x * d2), (point.f1865y * d3) + (point3.f1865y * 2.0d * d4 * d) + (point2.f1865y * d2), (((((point.f1866z * Math.pow(d, 2.0d)) + (point3.f1866z * ((2.0f * f3) * f))) + (point2.f1866z * d2)) - 1.0d) * AndroidUtilities.lerp(f2, 1.0f, androidx.core.math.MathUtils.clamp(this.realPointsCount / 16.0f, (float) BitmapDescriptorFactory.HUE_RED, 1.0f))) + 1.0d);
    }
