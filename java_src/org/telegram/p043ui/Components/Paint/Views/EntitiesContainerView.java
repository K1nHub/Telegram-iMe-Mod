...
public class EntitiesContainerView extends FrameLayout {
    ...
    /* renamed from: px */
    private float f1873px;

    /* renamed from: py */
    private float f1874py;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getPointerCount() == 1) {
            ...
            if (actionMasked == 0) {
                ...
                this.f1873px = motionEvent.getX();
                this.f1874py = motionEvent.getY();
                this.cancelled = false;
            } else if (!this.cancelled && actionMasked == 2) {
                float x = motionEvent.getX();
                float y = motionEvent.getY();
                if (this.hasTransformed || MathUtils.distance(x, y, this.f1873px, this.f1874py) > AndroidUtilities.touchSlop) {
                    this.hasTransformed = true;
                    onSelectedEntityRequest.hasPanned = true;
                    onSelectedEntityRequest.pan(x - this.f1873px, y - this.f1874py);
                    this.f1873px = x;
                    this.f1874py = y;
                }
