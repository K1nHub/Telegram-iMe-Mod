...
public class PhotoView extends EntityView {
    ...
    public PhotoView(Context context, Point point, float f, float f2, Size size, String str, int i, int i2) {
        ...
        imageReceiver.setRoundRadius(AndroidUtilities.m104dp(12));
    ...
    protected Rect getSelectionBounds() {
        ...
        float measuredWidth = (getMeasuredWidth() * getScale()) + (AndroidUtilities.m104dp(64) / scaleX);
        float measuredHeight = (getMeasuredHeight() * getScale()) + (AndroidUtilities.m104dp(64) / scaleX);
    ...
    public class PhotoViewSelectionView extends EntityView.SelectionView {
        ...
        protected int pointInsideHandle(float f, float f2) {
            float m105dp = AndroidUtilities.m105dp(19.5f);
            float m105dp2 = AndroidUtilities.m105dp(1.0f) + m105dp;
            float f3 = m105dp2 * 2.0f;
            float measuredWidth = getMeasuredWidth() - f3;
            float measuredHeight = getMeasuredHeight() - f3;
            float f4 = (measuredHeight / 2.0f) + m105dp2;
            if (f <= m105dp2 - m105dp || f2 <= f4 - m105dp || f >= m105dp2 + m105dp || f2 >= f4 + m105dp) {
                float f5 = m105dp2 + measuredWidth;
                if (f <= f5 - m105dp || f2 <= f4 - m105dp || f >= f5 + m105dp || f2 >= f4 + m105dp) {
                    return (f <= m105dp2 || f >= measuredWidth || f2 <= m105dp2 || f2 >= measuredHeight) ? 0 : 3;
                }
                return 2;
            }
            return 1;
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float m105dp = AndroidUtilities.m105dp(2.0f) + dpf2 + AndroidUtilities.m104dp(15);
            float f = m105dp * 2.0f;
            float measuredWidth = getMeasuredWidth() - f;
            float measuredHeight = getMeasuredHeight() - f;
            RectF rectF = AndroidUtilities.rectTmp;
            float f2 = m105dp + measuredWidth;
            float f3 = m105dp + measuredHeight;
            rectF.set(m105dp, m105dp, f2, f3);
            float m104dp = AndroidUtilities.m104dp(12);
            float min = Math.min(m104dp, measuredWidth / 2.0f);
            float f4 = measuredHeight / 2.0f;
            float min2 = Math.min(m104dp, f4);
            this.path.rewind();
            float f5 = min * 2.0f;
            float f6 = m105dp + f5;
            float f7 = 2.0f * min2;
            float f8 = m105dp + f7;
            rectF.set(m105dp, m105dp, f6, f8);
            this.path.arcTo(rectF, 180.0f, 90.0f);
            float f9 = f2 - f5;
            rectF.set(f9, m105dp, f2, f8);
            this.path.arcTo(rectF, 270.0f, 90.0f);
            canvas.drawPath(this.path, this.paint);
            this.path.rewind();
            float f10 = f3 - f7;
            rectF.set(m105dp, f10, f6, f3);
            this.path.arcTo(rectF, 180.0f, -90.0f);
            rectF.set(f9, f10, f2, f3);
            this.path.arcTo(rectF, 90.0f, -90.0f);
            canvas.drawPath(this.path, this.paint);
            float f11 = m105dp + f4;
            canvas.drawCircle(m105dp, f11, dpf2, this.dotStrokePaint);
            canvas.drawCircle(m105dp, f11, (dpf2 - AndroidUtilities.m104dp(1)) + 1.0f, this.dotPaint);
            canvas.drawCircle(f2, f11, dpf2, this.dotStrokePaint);
            canvas.drawCircle(f2, f11, (dpf2 - AndroidUtilities.m104dp(1)) + 1.0f, this.dotPaint);
            canvas.saveLayerAlpha(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight(), 255, 31);
            float f12 = m105dp + min2;
            float f13 = f3 - min2;
            canvas.drawLine(m105dp, f12, m105dp, f13, this.paint);
            canvas.drawLine(f2, f12, f2, f13, this.paint);
            canvas.drawCircle(f2, f11, (AndroidUtilities.m104dp(1) + dpf2) - 1.0f, this.clearPaint);
            canvas.drawCircle(m105dp, f11, (dpf2 + AndroidUtilities.m104dp(1)) - 1.0f, this.clearPaint);
