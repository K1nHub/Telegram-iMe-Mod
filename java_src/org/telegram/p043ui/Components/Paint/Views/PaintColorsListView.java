...
public class PaintColorsListView extends RecyclerListView {
    ...
    public static void drawColorCircle(Canvas canvas, float f, float f2, float f3, int i) {
        ...
        if (colorCirclePaint.getAlpha() != 255) {
            ...
            drawCheckerboard(canvas, rectF, AndroidUtilities.m104dp(4));
            canvas.restore();
            colorCirclePaint.setColor(i);
            canvas.drawArc(rectF, -45.0f, 180.0f, true, colorCirclePaint);
            return;
    ...
    private final class ColorView extends View {
        ...
        public ColorView(Context context) {
            super(context);
            setPadding(AndroidUtilities.m104dp(4), AndroidUtilities.m104dp(4), AndroidUtilities.m104dp(4), AndroidUtilities.m104dp(4));
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (this.selectProgress != BitmapDescriptorFactory.HUE_RED) {
                min -= (AndroidUtilities.m105dp(3.0f) + PaintColorsListView.this.outlinePaint.getStrokeWidth()) * this.selectProgress;
            }
            float width = ((getWidth() / 2.0f) + getPaddingLeft()) - getPaddingRight();
            float height = ((getHeight() / 2.0f) + getPaddingTop()) - getPaddingBottom();
            PaintColorsListView.drawColorCircle(canvas, width, height, min, this.mColor);
            if (this.selectProgress != BitmapDescriptorFactory.HUE_RED) {
                float min2 = (Math.min((getWidth() - getPaddingLeft()) - getPaddingRight(), (getHeight() - getPaddingTop()) - getPaddingBottom()) / 2.0f) - AndroidUtilities.m105dp(2.0f);
