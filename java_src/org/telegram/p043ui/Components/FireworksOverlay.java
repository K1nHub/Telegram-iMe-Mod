...
public class FireworksOverlay extends View {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            byte b = this.type;
            if (b == 0) {
                canvas.drawCircle(this.f1850x, this.f1851y, AndroidUtilities.m104dp((int) this.typeSize), FireworksOverlay.paint[this.colorType]);
            } else if (b == 1) {
                FireworksOverlay.this.rect.set(this.f1850x - AndroidUtilities.m104dp((int) this.typeSize), this.f1851y - AndroidUtilities.m104dp(2), this.f1850x + AndroidUtilities.m104dp((int) this.typeSize), this.f1851y + AndroidUtilities.m104dp(2));
                canvas.save();
                canvas.rotate(this.rotation, FireworksOverlay.this.rect.centerX(), FireworksOverlay.this.rect.centerY());
                canvas.drawRoundRect(FireworksOverlay.this.rect, AndroidUtilities.m104dp(2), AndroidUtilities.m104dp(2), FireworksOverlay.paint[this.colorType]);
                canvas.restore();
            } else if (b == 2) {
                ...
                float f = this.f1850x;
                float f2 = this.f1851y;
                drawable.setBounds(((int) f) - intrinsicWidth, ((int) f2) - intrinsicHeight, ((int) f) + intrinsicWidth, ((int) f2) + intrinsicHeight);
                canvas.save();
                canvas.rotate(this.rotation, this.f1850x, this.f1851y);
                byte b2 = this.typeSize;
                canvas.scale(b2 / 6.0f, b2 / 6.0f, this.f1850x, this.f1851y);
        ...
        public boolean update(int i) {
            float f = i / 16.0f;
            float f2 = this.f1850x;
            float f3 = this.moveX;
            this.f1850x = f2 + (f3 * f);
            this.f1851y += this.moveY * f;
            if (this.xFinished != 0) {
                float m104dp = AndroidUtilities.m104dp(1) * 0.5f;
                if (this.xFinished == 1) {
                    float f4 = this.moveX + (m104dp * f * 0.05f);
                    this.moveX = f4;
                    if (f4 >= m104dp) {
                        this.xFinished = (byte) 2;
                    }
                } else {
                    float f5 = this.moveX - ((m104dp * f) * 0.05f);
                    this.moveX = f5;
                    if (f5 <= (-m104dp)) {
            ...
            float f8 = (-AndroidUtilities.m105dp(1.0f)) / 2.0f;
            float f9 = this.moveY;
            boolean z = f9 < f8;
            if (f9 > f8) {
                this.moveY = f9 + ((AndroidUtilities.m105dp(1.0f) / 3.0f) * f * FireworksOverlay.this.speedCoef);
            } else {
                this.moveY = f9 + ((AndroidUtilities.m105dp(1.0f) / 3.0f) * f);
            }
            if (z && this.moveY > f8) {
            ...
            return this.f1851y >= ((float) FireworksOverlay.this.getHeightForAnimation());
        }
    }
    ...
    private void loadHeartDrawables() {
        ...
        while (true) {
            ...
            drawableArr[i] = ApplicationLoader.applicationContext.getResources().getDrawable(C3634R.C3636drawable.heart_confetti).mutate();
    ...
    private Particle createParticle(boolean z) {
        ...
        if (z) {
            particle.f1851y = (-Utilities.random.nextFloat()) * getHeightForAnimation() * 1.2f;
            particle.f1850x = AndroidUtilities.m104dp(5) + Utilities.random.nextInt(getWidthForAnimation() - AndroidUtilities.m104dp(10));
            particle.xFinished = particle.finishedStart;
        } else {
            int m104dp = AndroidUtilities.m104dp(Utilities.random.nextInt(10) + 4);
            int heightForAnimation = getHeightForAnimation() / 4;
            if (particle.side == 0) {
                particle.f1850x = -m104dp;
            } else {
                particle.f1850x = getWidthForAnimation() + m104dp;
            }
            particle.moveX = (particle.side != 0 ? -1 : 1) * (AndroidUtilities.m105dp(1.2f) + (Utilities.random.nextFloat() * AndroidUtilities.m104dp(4)));
            particle.moveY = -(AndroidUtilities.m104dp(4) + (Utilities.random.nextFloat() * AndroidUtilities.m104dp(4)));
            particle.f1851y = (heightForAnimation / 2) + Utilities.random.nextInt(heightForAnimation * 2);
        }
        return particle;
    }
