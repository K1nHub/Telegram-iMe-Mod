...
public class SeekBarView extends FrameLayout {
    ...
    public SeekBarView(Context context, boolean z, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.selectorWidth = AndroidUtilities.m107dp(32);
        this.thumbSize = AndroidUtilities.m107dp(24);
        this.currentRadius = AndroidUtilities.m107dp(6);
        if (Build.VERSION.SDK_INT >= 21) {
            Drawable createSelectorDrawable = Theme.createSelectorDrawable(ColorUtils.setAlphaComponent(getThemedColor(i), 40), 1, AndroidUtilities.m107dp(16));
    ...
    public boolean onTouch(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            this.f1956sx = motionEvent.getX();
            this.f1957sy = motionEvent.getY();
            return true;
        }
        if (motionEvent.getAction() == 1 || motionEvent.getAction() == 3) {
            this.captured = false;
            if (motionEvent.getAction() == 1) {
                if (Math.abs(motionEvent.getY() - this.f1957sy) < ViewConfiguration.get(getContext()).getScaledTouchSlop()) {
            ...
            if (!this.captured) {
                ...
                if (Math.abs(motionEvent.getY() - this.f1957sy) <= viewConfiguration.getScaledTouchSlop() && Math.abs(motionEvent.getX() - this.f1956sx) > viewConfiguration.getScaledTouchSlop()) {
                    this.captured = true;
    ...
    */
    protected void onDraw(android.graphics.Canvas r16) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p043ui.Components.SeekBarView.onDraw(android.graphics.Canvas):void");
    }
    ...
    private void drawProgressBar(Canvas canvas, RectF rectF, Paint paint) {
        ...
        tmpPath.reset();
        float m107dp2 = AndroidUtilities.m107dp(4) / (measuredWidth - f3);
        int i2 = 0;
        while (true) {
            if (i2 >= this.timestamps.size()) {
                i2 = -1;
                break;
            } else if (((Float) this.timestamps.get(i2).first).floatValue() >= m107dp2) {
                break;
            } else {
                i2++;
            }
        ...
        while (i3 <= i) {
            ...
            while (i3 != i && i3 != 0 && i3 < this.timestamps.size() - 1 && ((Float) this.timestamps.get(i3).first).floatValue() - floatValue <= m107dp2) {
                i3++;
                floatValue2 = ((Float) this.timestamps.get(i3).first).floatValue();
            }
            RectF rectF2 = AndroidUtilities.rectTmp;
            rectF2.left = AndroidUtilities.lerp(f3, measuredWidth, floatValue) + (i3 > 0 ? m108dp : 0.0f);
            ...
            if (f7 < f8) {
                ...
                if (i3 == i2 || (z && rectF2.left >= rectF.left)) {
                    ...
                    fArr[1] = m107dp;
                    fArr[0] = m107dp;
                    float f9 = 0.7f * m107dp * this.timestampsAppearing;
                    fArr[5] = f9;
                    fArr[4] = f9;
                    fArr[3] = f9;
                    c = 2;
                    ...
                    f = f3;
                    float f10 = 0.7f * m107dp * this.timestampsAppearing;
                    fArr2[7] = f10;
                    fArr2[6] = f10;
                    fArr2[1] = f10;
                    fArr2[0] = f10;
                    ...
                    float f11 = this.timestampsAppearing * 0.7f * m107dp;
                    fArr3[5] = f11;
                    fArr3[4] = f11;
                    fArr3[3] = f11;
                    fArr3[2] = f11;
    ...
    private void drawTimestampLabel(Canvas canvas) {
        ...
        float m107dp = (this.selectorWidth / 2.0f) + (this.lastDuration > 600000 ? AndroidUtilities.m107dp(42) : 0);
        float abs = Math.abs(m107dp - ((getMeasuredWidth() - (this.selectorWidth / 2.0f)) - (this.lastDuration > 600000 ? AndroidUtilities.m107dp(42) : 0))) - AndroidUtilities.m107dp(66);
        float f = this.lastWidth;
        if (f > BitmapDescriptorFactory.HUE_RED && Math.abs(f - abs) > 0.01f) {
            StaticLayout[] staticLayoutArr = this.timestampLabel;
        ...
        canvas.save();
        canvas.translate(m107dp + AndroidUtilities.m107dp(25), (getMeasuredHeight() / 2.0f) + AndroidUtilities.m107dp(14));
