...
public class SnowflakesEffect {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            if (this.type == 0) {
                ...
                canvas.drawPoint(this.f1968x, this.f1969y, SnowflakesEffect.this.particlePaint);
                return;
            }
            float f = -1.5707964f;
            ...
            if (snowflakesEffect.particleBitmap == null) {
                ...
                SnowflakesEffect.this.particleBitmap = Bitmap.createBitmap(AndroidUtilities.m107dp(16), AndroidUtilities.m107dp(16), Bitmap.Config.ARGB_8888);
                ...
                while (i < 6) {
                    ...
                    float f3 = cos * 0.66f;
                    canvas2.drawLine(m107dp, m107dp2, m107dp + cos, m107dp2 + sin, SnowflakesEffect.this.particleThinPaint);
                    double d2 = (float) (d - 1.5707963267948966d);
                    double d3 = f2;
                    Canvas canvas3 = canvas2;
                    double d4 = dpf22;
                    float f4 = m107dp + f3;
                    float f5 = m107dp2 + (sin * 0.66f);
                    canvas3.drawLine(f4, f5, m107dp + ((float) ((Math.cos(d2) * d3) - (Math.sin(d2) * d4))), m107dp2 + ((float) ((Math.sin(d2) * d3) + (Math.cos(d2) * d4))), SnowflakesEffect.this.particleThinPaint);
                    canvas3.drawLine(f4, f5, m107dp + ((float) (((-Math.cos(d2)) * d3) - (Math.sin(d2) * d4))), m107dp2 + ((float) (((-Math.sin(d2)) * d3) + (Math.cos(d2) * d4))), SnowflakesEffect.this.particleThinPaint);
                    f += 1.0471976f;
                    i++;
                    canvas2 = canvas3;
            ...
            canvas.save();
            float f6 = this.scale;
            canvas.scale(f6, f6, this.f1968x, this.f1969y);
            SnowflakesEffect snowflakesEffect2 = SnowflakesEffect.this;
            canvas.drawBitmap(snowflakesEffect2.particleBitmap, this.f1968x, this.f1969y, snowflakesEffect2.bitmapPaint);
            canvas.restore();
        }
    }
    ...
    public SnowflakesEffect(int i) {
        ...
        paint2.setStrokeWidth(AndroidUtilities.m108dp(0.5f));
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1968x;
                float f4 = particle.f1966vx;
                float f5 = particle.velocity;
                float f6 = (float) j;
                particle.f1968x = f3 + (((f4 * f5) * f6) / 500.0f);
                particle.f1969y += ((particle.f1967vy * f5) * f6) / 500.0f;
        ...
        if (this.particles.size() < i3) {
            for (int i5 = 0; i5 < i4; i5++) {
                if (this.particles.size() < i3 && Utilities.random.nextFloat() > 0.7f) {
                    ...
                    float nextFloat2 = i6 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m107dp(20)) - i6));
                    ...
                    particle.f1968x = nextFloat;
                    particle.f1969y = nextFloat2;
                    particle.f1966vx = cos;
                    particle.f1967vy = sin;
