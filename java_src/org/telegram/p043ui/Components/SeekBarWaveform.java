...
public class SeekBarWaveform {
    ...
    private void drawFill(Canvas canvas, float f) {
        ...
        if (f2 > BitmapDescriptorFactory.HUE_RED) {
            if (this.loadingPaint == null || Math.abs(this.loadingPaintWidth - this.width) > AndroidUtilities.m72dp(8) || this.loadingPaintColor1 != this.innerColor || this.loadingPaintColor2 != this.outerColor) {
    ...
    private void addBar(Path path, float f, float f2) {
        float dpf2 = AndroidUtilities.dpf2(2.0f);
        int m72dp = (this.height - AndroidUtilities.m72dp(14)) / 2;
        float f3 = f2 * this.waveScaling;
        RectF rectF = AndroidUtilities.rectTmp;
        float f4 = dpf2 / 2.0f;
        rectF.set((AndroidUtilities.dpf2(1.0f) + f) - f4, AndroidUtilities.m72dp(7) + m72dp + ((-f3) - f4), f + AndroidUtilities.dpf2(1.0f) + f4, m72dp + AndroidUtilities.m72dp(7) + f3 + f4);
