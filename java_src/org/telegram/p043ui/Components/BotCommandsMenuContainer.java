...
public class BotCommandsMenuContainer extends FrameLayout implements NestedScrollingParent {
    ...
    public BotCommandsMenuContainer(Context context) {
        ...
        RecyclerListView recyclerListView = new RecyclerListView(context) { // from class: org.telegram.ui.Components.BotCommandsMenuContainer.1
            ...
            public void dispatchDraw(Canvas canvas) {
                ...
                float m104dp = y - AndroidUtilities.m104dp(8);
                if (m104dp > BitmapDescriptorFactory.HUE_RED) {
                    int i = (int) m104dp;
                    BotCommandsMenuContainer.this.shadowDrawable.setBounds(-AndroidUtilities.m104dp(8), i - AndroidUtilities.m104dp(24), getMeasuredWidth() + AndroidUtilities.m104dp(8), i);
                    BotCommandsMenuContainer.this.shadowDrawable.draw(canvas);
                }
                canvas.drawRect(BitmapDescriptorFactory.HUE_RED, m104dp, getMeasuredWidth(), getMeasuredHeight() + AndroidUtilities.m104dp(16), BotCommandsMenuContainer.this.backgroundPaint);
                RectF rectF = AndroidUtilities.rectTmp;
                rectF.set((getMeasuredWidth() / 2.0f) - AndroidUtilities.m104dp(12), m104dp - AndroidUtilities.m104dp(4), (getMeasuredWidth() / 2.0f) + AndroidUtilities.m104dp(12), m104dp);
                canvas.drawRoundRect(rectF, AndroidUtilities.m104dp(4), AndroidUtilities.m104dp(4), BotCommandsMenuContainer.this.topBackground);
                super.dispatchDraw(canvas);
            }
        };
    ...
    private void checkDismiss() {
        ...
        if (this.listView.getTranslationY() > AndroidUtilities.m104dp(16)) {
            dismiss();
        } else {
            playEnterAnim(false);
        }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        recyclerListView.setTranslationY((recyclerListView.getMeasuredHeight() - this.listView.getPaddingTop()) + AndroidUtilities.m104dp(16));
        playEnterAnim(true);
        this.entering = false;
    }
    ...
    public void dismiss() {
        ...
        cancelCurrentAnimation();
        RecyclerListView recyclerListView = this.listView;
        ObjectAnimator ofFloat = ObjectAnimator.ofFloat(recyclerListView, FrameLayout.TRANSLATION_Y, recyclerListView.getTranslationY(), (getMeasuredHeight() - this.scrollYOffset) + AndroidUtilities.m104dp(40));
        this.currentAnimation = ofFloat;
    ...
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        if (motionEvent.getAction() != 0 || motionEvent.getY() >= this.scrollYOffset - AndroidUtilities.m104dp(24)) {
