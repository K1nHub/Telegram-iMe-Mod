...
public class BotCommandsMenuContainer extends FrameLayout implements NestedScrollingParent {
    ...
    public BotCommandsMenuContainer(Context context) {
        ...
        RecyclerListView recyclerListView = new RecyclerListView(context) { // from class: org.telegram.ui.Components.BotCommandsMenuContainer.1
            ...
            public void dispatchDraw(Canvas canvas) {
                ...
                float m107dp = y - AndroidUtilities.m107dp(8);
                if (m107dp > BitmapDescriptorFactory.HUE_RED) {
                    int i = (int) m107dp;
                    BotCommandsMenuContainer.this.shadowDrawable.setBounds(-AndroidUtilities.m107dp(8), i - AndroidUtilities.m107dp(24), getMeasuredWidth() + AndroidUtilities.m107dp(8), i);
                    BotCommandsMenuContainer.this.shadowDrawable.draw(canvas);
                }
                canvas.drawRect(BitmapDescriptorFactory.HUE_RED, m107dp, getMeasuredWidth(), getMeasuredHeight() + AndroidUtilities.m107dp(16), BotCommandsMenuContainer.this.backgroundPaint);
                RectF rectF = AndroidUtilities.rectTmp;
                rectF.set((getMeasuredWidth() / 2.0f) - AndroidUtilities.m107dp(12), m107dp - AndroidUtilities.m107dp(4), (getMeasuredWidth() / 2.0f) + AndroidUtilities.m107dp(12), m107dp);
                canvas.drawRoundRect(rectF, AndroidUtilities.m107dp(4), AndroidUtilities.m107dp(4), BotCommandsMenuContainer.this.topBackground);
                super.dispatchDraw(canvas);
            }
        };
    ...
    private void checkDismiss() {
        ...
        if (this.listView.getTranslationY() > AndroidUtilities.m107dp(16)) {
            dismiss();
        } else {
            playEnterAnim(false);
        }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        recyclerListView.setTranslationY((recyclerListView.getMeasuredHeight() - this.listView.getPaddingTop()) + AndroidUtilities.m107dp(16));
        playEnterAnim(true);
        this.entering = false;
    }
    ...
    public void dismiss() {
        ...
        cancelCurrentAnimation();
        RecyclerListView recyclerListView = this.listView;
        ObjectAnimator ofFloat = ObjectAnimator.ofFloat(recyclerListView, FrameLayout.TRANSLATION_Y, recyclerListView.getTranslationY(), (getMeasuredHeight() - this.scrollYOffset) + AndroidUtilities.m107dp(40));
        this.currentAnimation = ofFloat;
    ...
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        if (motionEvent.getAction() != 0 || motionEvent.getY() >= this.scrollYOffset - AndroidUtilities.m107dp(24)) {
