...
/* renamed from: org.telegram.ui.Components.AvatarConstructorPreviewCell */
...
public class AvatarConstructorPreviewCell extends FrameLayout {
    ...
    public AvatarConstructorPreviewCell(Context context, boolean z) {
        ...
        this.scheduleSwitchToNextRunnable = new Runnable() { // from class: org.telegram.ui.Components.AvatarConstructorPreviewCell.1
            ...
            public void run() {
                ...
                avatarConstructorPreviewCell6.progressToNext = BitmapDescriptorFactory.HUE_RED;
            ...
            if (stickerSets.isEmpty()) {
                ...
                for (int i2 = 0; i2 < featuredEmojiSets.size(); i2++) {
                    ...
                    if (tLRPC$Document != null) {
                        this.emojiList.document_id.add(Long.valueOf(tLRPC$Document.f1526id));
                        ...
                        if (!tLRPC$TL_stickerSetFullCovered.documents.isEmpty()) {
                            this.emojiList.document_id.add(Long.valueOf(tLRPC$TL_stickerSetFullCovered.documents.get(0).f1526id));
                        }
                    }
                }
            } else {
                for (int i3 = 0; i3 < stickerSets.size(); i3++) {
                    ...
                    if (!tLRPC$TL_messages_stickerSet.documents.isEmpty()) {
                        this.emojiList.document_id.add(Long.valueOf(tLRPC$TL_messages_stickerSet.documents.get(Math.abs(Utilities.fastRandom.nextInt() % tLRPC$TL_messages_stickerSet.documents.size())).f1526id));
                    }
                }
            }
        }
        ...
        this.textView.setText(LocaleController.getString("UseEmoji", C3419R.string.UseEmoji));
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (gradientTools != null) {
            gradientTools.setBounds(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getMeasuredWidth(), getMeasuredHeight());
        }
        GradientTools gradientTools2 = this.nextBackgroundDrawable;
        if (gradientTools2 != null) {
            gradientTools2.setBounds(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getMeasuredWidth(), getMeasuredHeight());
        }
        float f = this.progressToNext;
        if (f == 1.0f) {
            this.currentBackgroundDrawable.paint.setAlpha(255);
            canvas.drawRect(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getMeasuredWidth(), getMeasuredHeight(), this.currentBackgroundDrawable.paint);
            this.currentImage.setAlpha(1.0f);
            this.currentImage.setScaleX(1.0f);
            this.currentImage.setScaleY(1.0f);
            this.nextImage.setAlpha(BitmapDescriptorFactory.HUE_RED);
        } else {
            ...
            canvas.drawRect(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getMeasuredWidth(), getMeasuredHeight(), this.nextBackgroundDrawable.paint);
            this.progressToNext += 0.064f;
            float f2 = 1.0f - interpolation;
            this.currentImage.setAlpha(f2);
            this.currentImage.setScaleX(f2);
            this.currentImage.setScaleY(f2);
            this.currentImage.setPivotY(BitmapDescriptorFactory.HUE_RED);
