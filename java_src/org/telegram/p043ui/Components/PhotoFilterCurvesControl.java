...
/* renamed from: org.telegram.ui.Components.PhotoFilterCurvesControl */
...
public class PhotoFilterCurvesControl extends View {
    ...
    /* renamed from: org.telegram.ui.Components.PhotoFilterCurvesControl$PhotoFilterCurvesControlDelegate */
    /* loaded from: classes6.dex */
    ...
    public PhotoFilterCurvesControl(Context context, PhotoFilterView.CurvesToolValue curvesToolValue) {
        ...
        this.paint.setStrokeWidth(AndroidUtilities.m104dp(1));
        this.paint.setStyle(Paint.Style.STROKE);
        this.paintDash.setColor(-1711276033);
        this.paintDash.setStrokeWidth(AndroidUtilities.m104dp(2));
        this.paintDash.setStyle(Paint.Style.STROKE);
        this.paintCurve.setColor(-1);
        this.paintCurve.setStrokeWidth(AndroidUtilities.m104dp(2));
    ...
    */
    public boolean onTouchEvent(android.view.MotionEvent r8) {
        ...
        L76:
            return r3
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p043ui.Components.PhotoFilterCurvesControl.onTouchEvent(android.view.MotionEvent):boolean");
    }
    ...
    private void handlePan(int i, MotionEvent motionEvent) {
        ...
        if (i == 1) {
            ...
            if (i3 == 1) {
                curvesValue.blacksLevel = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.blacksLevel + min));
            } else if (i3 == 2) {
                curvesValue.shadowsLevel = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.shadowsLevel + min));
            } else if (i3 == 3) {
                curvesValue.midtonesLevel = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.midtonesLevel + min));
            } else if (i3 == 4) {
                curvesValue.highlightsLevel = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.highlightsLevel + min));
            } else if (i3 == 5) {
                curvesValue.whitesLevel = Math.max((float) BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.whitesLevel + min));
            }
            invalidate();
    ...
    private void selectSegmentWithPoint(float f) {
        ...
        Rect rect = this.actualArea;
        this.activeSegment = (int) Math.floor(((f - rect.f1935x) / (rect.width / 5.0f)) + 1.0f);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (i3 < 5) {
            ...
            canvas.drawText(format, rect3.f1935x + ((f - measureText) / 2.0f) + (i3 * f), (rect3.f1936y + rect3.height) - AndroidUtilities.m104dp(4), this.textPaint);
            i3++;
        }
        ...
        for (int i4 = 0; i4 < interpolateCurve.length / 2; i4++) {
            if (i4 == 0) {
                ...
                int i5 = i4 * 2;
                path.moveTo(rect4.f1935x + (interpolateCurve[i5] * rect4.width), rect4.f1936y + ((1.0f - interpolateCurve[i5 + 1]) * rect4.height));
            } else {
                Path path2 = this.path;
                Rect rect5 = this.actualArea;
                int i6 = i4 * 2;
                path2.lineTo(rect5.f1935x + (interpolateCurve[i6] * rect5.width), rect5.f1936y + ((1.0f - interpolateCurve[i6 + 1]) * rect5.height));
            }
        }
