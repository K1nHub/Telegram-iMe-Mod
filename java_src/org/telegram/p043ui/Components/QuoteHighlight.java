...
public class QuoteHighlight extends Path {
    ...
    /* renamed from: t */
    private final AnimatedFloat f1927t;
    ...
    public QuoteHighlight(final View view, final ViewParent viewParent, int i, ArrayList<MessageObject.TextLayoutBlock> arrayList, int i2, int i3, float f) {
        ...
        this.f1927t = new AnimatedFloat((float) BitmapDescriptorFactory.HUE_RED, new Runnable() { // from class: org.telegram.ui.Components.QuoteHighlight$$ExternalSyntheticLambda0
        ...
        paint.setPathEffect(new CornerPathEffect(AndroidUtilities.m104dp(4)));
        boolean z = false;
        ...
        if (this.rectangles.size() > 0) {
            ...
            rect.first = true;
            rect.top -= AndroidUtilities.m105dp(0.66f);
            rect2.last = true;
            rect2.bottom += AndroidUtilities.m105dp(0.66f);
        }
    }
    ...
    public void addRect(float f, float f2, float f3, float f4) {
        ...
        float f9 = max2 + f5;
        Rect rect = new Rect();
        rect.left = f6 - AndroidUtilities.m104dp(3);
        rect.right = f9 + AndroidUtilities.m104dp(3);
        rect.top = f8;
        rect.bottom = f4 + f7;
