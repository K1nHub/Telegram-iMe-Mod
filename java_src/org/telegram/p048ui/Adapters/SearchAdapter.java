...
public class SearchAdapter extends RecyclerListView.SelectionAdapter {
    ...
    public /* synthetic */ void lambda$processSearch$0(String str, int i, ArrayList arrayList, int i2) {
        ...
        while (i5 < arrayList.size()) {
            ...
            if ((this.allowSelf || !user.self) && ((!this.onlyMutual || user.mutual_contact) && ((longSparseArray = this.ignoreUsers) == null || longSparseArray.indexOfKey(tLRPC$TL_contact.user_id) < 0))) {
                ...
                if (UserObject.isReplyUser(user)) {
                    strArr4[2] = LocaleController.getString("RepliesTitle", C3158R.string.RepliesTitle).toLowerCase();
                } else if (user.self) {
                    strArr4[2] = LocaleController.getString("SavedMessages", C3158R.string.SavedMessages).toLowerCase();
                }
                int i7 = i6;
                int i8 = 0;
                char c = 0;
        ...
        if (this.allUnregistredContacts == null) {
            ...
            while (it.hasNext()) {
                ...
                contactEntry.f1651q1 = (next.first_name + " " + next.last_name).toLowerCase();
        ...
        for (int i10 = 0; i10 < this.allUnregistredContacts.size(); i10++) {
            ...
            if ((translitString != null && (contactEntry2.f1651q1.toLowerCase().contains(translitString) || contactEntry2.f1651q1.toLowerCase().contains(translitString))) || contactEntry2.f1651q1.toLowerCase().contains(lowerCase) || contactEntry2.f1651q1.toLowerCase().contains(lowerCase)) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int i) {
        ...
        if (itemViewType != 0) {
            if (itemViewType == 1) {
                ...
                if (i == this.unregistredContactsHeaderRow) {
                    graySectionCell.setText(LocaleController.getString("InviteToTelegramShort", C3158R.string.InviteToTelegramShort));
                    return;
                } else if (getItem(i) == null) {
                    graySectionCell.setText(LocaleController.getString("GlobalSearch", C3158R.string.GlobalSearch));
                    return;
                } else {
                    graySectionCell.setText(LocaleController.getString("PhoneNumberSearch", C3158R.string.PhoneNumberSearch));
                    return;
                }
            } else if (itemViewType == 2) {
                TextCell textCell = (TextCell) viewHolder.itemView;
                textCell.setColors(null, "windowBackgroundWhiteBlueText2");
                textCell.setText(LocaleController.formatString("AddContactByPhone", C3158R.string.AddContactByPhone, C3078PhoneFormat.getInstance().format("+" + ((String) getItem(i)))), false);
                return;
                ...
                profileSearchCell.setData(contact, null, ContactsController.formatName(contact.first_name, contact.last_name), C3078PhoneFormat.getInstance().format("+" + contact.shortPhones.get(0)), false, false);
                return;
            }
        }
        ...
        if (tLObject != null) {
            ...
            profileSearchCell2.setData(tLObject, null, z ? LocaleController.getString("SavedMessages", C3158R.string.SavedMessages) : str2, spannableStringBuilder, false, z);
