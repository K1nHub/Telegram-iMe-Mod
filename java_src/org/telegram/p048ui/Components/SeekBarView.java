...
public class SeekBarView extends FrameLayout {
    ...
    public SeekBarView(Context context, boolean z, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.selectorWidth = AndroidUtilities.m50dp(32);
        this.thumbSize = AndroidUtilities.m50dp(24);
        this.currentRadius = AndroidUtilities.m50dp(6);
        if (Build.VERSION.SDK_INT >= 21) {
            Drawable createSelectorDrawable = Theme.createSelectorDrawable(ColorUtils.setAlphaComponent(getThemedColor("player_progress"), 40), 1, AndroidUtilities.m50dp(16));
    ...
    public boolean onTouch(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            this.f1811sx = motionEvent.getX();
            this.f1812sy = motionEvent.getY();
            return true;
        }
        if (motionEvent.getAction() == 1 || motionEvent.getAction() == 3) {
            this.captured = false;
            if (motionEvent.getAction() == 1) {
                if (Math.abs(motionEvent.getY() - this.f1812sy) < ViewConfiguration.get(getContext()).getScaledTouchSlop()) {
            ...
            if (!this.captured) {
                ...
                if (Math.abs(motionEvent.getY() - this.f1812sy) <= viewConfiguration.getScaledTouchSlop() && Math.abs(motionEvent.getX() - this.f1811sx) > viewConfiguration.getScaledTouchSlop()) {
                    this.captured = true;
    ...
    */
    ...
    private void drawProgressBar(Canvas canvas, RectF rectF, Paint paint) {
        ...
        if (arrayList == null || arrayList.isEmpty()) {
            canvas.drawRoundRect(rectF, m50dp, m50dp, paint);
            return;
        }
        ...
        tmpPath.reset();
        float m50dp2 = AndroidUtilities.m50dp(4) / (measuredWidth - f3);
        int i2 = 0;
        while (true) {
            if (i2 >= this.timestamps.size()) {
                i2 = -1;
                break;
            } else if (((Float) this.timestamps.get(i2).first).floatValue() >= m50dp2) {
                break;
            } else {
                i2++;
            }
        ...
        while (i3 <= i) {
            ...
            if (f7 < f8) {
                ...
                if (i3 == i2 || (z && rectF2.left >= rectF.left)) {
                    ...
                    fArr[1] = m50dp;
                    fArr[0] = m50dp;
                    float f9 = 0.7f * m50dp * this.timestampsAppearing;
                    fArr[5] = f9;
                    fArr[4] = f9;
                    fArr[3] = f9;
                    c = 2;
                    ...
                    f = f3;
                    float f10 = 0.7f * m50dp * this.timestampsAppearing;
                    fArr2[7] = f10;
                    fArr2[6] = f10;
                    fArr2[1] = f10;
                    fArr2[0] = f10;
                    ...
                    float f11 = this.timestampsAppearing * 0.7f * m50dp;
                    fArr3[5] = f11;
                    fArr3[4] = f11;
                    fArr3[3] = f11;
                    fArr3[2] = f11;
