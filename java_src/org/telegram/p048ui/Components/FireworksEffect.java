...
public class FireworksEffect {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            ...
            canvas.drawPoint(this.f1708x, this.f1709y, FireworksEffect.this.particlePaint);
        }
    }
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1708x;
                float f4 = particle.f1706vx;
                float f5 = particle.velocity;
                float f6 = (float) j;
                particle.f1708x = f3 + (((f4 * f5) * f6) / 500.0f);
                float f7 = particle.f1709y;
                float f8 = particle.f1707vy;
                particle.f1709y = f7 + (((f5 * f8) * f6) / 500.0f);
                particle.f1707vy = f8 + (f6 / 100.0f);
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (Utilities.random.nextBoolean() && this.particles.size() + 8 < 150) {
            ...
            for (int i4 = 0; i4 < 8; i4++) {
                ...
                particle.f1708x = nextFloat;
                particle.f1709y = nextFloat2;
                particle.f1706vx = cos * 1.5f;
                particle.f1707vy = sin;
                particle.color = i3;
                particle.alpha = 1.0f;
