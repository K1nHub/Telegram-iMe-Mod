...
public class VoIPTimerView extends View {
    ...
    public VoIPTimerView(Context context) {
        ...
        this.textPaint.setShadowLayer(AndroidUtilities.m50dp(3), BitmapDescriptorFactory.HUE_RED, AndroidUtilities.m51dp(0.6666667f), 1275068416);
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (staticLayout != null) {
            ...
            setMeasuredDimension(View.MeasureSpec.getSize(i), AndroidUtilities.m50dp(15));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        StaticLayout staticLayout = this.timerLayout;
        int i = 0;
        int width = staticLayout == null ? 0 : staticLayout.getWidth() + AndroidUtilities.m50dp(21);
        canvas.save();
        canvas.translate((getMeasuredWidth() - width) / 2.0f, BitmapDescriptorFactory.HUE_RED);
        canvas.save();
        canvas.translate(BitmapDescriptorFactory.HUE_RED, (getMeasuredHeight() - AndroidUtilities.m50dp(11)) / 2.0f);
        while (i < 4) {
            int i2 = i + 1;
            Paint paint = i2 > this.signalBarCount ? this.inactivePaint : this.activePaint;
            float f = i;
            this.rectF.set(AndroidUtilities.dpf2(4.16f) * f, AndroidUtilities.dpf2(2.75f) * (3 - i), (AndroidUtilities.dpf2(4.16f) * f) + AndroidUtilities.dpf2(2.75f), AndroidUtilities.m50dp(11));
            canvas.drawRoundRect(this.rectF, AndroidUtilities.dpf2(0.7f), AndroidUtilities.dpf2(0.7f), paint);
            i = i2;
        }
        canvas.restore();
        if (staticLayout != null) {
            canvas.translate(AndroidUtilities.m50dp(21), BitmapDescriptorFactory.HUE_RED);
