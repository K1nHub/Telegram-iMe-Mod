...
public class PhotoFilterCurvesControl extends View {
    ...
    public PhotoFilterCurvesControl(Context context, PhotoFilterView.CurvesToolValue curvesToolValue) {
        ...
        this.paint.setStrokeWidth(AndroidUtilities.m54dp(1));
        this.paint.setStyle(Paint.Style.STROKE);
        this.paintDash.setColor(-1711276033);
        this.paintDash.setStrokeWidth(AndroidUtilities.m54dp(2));
        this.paintDash.setStyle(Paint.Style.STROKE);
        this.paintCurve.setColor(-1);
        this.paintCurve.setStrokeWidth(AndroidUtilities.m54dp(2));
    ...
    */
    public boolean onTouchEvent(android.view.MotionEvent r8) {
        ...
        L76:
            return r3
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p044ui.Components.PhotoFilterCurvesControl.onTouchEvent(android.view.MotionEvent):boolean");
    }
    ...
    private void selectSegmentWithPoint(float f) {
        ...
        Rect rect = this.actualArea;
        this.activeSegment = (int) Math.floor(((f - rect.f1734x) / (rect.width / 5.0f)) + 1.0f);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (i3 < 5) {
            ...
            canvas.drawText(format, rect3.f1734x + ((f - measureText) / 2.0f) + (i3 * f), (rect3.f1735y + rect3.height) - AndroidUtilities.m54dp(4), this.textPaint);
            i3++;
        }
        ...
        for (int i4 = 0; i4 < interpolateCurve.length / 2; i4++) {
            if (i4 == 0) {
                ...
                int i5 = i4 * 2;
                path.moveTo(rect4.f1734x + (interpolateCurve[i5] * rect4.width), rect4.f1735y + ((1.0f - interpolateCurve[i5 + 1]) * rect4.height));
            } else {
                Path path2 = this.path;
                Rect rect5 = this.actualArea;
                int i6 = i4 * 2;
                path2.lineTo(rect5.f1734x + (interpolateCurve[i6] * rect5.width), rect5.f1735y + ((1.0f - interpolateCurve[i6 + 1]) * rect5.height));
            }
        }
