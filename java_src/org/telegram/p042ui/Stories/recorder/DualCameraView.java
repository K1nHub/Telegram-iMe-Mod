...
/* renamed from: org.telegram.ui.Stories.recorder.DualCameraView */
...
public class DualCameraView extends CameraView implements CameraController.ErrorCallback {
    ...
    /* renamed from: cx */
    private float f1975cx;

    /* renamed from: cy */
    private float f1976cy;
    ...
    private boolean checkTap(MotionEvent motionEvent) {
        if (motionEvent.getAction() == 0) {
            ...
            if (System.currentTimeMillis() - this.tapTime <= ViewConfiguration.getTapTimeout() && MathUtils.distance(this.tapX, this.tapY, motionEvent.getX(), motionEvent.getY()) < AndroidUtilities.m102dp(10)) {
                if (isAtDual(this.tapX, this.tapY)) {
                    switchCamera();
                    this.lastFocusToPoint = null;
                } else {
    ...
    private boolean touchEvent(MotionEvent motionEvent) {
        ...
        if (isDual()) {
            ...
            if (motionEvent.getAction() == 2 && this.down) {
                if (MathUtils.distance(f3, f4, f5, f6) > AndroidUtilities.m102dp(2) && (runnable = this.longpressRunnable) != null) {
                    AndroidUtilities.cancelRunOnUIThread(runnable);
                    this.longpressRunnable = null;
                }
                if (motionEvent.getPointerCount() > 1) {
                    if (this.lastTouchDistance != BitmapDescriptorFactory.HUE_RED) {
                        ...
                        if (this.f1978w * f7 > getWidth() * 0.7f) {
                            width = getWidth() * 0.7f;
                            f2 = this.f1978w;
                        } else {
                            if (this.f1978w * f7 < getWidth() * 0.2f) {
                                width = getWidth() * 0.2f;
                                f2 = this.f1978w;
                            }
                ...
                if (this.allowRotation && !this.doNotSpanRotation) {
                    if (Math.abs(round) < 5.0f) {
                        this.finalMatrix.postRotate(round, this.f1975cx, this.f1976cy);
                        if (!this.snappedRotation) {
                            try {
                                performHapticFeedback(9, 1);
                ...
                float f9 = this.f1975cx;
                if (f9 < BitmapDescriptorFactory.HUE_RED) {
                    this.finalMatrix.postTranslate(-f9, BitmapDescriptorFactory.HUE_RED);
                } else if (f9 > getWidth()) {
                    this.finalMatrix.postTranslate(getWidth() - this.f1975cx, BitmapDescriptorFactory.HUE_RED);
                }
                float f10 = this.f1976cy;
                if (f10 < BitmapDescriptorFactory.HUE_RED) {
                    this.finalMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, -f10);
                } else if (f10 > getHeight() - AndroidUtilities.m102dp((int) ImageReceiver.DEFAULT_CROSSFADE_DURATION)) {
                    this.finalMatrix.postTranslate(BitmapDescriptorFactory.HUE_RED, (getHeight() - AndroidUtilities.m102dp((int) ImageReceiver.DEFAULT_CROSSFADE_DURATION)) - this.f1976cy);
                }
                this.finalMatrix.postConcat(this.toGL);
                dualPosition.set(this.finalMatrix);
                updateDualPosition();
                float f11 = this.f1976cy;
                boolean z5 = Math.min(f11, f11 - (this.f1977h / 2.0f)) < ((float) AndroidUtilities.m102dp(66));
                float f12 = this.f1976cy;
                boolean z6 = Math.max(f12, (this.f1977h / 2.0f) + f12) > ((float) (getHeight() - AndroidUtilities.m102dp(66)));
                if (this.atTop != z5) {
                    this.atTop = z5;
    ...
    private void extractPointsData(Matrix matrix) {
        ...
        this.f1975cx = fArr2[0];
        this.f1976cy = fArr2[1];
        fArr2[0] = 1.0f;
        fArr2[1] = 0.0f;
        matrix.mapPoints(fArr2);
        float[] fArr3 = this.vertices;
        this.angle = (float) Math.toDegrees(Math.atan2(fArr3[1] - this.f1976cy, fArr3[0] - this.f1975cx));
        float f = this.f1975cx;
        float f2 = this.f1976cy;
        float[] fArr4 = this.vertices;
        this.f1978w = MathUtils.distance(f, f2, fArr4[0], fArr4[1]) * 2.0f;
        float[] fArr5 = this.vertices;
        fArr5[0] = 0.0f;
        fArr5[1] = 1.0f;
        matrix.mapPoints(fArr5);
        float f3 = this.f1975cx;
        float f4 = this.f1976cy;
        float[] fArr6 = this.vertices;
        this.f1977h = MathUtils.distance(f3, f4, fArr6[0], fArr6[1]) * 2.0f;
    }
    ...
    public void onError(int i, Camera camera, CameraSession cameraSession) {
        if (isDual()) {
            if (!dualAvailableDefault(getContext(), false)) {
                ...
                new AlertDialog.Builder(getContext()).setTitle(LocaleController.getString(C3630R.string.DualErrorTitle)).setMessage(LocaleController.getString(C3630R.string.DualErrorMessage)).setPositiveButton(LocaleController.getString(C3630R.string.OK), null).show();
            }
            log(false);
            toggleDual();
        }
