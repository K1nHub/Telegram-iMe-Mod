...
public class StoryContainsEmojiButton extends View {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        int m102dp = AndroidUtilities.m102dp(29);
        StaticLayout staticLayout = this.layout;
        setMeasuredDimension(z ? View.MeasureSpec.getSize(i) : getMinimumWidth(), paddingTop + AndroidUtilities.lerp(m102dp, staticLayout == null ? AndroidUtilities.m102dp(29) : staticLayout.getHeight(), this.loadT) + getPaddingBottom());
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f < 1.0f) {
            ...
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop(), getMeasuredWidth() - getPaddingRight(), getPaddingTop() + AndroidUtilities.m102dp(12), Path.Direction.CW);
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop() + AndroidUtilities.m102dp(16), getPaddingLeft() + (((getMeasuredWidth() - getPaddingRight()) - getPaddingLeft()) * 0.46f), getPaddingTop() + AndroidUtilities.m102dp(28), Path.Direction.CW);
    ...
    public /* synthetic */ void lambda$load$1(TLObject tLObject, Object obj, ArrayList arrayList, boolean[] zArr, int i) {
        ...
        if (this.inputSets != null && arrayList != null && !arrayList.isEmpty()) {
            for (int i3 = 0; i3 < arrayList.size(); i3++) {
                ...
                long j = tLRPC$InputStickerSet.f1623id;
                int i4 = 0;
                while (true) {
                    if (i4 >= this.inputSets.size()) {
                        z = false;
                        break;
                    } else if (this.inputSets.get(i4).f1623id == j) {
                        z = true;
                        break;
                    } else {
                        i4++;
    ...
    private void set(TLRPC$TL_messages_stickerSet tLRPC$TL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                break;
            } else if (arrayList.get(i).f1608id == tLRPC$TL_messages_stickerSet.set.thumb_document_id) {
        ...
        if (z && this.stickers) {
            string = LocaleController.getString(C3630R.string.StoryContainsStickersEmojiFrom);
        } else if (z) {
            string = LocaleController.getString(C3630R.string.StoryContainsEmojiFrom);
        } else {
            string = LocaleController.getString(C3630R.string.StoryContainsStickersFrom);
        }
    ...
    private void set(TLRPC$StickerSetCovered tLRPC$StickerSetCovered) {
        ...
        if (tLRPC$Document == null && (tLRPC$StickerSetCovered instanceof TLRPC$TL_stickerSetFullCovered)) {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                if (arrayList.get(i).f1608id == tLRPC$StickerSetCovered.set.thumb_document_id) {
                    tLRPC$Document = arrayList.get(i);
                }
            }
            if (tLRPC$Document == null && !arrayList.isEmpty()) {
    ...
    private void animateLoad(boolean z) {
        ...
        if (z) {
            ...
            if (this.layout == null || Math.abs(getMeasuredHeight() - ((getPaddingTop() + this.layout.getHeight()) + getPaddingBottom())) > AndroidUtilities.m102dp(3)) {
                z2 = true;
            }
