...
public class ShapeDetector {
    ...
    public static class Point {
        ...
        /* renamed from: x */
        public double f1862x;

        /* renamed from: y */
        public double f1863y;
        ...
        public double distance(double d, double d2) {
            return Math.sqrt(Math.pow(d - this.f1862x, 2.0d) + Math.pow(d2 - this.f1863y, 2.0d));
        }

        public double distance(Point point) {
            return distance(point.f1862x, point.f1863y);
        }
    }
    ...
    public /* synthetic */ void lambda$new$2() {
        ...
        synchronized (this) {
            ...
            translate(fullClone, -centroid.f1862x, -centroid.f1863y);
    ...
    private ArrayList<Point> resample(ArrayList<Point> arrayList, int i) {
        ...
        while (i4 < arrayList.size()) {
            ...
            if (d3 >= pathLength) {
                ...
                i2 = i3;
                d = pathLength;
                Point point = new Point(arrayList.get(i5).f1862x + ((arrayList.get(i4).f1862x - arrayList.get(i5).f1862x) * d4), arrayList.get(i5).f1863y + (d4 * (arrayList.get(i4).f1863y - arrayList.get(i5).f1863y)));
    ...
    private double distanceAtAngle(ArrayList<Point> arrayList, Point point, ArrayList<Point> arrayList2, double d) {
        ...
        while (i < min) {
            ...
            double d3 = point3.f1862x;
            int i2 = i;
            double d4 = point2.f1862x;
            double d5 = point3.f1863y;
            int i3 = min;
            double d6 = point2.f1863y;
            double d7 = cos;
    ...
    private ArrayList<Point> fullClone(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            arrayList2.add(new Point(point.f1862x, point.f1863y));
        }
        return arrayList2;
    }
    ...
    private void rotate(ArrayList<Point> arrayList, double d, Point point) {
        ...
        while (i < arrayList.size()) {
            ...
            double d2 = point2.f1862x;
            double d3 = point.f1862x;
            double d4 = point2.f1863y;
            int i2 = i;
            double d5 = point.f1863y;
            point2.f1863y = ((d2 - d3) * sin) + ((d4 - d5) * cos) + d5;
            point2.f1862x = (((d2 - d3) * cos) - ((d4 - d5) * sin)) + d3;
            i = i2 + 1;
        }
    }
    ...
    private RectD boundingBox(ArrayList<Point> arrayList) {
        ...
        double d = arrayList.get(0).f1862x;
        double d2 = arrayList.get(0).f1863y;
        RectD rectD = new RectD(d, d2, d, d2);
        for (int i = 1; i < arrayList.size(); i++) {
            Point point = arrayList.get(i);
            rectD.union(point.f1862x, point.f1863y);
        }
        return rectD;
    }
    ...
    private Point centroid(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            Point point2 = arrayList.get(i);
            point.f1862x += point2.f1862x;
            point.f1863y += point2.f1863y;
        }
        point.f1862x /= arrayList.size();
        point.f1863y /= arrayList.size();
        return point;
    }

    private double indicativeAngle(ArrayList<Point> arrayList) {
        Point centroid = centroid(arrayList);
        return Math.atan2(centroid.f1863y - arrayList.get(0).f1863y, centroid.f1862x - arrayList.get(0).f1862x);
    }
    ...
    private Shape constructShape(int i, ArrayList<Point> arrayList) {
        ...
        if (i == 4) {
            ...
            shape.centerX = (float) point.f1862x;
            shape.centerY = (float) point.f1863y;
            shape.middleX = (float) point2.f1862x;
            shape.middleY = (float) point2.f1863y;
            shape.radiusX = (float) point3.f1862x;
            shape.radiusY = (float) point3.f1863y;
            shape.arrowTriangleLength = 16.0f;
        } else {
            Point centroid = centroid(arrayList);
            shape.centerX = (float) centroid.f1862x;
            shape.centerY = (float) centroid.f1863y;
            RectD boundingBox = boundingBox(arrayList);
            ...
            if (i == 2 && (findAnglePoint = findAnglePoint(arrayList, 1)) > 0) {
                ...
                shape.rotation = (float) Math.atan2(point4.f1863y - shape.centerY, point4.f1862x - shape.centerX);
            }
        }
        return shape;
    }
    ...
    public /* synthetic */ void lambda$showSaveLearnDialog$3(ArrayList arrayList, DialogInterface dialogInterface, int i) {
        if (i == 0) {
            ...
            for (int i2 = 0; i2 < this.templates.size(); i2++) {
                ...
                for (int i3 = 0; i3 < template.points.size(); i3++) {
                    ...
                    sb.append("[");
                    sb.append(Math.round(point.f1862x));
                    sb.append(",");
                    sb.append(Math.round(point.f1863y));
                    sb.append("]");
                }
                sb.append("],\n\t\t\"freq\": ");
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            str = str + "," + Math.round(((Point) arrayList.get(i4)).f1862x) + "," + Math.round(((Point) arrayList.get(i4)).f1863y);
        }
