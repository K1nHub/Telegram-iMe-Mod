...
public class LocationMarker extends View {
    ...
    public LocationMarker(Context context, float f) {
        ...
        this.icon = context.getResources().getDrawable(C3630R.C3632drawable.map_pin3).mutate();
            ...
            for (int i = 0; i < tLRPC$TL_messages_stickerSet.packs.size(); i++) {
                ...
                if (tLRPC$TL_stickerPack.emoticon.contains(str) && !tLRPC$TL_stickerPack.documents.isEmpty()) {
                    ...
                    for (int i2 = 0; i2 < tLRPC$TL_messages_stickerSet.documents.size(); i2++) {
                        if (tLRPC$TL_messages_stickerSet.documents.get(i2).f1608id == longValue) {
    ...
    private void setupLayout() {
        if (this.relayout) {
            ...
            this.f1872w = (f5 * f6) + (this.layoutWidth * this.textScale);
            this.f1871h = ((rectF2.top + rectF2.bottom) * f6) + Math.max(f6 * 21.33f, this.layout.getHeight() * this.textScale);
            this.relayout = false;
        }
    }
    ...
    protected void onMeasure(int i, int i2) {
        setupLayout();
        setMeasuredDimension(this.padx + Math.round(this.f1872w) + this.padx, this.pady + Math.round(this.f1871h) + this.pady);
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        int i = this.padx;
        int i2 = this.pady;
        rectF.set(i, i2, i + this.f1872w, i2 + this.f1871h);
        ...
        if (this.hasFlag) {
            ...
            imageReceiver.setImageCoords(this.padx + ((this.padding.left + 2.25f) * f3), this.pady + ((this.f1871h - (f3 * 21.33f)) / 2.0f), f3 * 21.33f, f3 * 21.33f);
            canvas.save();
        ...
        canvas.translate(f7 + ((f8 + f2 + 21.33f + 3.25f) * this.density), this.pady + (this.f1871h / 2.0f));
