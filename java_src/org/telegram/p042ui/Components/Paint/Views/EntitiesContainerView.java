...
public class EntitiesContainerView extends FrameLayout {
    ...
    /* renamed from: px */
    private float f1868px;

    /* renamed from: py */
    private float f1869py;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getPointerCount() == 1) {
            ...
            if (actionMasked == 0) {
                ...
                this.f1868px = motionEvent.getX();
                this.f1869py = motionEvent.getY();
                this.cancelled = false;
            } else if (!this.cancelled && actionMasked == 2) {
                float x = motionEvent.getX();
                float y = motionEvent.getY();
                if (this.hasTransformed || MathUtils.distance(x, y, this.f1868px, this.f1869py) > AndroidUtilities.touchSlop) {
                    this.hasTransformed = true;
                    onSelectedEntityRequest.hasPanned = true;
                    onSelectedEntityRequest.pan(x - this.f1868px, y - this.f1869py);
                    this.f1868px = x;
                    this.f1869py = y;
                }
