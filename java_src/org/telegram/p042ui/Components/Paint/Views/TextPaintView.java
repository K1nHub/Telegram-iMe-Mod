...
public class TextPaintView extends EntityView {
    ...
    public TextPaintView(Context context, Point point, int i, CharSequence charSequence, Swatch swatch, int i2) {
        ...
        this.editText.setPadding(AndroidUtilities.m102dp(7), AndroidUtilities.m102dp(7), AndroidUtilities.m102dp(7), AndroidUtilities.m102dp(7));
        this.editText.setClickable(false);
        this.editText.setEnabled(false);
        this.editText.setCursorColor(-1);
        this.editText.setTextSize(0, this.baseFontSize);
        this.editText.setCursorSize(AndroidUtilities.m103dp(this.baseFontSize * 0.4f));
    ...
    @Override // org.telegram.p042ui.Components.Paint.Views.EntityView
    ...
    public void updateHint() {
        if (this.editText.getText().length() <= 0) {
            this.editText.setHint(LocaleController.getString(C3630R.string.TextPlaceholder));
    ...
    public void setBaseFontSize(int i) {
        ...
        this.editText.setCursorSize(AndroidUtilities.m103dp(f * 0.4f));
    ...
    protected Rect getSelectionBounds() {
        ...
        float measuredWidth = (getMeasuredWidth() * getScale()) + (AndroidUtilities.m102dp(64) / scaleX);
        float measuredHeight = (getMeasuredHeight() * getScale()) + (AndroidUtilities.m102dp(52) / scaleX);
    ...
    public class TextViewSelectionView extends EntityView.SelectionView {
        ...
        @Override // org.telegram.p042ui.Components.Paint.Views.EntityView.SelectionView
        protected int pointInsideHandle(float f, float f2) {
            float m103dp = AndroidUtilities.m103dp(19.5f);
            float m103dp2 = AndroidUtilities.m103dp(1.0f) + m103dp;
            float f3 = m103dp2 * 2.0f;
            float measuredWidth = getMeasuredWidth() - f3;
            float measuredHeight = ((getMeasuredHeight() - f3) / 2.0f) + m103dp2;
            if (f <= m103dp2 - m103dp || f2 <= measuredHeight - m103dp || f >= m103dp2 + m103dp || f2 >= measuredHeight + m103dp) {
                float f4 = m103dp2 + measuredWidth;
                return (f <= f4 - m103dp || f2 <= measuredHeight - m103dp || f >= f4 + m103dp || f2 >= measuredHeight + m103dp) ? 0 : 2;
            }
            return 1;
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float m103dp = AndroidUtilities.m103dp(2.0f) + dpf2 + AndroidUtilities.m102dp(15);
            float f = m103dp * 2.0f;
            float measuredWidth = getMeasuredWidth() - f;
            float measuredHeight = getMeasuredHeight() - f;
            RectF rectF = AndroidUtilities.rectTmp;
            float f2 = m103dp + measuredWidth;
            float f3 = m103dp + measuredHeight;
            rectF.set(m103dp, m103dp, f2, f3);
            float m102dp = AndroidUtilities.m102dp(12);
            float min = Math.min(m102dp, measuredWidth / 2.0f);
            float f4 = measuredHeight / 2.0f;
            float min2 = Math.min(m102dp, f4);
            this.path.rewind();
            float f5 = min * 2.0f;
            float f6 = m103dp + f5;
            float f7 = 2.0f * min2;
            float f8 = m103dp + f7;
            rectF.set(m103dp, m103dp, f6, f8);
            this.path.arcTo(rectF, 180.0f, 90.0f);
            float f9 = f2 - f5;
            rectF.set(f9, m103dp, f2, f8);
            this.path.arcTo(rectF, 270.0f, 90.0f);
            canvas.drawPath(this.path, this.paint);
            this.path.rewind();
            float f10 = f3 - f7;
            rectF.set(m103dp, f10, f6, f3);
            this.path.arcTo(rectF, 180.0f, -90.0f);
            rectF.set(f9, f10, f2, f3);
            this.path.arcTo(rectF, 90.0f, -90.0f);
            canvas.drawPath(this.path, this.paint);
            float f11 = m103dp + f4;
            canvas.drawCircle(m103dp, f11, dpf2, this.dotStrokePaint);
            canvas.drawCircle(m103dp, f11, (dpf2 - AndroidUtilities.m102dp(1)) + 1.0f, this.dotPaint);
            canvas.drawCircle(f2, f11, dpf2, this.dotStrokePaint);
            canvas.drawCircle(f2, f11, (dpf2 - AndroidUtilities.m102dp(1)) + 1.0f, this.dotPaint);
            canvas.saveLayerAlpha(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight(), 255, 31);
            float f12 = m103dp + min2;
            float f13 = f3 - min2;
            canvas.drawLine(m103dp, f12, m103dp, f13, this.paint);
            canvas.drawLine(f2, f12, f2, f13, this.paint);
            canvas.drawCircle(f2, f11, (AndroidUtilities.m102dp(1) + dpf2) - 1.0f, this.clearPaint);
            canvas.drawCircle(m103dp, f11, (dpf2 + AndroidUtilities.m102dp(1)) - 1.0f, this.clearPaint);
