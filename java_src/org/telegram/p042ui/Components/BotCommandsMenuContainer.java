...
public class BotCommandsMenuContainer extends FrameLayout implements NestedScrollingParent {
    ...
    public BotCommandsMenuContainer(Context context) {
        ...
        RecyclerListView recyclerListView = new RecyclerListView(context) { // from class: org.telegram.ui.Components.BotCommandsMenuContainer.1
            ...
            public void dispatchDraw(Canvas canvas) {
                ...
                float m102dp = y - AndroidUtilities.m102dp(8);
                if (m102dp > BitmapDescriptorFactory.HUE_RED) {
                    int i = (int) m102dp;
                    BotCommandsMenuContainer.this.shadowDrawable.setBounds(-AndroidUtilities.m102dp(8), i - AndroidUtilities.m102dp(24), getMeasuredWidth() + AndroidUtilities.m102dp(8), i);
                    BotCommandsMenuContainer.this.shadowDrawable.draw(canvas);
                }
                canvas.drawRect(BitmapDescriptorFactory.HUE_RED, m102dp, getMeasuredWidth(), getMeasuredHeight() + AndroidUtilities.m102dp(16), BotCommandsMenuContainer.this.backgroundPaint);
                RectF rectF = AndroidUtilities.rectTmp;
                rectF.set((getMeasuredWidth() / 2.0f) - AndroidUtilities.m102dp(12), m102dp - AndroidUtilities.m102dp(4), (getMeasuredWidth() / 2.0f) + AndroidUtilities.m102dp(12), m102dp);
                canvas.drawRoundRect(rectF, AndroidUtilities.m102dp(4), AndroidUtilities.m102dp(4), BotCommandsMenuContainer.this.topBackground);
                super.dispatchDraw(canvas);
            }
        };
    ...
    private void checkDismiss() {
        ...
        if (this.listView.getTranslationY() > AndroidUtilities.m102dp(16)) {
            dismiss();
        } else {
            playEnterAnim(false);
        }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        recyclerListView.setTranslationY((recyclerListView.getMeasuredHeight() - this.listView.getPaddingTop()) + AndroidUtilities.m102dp(16));
        playEnterAnim(true);
        this.entering = false;
    }
    ...
    public void dismiss() {
        ...
        cancelCurrentAnimation();
        RecyclerListView recyclerListView = this.listView;
        ObjectAnimator ofFloat = ObjectAnimator.ofFloat(recyclerListView, FrameLayout.TRANSLATION_Y, recyclerListView.getTranslationY(), (getMeasuredHeight() - this.scrollYOffset) + AndroidUtilities.m102dp(40));
        this.currentAnimation = ofFloat;
    ...
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        if (motionEvent.getAction() != 0 || motionEvent.getY() >= this.scrollYOffset - AndroidUtilities.m102dp(24)) {
