...
public class ChoosingStickerStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float f = 2.0f;
        if (this.increment) {
            m103dp = (AndroidUtilities.m103dp(2.1f) * interpolation2) + ((AndroidUtilities.m102dp(7) - AndroidUtilities.m103dp(2.1f)) * (1.0f - interpolation2));
            interpolation = AndroidUtilities.dpf2(1.5f) * (1.0f - cubicBezierInterpolator.getInterpolation(this.progress / 2.0f));
        } else {
            m103dp = (AndroidUtilities.m103dp(2.1f) * (1.0f - interpolation2)) + ((AndroidUtilities.m102dp(7) - AndroidUtilities.m103dp(2.1f)) * interpolation2);
            interpolation = CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(this.progress / 2.0f) * AndroidUtilities.dpf2(1.5f);
        }
        float m102dp = AndroidUtilities.m102dp(11) / 2.0f;
        ...
        if (paint.getStrokeWidth() != AndroidUtilities.m103dp(0.8f)) {
            paint.setStrokeWidth(AndroidUtilities.m103dp(0.8f));
        }
        int i2 = 0;
        while (i2 < 2) {
            canvas.save();
            canvas.translate((paint.getStrokeWidth() / f) + interpolation + (AndroidUtilities.m102dp(9) * i2) + getBounds().left + AndroidUtilities.dpf2(0.2f), (paint.getStrokeWidth() / f) + AndroidUtilities.dpf2(f) + getBounds().top);
            RectF rectF = AndroidUtilities.rectTmp;
            rectF.set(BitmapDescriptorFactory.HUE_RED, dpf22, AndroidUtilities.m102dp(7), AndroidUtilities.m102dp(11) - dpf22);
            canvas.drawOval(rectF, paint);
            canvas.drawCircle(m103dp, m102dp, dpf2, paint2);
            canvas.restore();
            i2++;
            f = 2.0f;
        }
