...
public class OtherDocumentPlaceholderDrawable extends RecyclableDrawable implements DownloadController.FileDownloadProgressListener {
    ...
    public OtherDocumentPlaceholderDrawable(Context context, View view, MessageObject messageObject) {
        docPaint.setTextSize(AndroidUtilities.m102dp(14));
        namePaint.setTextSize(AndroidUtilities.m102dp(19));
        sizePaint.setTextSize(AndroidUtilities.m102dp(15));
        buttonPaint.setTextSize(AndroidUtilities.m102dp(15));
        percentPaint.setTextSize(AndroidUtilities.m102dp(15));
        openPaint.setTextSize(AndroidUtilities.m102dp(15));
        ...
        if (document != null) {
            ...
            this.ext = upperCase;
            if (((int) Math.ceil(docPaint.measureText(upperCase))) > AndroidUtilities.m102dp(40)) {
                this.ext = TextUtils.ellipsize(this.ext, docPaint, AndroidUtilities.m102dp(40), TextUtils.TruncateAt.END).toString();
            }
            ...
            if (((int) Math.ceil(namePaint.measureText(this.fileName))) > AndroidUtilities.m102dp(320)) {
                this.fileName = TextUtils.ellipsize(this.fileName, namePaint, AndroidUtilities.m102dp(320), TextUtils.TruncateAt.END).toString();
            }
        }
        checkFileExist();
    }
    ...
    public void draw(Canvas canvas) {
        ...
        int m102dp2 = (height - AndroidUtilities.m102dp((int) PsExtractor.VIDEO_STREAM_MASK)) / 2;
        int m102dp3 = (width - AndroidUtilities.m102dp(48)) / 2;
        this.thumbDrawable.setBounds(m102dp3, m102dp2, AndroidUtilities.m102dp(48) + m102dp3, AndroidUtilities.m102dp(48) + m102dp2);
        this.thumbDrawable.draw(canvas);
        canvas.drawText(this.ext, (width - ((int) Math.ceil(docPaint.measureText(this.ext)))) / 2, AndroidUtilities.m102dp(31) + m102dp2, docPaint);
        canvas.drawText(this.fileName, (width - ((int) Math.ceil(namePaint.measureText(this.fileName)))) / 2, AndroidUtilities.m102dp(96) + m102dp2, namePaint);
        canvas.drawText(this.fileSize, (width - ((int) Math.ceil(sizePaint.measureText(this.fileSize)))) / 2, AndroidUtilities.m102dp(125) + m102dp2, sizePaint);
        if (this.loaded) {
            string = LocaleController.getString("OpenFile", C3630R.string.OpenFile);
            textPaint = openPaint;
            m102dp = 0;
        } else {
            if (this.loading) {
                string = LocaleController.getString("Cancel", C3630R.string.Cancel).toUpperCase();
            } else {
                string = LocaleController.getString("TapToDownload", C3630R.string.TapToDownload);
            }
            m102dp = AndroidUtilities.m102dp(28);
            textPaint = buttonPaint;
        }
        canvas.drawText(string, (width - ((int) Math.ceil(textPaint.measureText(string)))) / 2, AndroidUtilities.m102dp(235) + m102dp2 + m102dp, textPaint);
        if (this.progressVisible) {
            if (this.progress != null) {
                canvas.drawText(this.progress, (width - ((int) Math.ceil(percentPaint.measureText(str)))) / 2, AndroidUtilities.m102dp(210) + m102dp2, percentPaint);
            }
            int m102dp4 = (width - AndroidUtilities.m102dp((int) PsExtractor.VIDEO_STREAM_MASK)) / 2;
            int m102dp5 = m102dp2 + AndroidUtilities.m102dp(232);
            progressPaint.setColor(-10327179);
            progressPaint.setAlpha((int) (this.animatedAlphaValue * 255.0f));
            float f = m102dp5;
            canvas.drawRect(((int) (AndroidUtilities.m102dp((int) PsExtractor.VIDEO_STREAM_MASK) * this.animatedProgressValue)) + m102dp4, f, AndroidUtilities.m102dp((int) PsExtractor.VIDEO_STREAM_MASK) + m102dp4, AndroidUtilities.m102dp(2) + m102dp5, progressPaint);
            progressPaint.setColor(-1);
            progressPaint.setAlpha((int) (this.animatedAlphaValue * 255.0f));
            float f2 = m102dp4;
            canvas.drawRect(f2, f, f2 + (AndroidUtilities.m102dp((int) PsExtractor.VIDEO_STREAM_MASK) * this.animatedProgressValue), m102dp5 + AndroidUtilities.m102dp(2), progressPaint);
            updateAnimation();
        }
        canvas.restore();
    }
    ...
    @Override // org.telegram.p042ui.Components.RecyclableDrawable
    public void recycle() {
