...
public class IdenticonDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (bArr.length == 16) {
            ...
            float f = 8.0f * floor;
            float max = Math.max((float) BitmapDescriptorFactory.HUE_RED, (getBounds().width() - f) / 2.0f);
            float max2 = Math.max((float) BitmapDescriptorFactory.HUE_RED, (getBounds().height() - f) / 2.0f);
            int i = 0;
        ...
        float max3 = Math.max((float) BitmapDescriptorFactory.HUE_RED, (getBounds().width() - f4) / 2.0f);
        float max4 = Math.max((float) BitmapDescriptorFactory.HUE_RED, (getBounds().height() - f4) / 2.0f);
        int i4 = 0;
