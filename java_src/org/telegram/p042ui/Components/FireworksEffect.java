...
public class FireworksEffect {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            ...
            FireworksEffect.this.particlePaint.setStrokeWidth(AndroidUtilities.m103dp(1.5f) * this.scale);
            FireworksEffect.this.particlePaint.setAlpha((int) (this.alpha * 255.0f));
            canvas.drawPoint(this.f1844x, this.f1845y, FireworksEffect.this.particlePaint);
        }
    }
    ...
    public FireworksEffect() {
        ...
        paint.setStrokeWidth(AndroidUtilities.m103dp(1.5f));
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1844x;
                float f4 = particle.f1842vx;
                float f5 = particle.velocity;
                float f6 = (float) j;
                particle.f1844x = f3 + (((f4 * f5) * f6) / 500.0f);
                float f7 = particle.f1845y;
                float f8 = particle.f1843vy;
                particle.f1845y = f7 + (((f5 * f8) * f6) / 500.0f);
                particle.f1843vy = f8 + (f6 / 100.0f);
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (Utilities.random.nextBoolean() && this.particles.size() + 8 < 150) {
            ...
            float nextFloat2 = i2 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m102dp(20)) - i2));
            ...
            for (int i4 = 0; i4 < 8; i4++) {
                ...
                particle.f1844x = nextFloat;
                particle.f1845y = nextFloat2;
                particle.f1842vx = cos * 1.5f;
                particle.f1843vy = sin;
                particle.color = i3;
                particle.alpha = 1.0f;
