...
public class TransformableLoginButtonView extends View {
    ...
    public TransformableLoginButtonView(Context context) {
        ...
        this.outlinePaint.setStrokeWidth(AndroidUtilities.m102dp(2));
    ...
    protected void onDraw(Canvas canvas) {
        if (this.drawBackground) {
            float m103dp = AndroidUtilities.m103dp(((this.transformType == 0 ? this.progress : 1.0f) * 26.0f) + 6.0f);
            this.rect.set(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight());
            canvas.drawRoundRect(this.rect, m103dp, m103dp, this.backgroundPaint);
        }
        int i = this.transformType;
        if (i == IdFabric$CustomType.TRANSFORMABLE_LOGIN_BUTTON_SEARCH) {
            Drawable drawable = ContextCompat.getDrawable(getContext(), C3630R.C3632drawable.fork_ic_search_contact_30);
            ...
            if (textPaint != null && this.buttonText != null) {
                ...
                canvas.drawText(this.buttonText, (getWidth() - this.buttonWidth) / 2.0f, ((getHeight() / 2.0f) + (this.textPaint.getTextSize() / 2.0f)) - AndroidUtilities.m103dp(1.75f), this.textPaint);
                this.textPaint.setAlpha(alpha);
            }
            float max = (Math.max(0.4f, this.progress) - 0.4f) / 0.6f;
            if (max != BitmapDescriptorFactory.HUE_RED) {
                float m103dp2 = AndroidUtilities.m103dp(21.0f) + ((getWidth() - (AndroidUtilities.m103dp(21.0f) * 2)) * max);
                float height2 = getHeight() / 2.0f;
                canvas.drawLine(AndroidUtilities.m103dp(21.0f), height2, m103dp2, height2, this.outlinePaint);
                double m103dp3 = AndroidUtilities.m103dp(9.0f) * max;
                float cos = (float) (m103dp2 - (Math.cos(0.7853981633974483d) * m103dp3));
                float sin = (float) (Math.sin(0.7853981633974483d) * m103dp3);
                canvas.drawLine(m103dp2, height2, cos, height2 - sin, this.outlinePaint);
                canvas.drawLine(m103dp2, height2, cos, height2 + sin, this.outlinePaint);
            }
        } else if (i == 1) {
            float m103dp4 = AndroidUtilities.m103dp(21.0f);
            float width2 = getWidth() - AndroidUtilities.m103dp(21.0f);
            float height3 = getHeight() / 2.0f;
            canvas.save();
            canvas.translate((-AndroidUtilities.m102dp(2)) * this.progress, BitmapDescriptorFactory.HUE_RED);
            canvas.rotate(this.progress * 90.0f, getWidth() / 2.0f, getHeight() / 2.0f);
            canvas.drawLine(((width2 - m103dp4) * this.progress) + m103dp4, height3, width2, height3, this.outlinePaint);
            int m103dp5 = AndroidUtilities.m103dp((this.progress * (-1.0f)) + 9.0f);
            int m103dp6 = AndroidUtilities.m103dp((this.progress * 7.0f) + 9.0f);
            double d = width2;
            double d2 = m103dp5;
            double d3 = height3;
