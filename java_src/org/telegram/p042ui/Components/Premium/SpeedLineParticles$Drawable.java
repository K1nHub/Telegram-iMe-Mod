...
public class SpeedLineParticles$Drawable {
    ...
    /* renamed from: dt */
    private final float f1907dt = 1000.0f / AndroidUtilities.screenRefreshRate;
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.particles.size(); i++) {
            ...
            if (currentTimeMillis > particle.lifeTime || !this.screenRect.contains(particle.f1908x, particle.f1909y)) {
    ...
    private class Particle {
        ...
        /* renamed from: x */
        private float f1908x;

        /* renamed from: y */
        private float f1909y;
        ...
        public void draw(Canvas canvas, int i, long j) {
            int i2 = i * 4;
            SpeedLineParticles$Drawable.this.lines[i2] = this.f1908x;
            SpeedLineParticles$Drawable.this.lines[i2 + 1] = this.f1909y;
            SpeedLineParticles$Drawable.this.lines[i2 + 2] = this.f1908x + (AndroidUtilities.m102dp(30) * this.vecX);
            SpeedLineParticles$Drawable.this.lines[i2 + 3] = this.f1909y + (AndroidUtilities.m102dp(30) * this.vecY);
            if (SpeedLineParticles$Drawable.this.paused) {
                return;
            }
            float m102dp = AndroidUtilities.m102dp(4) * (SpeedLineParticles$Drawable.this.f1907dt / 660.0f);
            SpeedLineParticles$Drawable speedLineParticles$Drawable = SpeedLineParticles$Drawable.this;
            float f = m102dp * speedLineParticles$Drawable.speedScale;
            this.f1908x += this.vecX * f;
            this.f1909y += this.vecY * f;
            float f2 = this.inProgress;
            if (f2 != 1.0f) {
                float f3 = f2 + (speedLineParticles$Drawable.f1907dt / 200.0f);
                this.inProgress = f3;
                if (f3 > 1.0f) {
        ...
        public void genPosition(long j, boolean z) {
            ...
            this.f1908x = abs;
            this.f1909y = rectF.top + Math.abs(Utilities.fastRandom.nextInt() % rectF.height());
            double atan2 = Math.atan2(abs - SpeedLineParticles$Drawable.this.rect.centerX(), this.f1909y - SpeedLineParticles$Drawable.this.rect.centerY());
