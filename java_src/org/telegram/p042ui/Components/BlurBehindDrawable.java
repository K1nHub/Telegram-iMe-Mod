...
import org.telegram.p042ui.Components.BlurBehindDrawable;
            ...
            if (this.renderingBitmap == null || this.invalidate) {
                ...
                for (int i = 0; i < 2; i++) {
                    if (this.blurredBitmapTmp[i] == null || this.parentView.getMeasuredWidth() != this.lastW || this.parentView.getMeasuredHeight() != this.lastH) {
                        ...
                        int m102dp = AndroidUtilities.statusBarHeight + AndroidUtilities.m102dp(200);
                        this.toolbarH = m102dp;
                        if (i == 0) {
                            measuredHeight = m102dp;
                        }
                        try {
    ...
    private void generateBlurredBitmaps() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (measuredHeight != 0 && measuredWidth != 0) {
                ...
                this.toolbarH = m102dp;
                if (i != 0) {
                    m102dp = measuredHeight;
                }
                if (bitmapArr[i] == null || bitmapArr[i].getHeight() != m102dp || bitmapArr[i].getWidth() != this.parentView.getMeasuredWidth()) {
                    DispatchQueue dispatchQueue = this.queue;
                    if (dispatchQueue != null) {
                        dispatchQueue.cleanupQueue();
                    }
                    int i2 = (int) (measuredWidth / 6.0f);
                    this.blurredBitmapTmp[i] = Bitmap.createBitmap(i2, (int) (m102dp / 6.0f), Bitmap.Config.ARGB_8888);
                    if (i == 1) {
                        this.blurredBitmapTmp[i].eraseColor(getThemedColor(Theme.key_windowBackgroundWhite));
                    }
                    this.blurCanvas[i] = new Canvas(this.blurredBitmapTmp[i]);
