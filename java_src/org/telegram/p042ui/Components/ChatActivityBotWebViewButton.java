...
public class ChatActivityBotWebViewButton extends FrameLayout {
    ...
    public void draw(Canvas canvas) {
        canvas.save();
        float height = (getHeight() - AndroidUtilities.m102dp(32)) / 2.0f;
        float max = Math.max((getWidth() - this.menuButtonWidth) - AndroidUtilities.m102dp(4), getHeight()) * this.progress;
        float m102dp = AndroidUtilities.m102dp(16) + max;
        RectF rectF = AndroidUtilities.rectTmp;
        rectF.set(AndroidUtilities.m102dp(14) - max, (AndroidUtilities.m102dp(4) + height) - max, AndroidUtilities.m102dp(6) + this.menuButtonWidth + max, (getHeight() - AndroidUtilities.m102dp(12)) + max);
        this.path.rewind();
        this.path.addRoundRect(rectF, m102dp, m102dp, Path.Direction.CW);
        canvas.clipPath(this.path);
        canvas.drawColor(this.backgroundColor);
        canvas.saveLayerAlpha(rectF, (int) ((1.0f - (Math.min(0.5f, this.progress) / 0.5f)) * 255.0f), 31);
        canvas.translate(AndroidUtilities.m102dp(10), height);
        ...
        canvas.restore();
        canvas.translate((-AndroidUtilities.m102dp(8)) * (1.0f - this.progress), BitmapDescriptorFactory.HUE_RED);
        super.draw(canvas);
        canvas.restore();
    }
