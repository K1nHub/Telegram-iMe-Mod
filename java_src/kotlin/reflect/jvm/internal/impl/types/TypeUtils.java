...
public class TypeUtils {
    ...
    /* JADX WARN: Removed duplicated region for block: B:117:0x01e3 A[ADDED_TO_REGION] */
    ...
    */
    ...
    public static List<KotlinType> getImmediateSupertypes(KotlinType kotlinType) {
        ...
        Collection<KotlinType> mo2055getSupertypes = kotlinType.getConstructor().mo2055getSupertypes();
        ArrayList arrayList = new ArrayList(mo2055getSupertypes.size());
        for (KotlinType kotlinType2 : mo2055getSupertypes) {
    ...
    public static boolean isNullableType(KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            for (KotlinType kotlinType2 : constructor.mo2055getSupertypes()) {
    ...
    public static boolean hasNullableSuperType(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2054getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
    ...
    public static ClassDescriptor getClassDescriptor(KotlinType kotlinType) {
        ...
        if (mo2054getDeclarationDescriptor instanceof ClassDescriptor) {
            return (ClassDescriptor) mo2054getDeclarationDescriptor;
        }
        return null;
    }
    ...
    private static boolean contains(KotlinType kotlinType, Function1<UnwrappedType, Boolean> function1, SmartSet<KotlinType> smartSet) {
        ...
        if (smartSet == null || !smartSet.contains(kotlinType)) {
            ...
            if (flexibleType == null || !(contains(flexibleType.getLowerBound(), function1, smartSet) || contains(flexibleType.getUpperBound(), function1, smartSet))) {
                ...
                TypeConstructor constructor = kotlinType.getConstructor();
                if (constructor instanceof IntersectionTypeConstructor) {
                    for (KotlinType kotlinType2 : ((IntersectionTypeConstructor) constructor).mo2055getSupertypes()) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2054getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo2054getDeclarationDescriptor();
        }
        return null;
    }
}
