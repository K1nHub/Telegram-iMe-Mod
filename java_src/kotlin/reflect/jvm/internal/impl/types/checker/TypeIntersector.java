...
public final class TypeIntersector {
    ...
    public final SimpleType intersectTypes$descriptors(List<? extends SimpleType> types) {
        ...
        for (SimpleType simpleType : types) {
            if (simpleType.getConstructor() instanceof IntersectionTypeConstructor) {
                Collection<KotlinType> mo2054getSupertypes = simpleType.getConstructor().mo2054getSupertypes();
                Intrinsics.checkNotNullExpressionValue(mo2054getSupertypes, "type.constructor.supertypes");
                collectionSizeOrDefault2 = CollectionsKt__IterablesKt.collectionSizeOrDefault(mo2054getSupertypes, 10);
                ArrayList arrayList2 = new ArrayList(collectionSizeOrDefault2);
                for (KotlinType it : mo2054getSupertypes) {
    ...
    private final SimpleType intersectTypesWithoutIntersectionType(final Set<? extends SimpleType> set) {
        ...
        Collection<SimpleType> filterTypes = filterTypes(set, new C3481x702eebb8(this));
        filterTypes.isEmpty();
        SimpleType findIntersectionType = IntegerLiteralTypeConstructor.Companion.findIntersectionType(filterTypes);
        if (findIntersectionType != null) {
            return findIntersectionType;
        }
        Collection<SimpleType> filterTypes2 = filterTypes(filterTypes, new C3482xc97d8c34(NewKotlinTypeChecker.Companion.getDefault()));
