...
public final class DebugMetadataKt {
    public static final StackTraceElement getStackTraceElement(BaseContinuationImpl baseContinuationImpl) {
        ...
        checkDebugMetadataVersion(1, debugMetadataAnnotation.m144v());
        int label = getLabel(baseContinuationImpl);
        int i = label < 0 ? -1 : debugMetadataAnnotation.m146l()[label];
        String moduleName = ModuleNameRetriever.INSTANCE.getModuleName(baseContinuationImpl);
        if (moduleName == null) {
            str = debugMetadataAnnotation.m148c();
        } else {
            str = moduleName + '/' + debugMetadataAnnotation.m148c();
        }
        return new StackTraceElement(str, debugMetadataAnnotation.m145m(), debugMetadataAnnotation.m147f(), i);
    }
