...
public final class C0232xfdb59cc4 implements LifecycleEventObserver {
    ...
    public void onStateChanged(LifecycleOwner source, Lifecycle.Event event) {
        ...
        if (event == Lifecycle.Event.Companion.upTo(this.$state)) {
            ...
            try {
                ...
                m1943constructorimpl = Result.m1943constructorimpl(function0.invoke());
            } catch (Throwable th) {
                Result.Companion companion2 = Result.Companion;
                m1943constructorimpl = Result.m1943constructorimpl(ResultKt.createFailure(th));
            }
            cancellableContinuation.resumeWith(m1943constructorimpl);
        } else if (event == Lifecycle.Event.ON_DESTROY) {
            this.$this_suspendWithStateAtLeastUnchecked.removeObserver(this);
            CancellableContinuation<Object> cancellableContinuation2 = this.$co;
            Result.Companion companion3 = Result.Companion;
            cancellableContinuation2.resumeWith(Result.m1943constructorimpl(ResultKt.createFailure(new LifecycleDestroyedException())));
        }
    }
}
