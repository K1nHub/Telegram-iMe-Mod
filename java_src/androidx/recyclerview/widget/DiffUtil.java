...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f51x - snake2.f51x;
            return i == 0 ? snake.f52y - snake2.f52y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f51x += range.oldListStart;
                diffPartial.f52y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f51x;
                    range2.newListEnd = diffPartial.f52y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f51x - 1;
                    range2.newListEnd = diffPartial.f52y;
                } else {
                    range2.oldListEnd = diffPartial.f51x;
                    range2.newListEnd = diffPartial.f52y - 1;
                }
                arrayList2.add(range2);
                if (diffPartial.reverse) {
                    if (diffPartial.removal) {
                        int i2 = diffPartial.f51x;
                        int i3 = diffPartial.size;
                        range.oldListStart = i2 + i3 + 1;
                        range.newListStart = diffPartial.f52y + i3;
                    } else {
                        int i4 = diffPartial.f51x;
                        int i5 = diffPartial.size;
                        range.oldListStart = i4 + i5;
                        range.newListStart = diffPartial.f52y + i5 + 1;
                    }
                } else {
                    int i6 = diffPartial.f51x;
                    int i7 = diffPartial.size;
                    range.oldListStart = i6 + i7;
                    range.newListStart = diffPartial.f52y + i7;
                }
    ...
    */
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake != null && snake.f51x == 0 && snake.f52y == 0) {
                return;
            }
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                for (int i7 = 0; i7 < snake.size; i7++) {
                    int i8 = snake.f51x + i7;
                    int i9 = snake.f52y + i7;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f51x + i3;
                int i5 = snake.f52y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    ...
                    int i7 = snake.f51x;
                    if ((iArr[i7 + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(i7 + i6, 1, this.mCallback.getChangePayload(i7 + i6, snake.f52y + i6));
                    }
                }
                i = snake.f51x;
                i2 = snake.f52y;
