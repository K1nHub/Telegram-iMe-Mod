...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    */
    ...
    private float computeTargetVelocity(int i, float f, float f2, float f3) {
        ...
        int i2 = (edgeValue > BitmapDescriptorFactory.HUE_RED ? 1 : (edgeValue == BitmapDescriptorFactory.HUE_RED ? 0 : -1));
        if (i2 == 0) {
    ...
    private float getEdgeValue(float f, float f2, float f3, float f4) {
        float interpolation;
        float constrain = constrain(f * f2, (float) BitmapDescriptorFactory.HUE_RED, f3);
        float constrainEdgeValue = constrainEdgeValue(f2 - f4, constrain) - constrainEdgeValue(f4, constrain);
        if (constrainEdgeValue < BitmapDescriptorFactory.HUE_RED) {
            interpolation = -this.mEdgeInterpolator.getInterpolation(-constrainEdgeValue);
        } else if (constrainEdgeValue <= BitmapDescriptorFactory.HUE_RED) {
            return BitmapDescriptorFactory.HUE_RED;
        } else {
            interpolation = this.mEdgeInterpolator.getInterpolation(constrainEdgeValue);
        }
    ...
    public static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            if (j3 < 0 || j < j3) {
                return AutoScrollHelper.constrain(((float) (j - j2)) / this.mRampUpDuration, (float) BitmapDescriptorFactory.HUE_RED, 1.0f) * 0.5f;
            }
            float f = this.mStopValue;
            return (1.0f - f) + (f * AutoScrollHelper.constrain(((float) (j - j3)) / this.mEffectiveRampDown, (float) BitmapDescriptorFactory.HUE_RED, 1.0f));
        }
